...
class SqlModelBuilder:
    ...
    def create_model(self, name, fields) -> type:
        ...
        model = None
        attrs = {"Meta": Meta, "__module__": f"{apps_label}.models", "objects": models.Manager()}
        if fields:
        ...
        try:
            with connection.cursor() as cursor:
                if hasattr(connection, "introspection"):
                    ...
                    table_info = {info.name: info for info in table_info if info.name == self.table}
                    if len(table_info) == 0:
                    ...
                    if hasattr(connection.introspection, "get_primary_columns"):
                        primary_key_columns = connection.introspection.get_primary_columns(
                            cursor, self.table
                        )
                        primary_key_column = (
                ...
                for row in table_description:
                    ...
                    # Add comment.
                    if hasattr(connection.features, "supports_comments") and row.comment:
