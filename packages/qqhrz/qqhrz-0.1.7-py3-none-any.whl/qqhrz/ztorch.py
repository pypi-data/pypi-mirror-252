...
class Vocab:
    ...
    def __init__(self, word_list, sepc=['<pad>'], freq=5):
        """
        构建词表
        :param word_list: 可以是一维的词的列表，也可以是二维的列表
        :param sepc: 一些特别的表示词，默认包含'<pad>'
        :param freq: 过滤出现次数低于freq的词，freq默认是5
        :return: 词表
        """
        if isinstance(word_list[0], (list, tuple, np.ndarray)):
            word_list = [words for arr in word_list for words in arr]
        self.sepc = ['<unk>'] + sepc
        self.__unk = 0
        # 词频数的字典
        word_freq_dict = collections.Counter(word_list)
        # 所有出现次数大于freq的词+spec的列表
        self.words = self.sepc + [w for w in word_freq_dict.keys() if word_freq_dict[w] > freq and w not in self.sepc]
        self.idx_to_token = self.words
        # 词索引字典
        self.word2index = {w: i for i, w in enumerate(self.words)}   
    
    def __getitem__(self, index):
        if isinstance(index, (list, tuple, np.ndarray)):
            return [self.word2index.get(i, self.__unk) for i in index]
        return [self.word2index.get(index, self.__unk)]
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, vocab_size, num_hiddens, num_lays, num_heads, normalized_shape, ffn_num_hiddens, dropout,
                 max_len=1000, use_bias=False):
        super(TransformerEncoder, self).__init__()
        self.num_hiddens = num_hiddens
        self.embedding = nn.Embedding(vocab_size, num_hiddens)
        self.position_encoder = PositionEncoder(num_hiddens, dropout, max_len)
        # 编码块
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, vocab_size, num_hiddens, num_lays, num_heads, normalized_shape, ffn_num_hiddens, dropout, max_len=1000):
...
def get_maskLM(tokens: list, vocab):
    ...
    for predict_position in candidate_predict_position:
        if tokens[predict_position] in ['<cls>', '<seq>']:
            continue
        ...
        predict_labels.append(vocab[tokens[predict_position]][0])
...
if __name__ == '__main__':
    words = [['i', 'like', 'it'], ['you', 'are', 'my', 'good', 'friend']]
    vocab = Vocab(words, sepc=['<pad>', '<mask>', '<cls>', '<sep>'], freq=0)
    print(get_bert_input(vocab, words, 15))
