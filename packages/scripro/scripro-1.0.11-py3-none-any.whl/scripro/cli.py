...
def get_target_score(args):
    piclkle_file = args.scripro_result
    tf = args.tf_name
    project = args.project
    print("Data preprocessing...")
    with open('piclkle_file', 'rb') as file:
        scripro_data = pickle.load(file)
    target = scripro_data.get_tf_target(tf)
    target.to_csv(project+'.csv', index=False, sep=',')
    return 
...
def add_target_parser( subparsers ):
    """Add main function 'enrich' argument parsers.
    """
    argparser_target = subparsers.add_parser("get_target", help="Calculate TF and target gene score")
