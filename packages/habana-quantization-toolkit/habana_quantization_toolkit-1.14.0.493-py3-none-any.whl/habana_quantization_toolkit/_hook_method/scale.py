...
def matmul_scales_to_mod_config(mod, scales, params):
  ...
  input_config = [quantdequant_config(None, functools.partial(cast_to_fp8_fcn, dtype=lp_dtype, scale_inv=s_inv), None, None, lp_dtype, hp_dtype) for s_inv in scales_inv.inputs]
  # outputs as bf16, and descaled in gemm under PatchedMatmul, so no need to work here
  output_config = quantdequant_config(None, None, None, None, lp_dtype, hp_dtype)
...
def linear_scales_to_mod_config(mod, scales, params):
  ...
  lp_dtype=params['lp_dtype']
  hp_dtype = params['hp_dtype']
  input_config = [quantdequant_config(None, functools.partial(cast_to_fp8_fcn, dtype=lp_dtype, scale_inv=scales_inv.inputs[0]), None, None, lp_dtype, hp_dtype)]
...
def kv_cache_scales_to_mod_config(mod, scales, params):
  ...
  hp_dtype = params['hp_dtype']
  input_config = [quantdequant_config(None, functools.partial(cast_to_fp8_fcn, dtype=lp_dtype, scale_inv=scales_inv.inputs[0]), None, None, lp_dtype, hp_dtype)]
  output_config = quantdequant_config(None, None, None, functools.partial(cast_from_fp8_fcn, dtype=hp_dtype, scale=scales.outputs), lp_dtype, hp_dtype)
  config=module_config(input_config, output_config)
  return config

def get_config(model, measurement, mod_dict, method, scale_convert_method, params, scales_file=None, recalc_scales=False, mod_list=None):
  with torch.no_grad():
    qconfig={}
    ...
    for mname in mod_list:
      ...
      # TODO SW-169781: Fix PCQ scalars and use scales_to_scalar directly
      scales[mname] = scale_convert_method(scales[mname])
...
# TODO SW-169781: Remove this for PCQ and use scales_to_scalar directly
scale_convert_methods = {
                  'without_scale': scales_to_scalar,
                  'unit_scale': scales_to_scalar,
                  'act_maxabs_pts_weight_maxabs_pts_pow2_hw': scales_to_scalar,
                  'act_maxabs_pts_weight_maxabs_pts_pow2': scales_to_scalar,
                  'act_maxabs_pts_pow2_hw_weights_maxabs_pcs_pow2': pass_scales,
                  'act_maxabs_pts_pow2_hw_weights_opt_pcs_pow2': pass_scales,
                  'smoothquant_weights_opt_pow2': pass_scales,
                  'smoothquant_weights_maxabs_pow2': pass_scales,
                  'weaksmoothquant_weights_maxabs_pow2': pass_scales,
}
