...
from habana_frameworks.torch.hpex.kernels.Fp8Ops import cast_to_fp8_v2, cast_from_fp8
...
cast_to_fp8_fcn = lambda x, dtype, scale_inv=None: cast_to_fp8_v2(x, scale_inv, False, False, dtype)[0]
cast_from_fp8_fcn = lambda x, dtype, scale=None: cast_from_fp8(x, scale, dtype)
...
def save_file(d, source_format, fname, mode):
  ...
  df={"GlobalRank": config['global_rank'], "LocalRank": config['local_rank'], "Mode": mode, "Nodes": dc}
  try:
