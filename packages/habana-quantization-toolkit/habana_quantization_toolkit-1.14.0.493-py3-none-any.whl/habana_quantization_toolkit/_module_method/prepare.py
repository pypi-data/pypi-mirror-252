...
from .._quant_common.helper_modules import BMM, ScaledDotProductAttention
from .._quant_common.quant_config import Fp8cfg

from . import QuantizedLinear, QuantizedConv2d, QuantizedBmm
from .modules import QuantizedScaledDotProductAttention
...
def remove_modules(model):
    modules = config['blacklist']['names']
    named_modules = get_named_modules(model, types=QuantizedModule)
    for name, module in named_modules:
        if name in modules:
            print('removed ' + name + ' from quantization because of blacklist')
            module.stop_quant()

# run mkpatch and set_names
def init_model(model):
    types = config['whitelist']['types']
