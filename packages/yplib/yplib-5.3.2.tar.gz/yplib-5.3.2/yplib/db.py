...
def get_table_sql(file_path):
    ...
    for one_table in table_list:
        # table_name, column_name_list, type_list, column_name_type_list, comment_list, info_list
        table_one_list = ['', [], [], [], [], []]
        # 遍历这个表的,解析出这个表结构数据
            ...
            if one_sql.lower().startswith('CREATE TABLE `'.lower()):
                ...
                if one_sql.startswith('`'):
                    ...
                    if column_match:
                        ...
                        table_one_list[3].append([column_name, column_type])
                        comment = ''
                        comment_index = -1
                        ...
                        table_one_list[4].append(comment.strip())
