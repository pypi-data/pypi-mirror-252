...
class CurlSession(BaseModel):
    ...
    def session(self):
        if not self._session:
            self._session = AsyncSession(timeout=3600, headers={
    ...
    async def generate(self, session: Union[AsyncSession, CurlSession]) -> NaiResult:
        ...
        try:
            ...
            with zip_file as zf:
                ...
                if not file_list:
                    raise ServerError(msg="Returned zip file is empty.")
                with zf.open(file_list[0]) as file:
                    ...
                    png_bytes.seek(0)
                    try:
                        _img_exif = NaiPic.read_from_img(png_bytes)
                    ...
                    png_bytes = _img_exif.generate_to_img_io(img_io=png_bytes)
                    png_bytes.seek(0)
            return NaiResult(
                ...
                files=(f"{str(self.input[:7])}-s{shortuuid.uuid()[:5]}.png", png_bytes.getvalue())
            )
