...
latex_maths_grammar = """
    ...
    product: WORD WORD+ | command command+ | WORD "\\\\cdot" command | command "\\\\cdot" command | WORD "\\\\times" command 
    ...
    circular_integral: "\\\\oint" command "\\\\d{" (WORD|vector) "}"
...
class LaTeXTransformerMaths(Transformer):
    ...
    def dx(self, items):
        if len(items[0]) == 1:
            ...
            return f"derivative of {items[0]} "
    ...
    def circular_integral(self, items):
        return f"circular integral of {items[0]} with respect to {items[1]}"
