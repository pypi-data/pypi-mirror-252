...
def download_model(model_id, hf_url='https://hf-mirror.com/', threads=10, exclude=None):
    ...
    for file in ls_files:
        ...
        status, _ = subprocess.getstatusoutput('aria2c -x {} -s {} -k 1M -c {} -d {} -o {}'.format(threads, threads, url, file_dir, file_name))
        if status != 0:
            os.system('rm ' + file)
...
def download_dataset(model_id, hf_url='https://hf-mirror.com/', threads=10, exclude=None):
    if model_id.find('/') < 1:
        print('model_id格式不对！')
        return None
