...
class video_decoder(MediaComplexNode):
    ...
    def __init__(self, name, device, params, node_attr):
        ...
        self.decode = fn._VideoDecoder(output_format=output_format,
                                       ...
                                       frames_per_clip=self.frame_per_clip)
        ...
        if(output_format == "rgb-i"):
            self.reshape_pre_gather = fn.Reshape(
                size=[channels, width, height, self.max_frame_vid * self.batch_size], tensorDim=4, layout='')
            self.reshape_post_gather = fn.Reshape(
                size=[channels, width, height, self.frame_per_clip, self.batch_size], tensorDim=5, layout='')
        elif(output_format == "rgb-p"):
            self.reshape_pre_gather = fn.Reshape(
                size=[width, height, channels, self.max_frame_vid * self.batch_size], tensorDim=4, layout='')
            self.reshape_post_gather = fn.Reshape(
                size=[width, height, channels, self.frame_per_clip, self.batch_size], tensorDim=5, layout='')
        else:
        ...
        print("video_decoder: op width {} op height {} frames_per_clip {} max_frame_vid {}".format(
            width, height, self.frame_per_clip, self.max_frame_vid))

    def __call__(self, files, vid_offset, resample=None, crop_val=None):
        if resample == None:
            video = self.decode(files, vid_offset)
        elif crop_val == None:
            video = self.decode(files, vid_offset, resample)
        else:
            video = self.decode(files, vid_offset, resample, crop_val)
