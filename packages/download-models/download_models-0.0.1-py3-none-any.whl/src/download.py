...
def _download_model(_repo_id, _repo_type):
    if _repo_type == "model":
        _local_dir = 'modeldata/models/' + _repo_id
    else:
        _local_dir = 'modeldata/datasets/' + _repo_id
    try:
    ...
    try:
        if _repo_type == "model":
            snapshot_download(repo_id=_repo_id, cache_dir=_cache_dir, ignore_patterns=["*.h5", "*.ot", "*.msgpack"], local_dir=_local_dir, local_dir_use_symlinks=_local_dir_use_symlinks,
