...
def _run_DIC_euler(images, **params_euler):
    ...
    if type(params_euler.mask) == str and params_euler.mask == 'auto':
        mask = imref > 0
    elif type(params_euler.mask) == str:
    ...
    params.output = True
    params.restart = 0
    params.result_path = result_path / f"step_{0:04d}_{0:04d}"
    ...
    if params_euler.restart == 0:
        ...
        copyfile(
            params.result_path / 'residuals' / f'res{0:05d}.vtk',
            result_path / 'residuals' / f'res{0:05d}.vtk'
        )
        copyfile(
            params.result_path / 'residuals' / f'res{1:05d}.vtk',
            result_path / 'residuals' / f'res{1:05d}.vtk'
        )

    mesh_step0, pd, cd = read_mesh(result_path / f'step_{0:04d}_{1:04d}' / 'fields' /  f'dic{1:05d}.vtk')
    Ucum = extrapolate(mesh_step0, pd['U'].T)
    ...
    for i in range(1, nsteps):
        if i >= params_euler.restart:
            ...
            if mask is not None:
                params.mask = interp(mask.astype('f4'), mesh_step0, -Ucum, out=0) >= 0.5

        mesh_init = mesh_step0.copy()
        mesh_init.xn += Ucum[0]
        mesh_init.yn += Ucum[1]
        if Ucum.shape[0] == 3:
            mesh_init.zn += Ucum[2]
        ...
        if i >= params_euler.restart:
            ...
            if params_euler.euler_remesh:
                ...
                for si, s in enumerate(params_euler.scales):
                    if isinstance(params_euler.meshes[si], str):
                        params_euler.meshes = tuple([read_mesh(fff, False) for fff in params_euler.meshes])
                    params.meshes[si] = params_euler.meshes[si].copy()
                    Ucum_s = interp_U(params.meshes[si], mesh_step0, Ucum, out='mean')
                    if len(Ucum_s) == 2:
                        from .fem import FEM
                        fem = FEM(params.meshes[si], 1)

                        fem.bcs = [
                            (params.meshes[si].outline, Ucum_s[:,params.meshes[si].outline])
                        ]

                        Uf = fem.solve()

                        params.meshes[si].xn += Uf[0]
                        params.meshes[si].yn += Uf[1]
                    else:
                        params.meshes[si].xn += Ucum_s[0]
                        params.meshes[si].yn += Ucum_s[1]
                    if len(Ucum_s) == 3:
                        params.meshes[si].zn += Ucum_s[2]
        ...
        U = extrapolate(mesh_out, np.vstack(U[0]))

        if params_euler.euler_remesh:
            Ucum += np.vstack(interp_U(mesh_init, mesh_out, U, 1))
        else:
            Ucum[:imref.ndim,:] += U
        ...
        if i >= params_euler.restart:
            ...
            res = abs(imref-interp(imdef, mesh_step0, Ucum))/(imref.max()-imref.min())
            if mask is not None:
                res[~params.mask] = 0

            if mask is not None:
                norm = pixsum_by_elts(mask, mesh_step0)
                cell_values['residual'] = np.full(mesh_step0.Ne, np.nan)
                cell_values['residual'][norm>0] = pixsum_by_elts(res, mesh_step0)[norm>0]/norm[norm>0]*100
            else:
                if mesh_step0.ndim == 2:
                    cell_values['residual'] = pixsum_by_elts(res, mesh_step0)/mesh_step0.surf*100
                else:
                    cell_values['residual'] = pixsum_by_elts(res, mesh_step0)/mesh_step0.vol*100
