...
class ReadData:
    ...
    """
    def __init__(self,xlsx_mysql_sql:str,needful_field:List[str]=None,select_sql=None,xlsx_sheet_index=0):
        """
        ...
        name=xlsx_mysql_sql
        if name.endswith('.xlsx'):#判断是否为.xlsx文件读取数据
            self.read_xlsx(name,xlsx_sheet_index)
        else:
            self.read_mysql(name,select_sql)#当做查询数据库数据
    ...
    def group(self,*args):
        ...
        for row in self.data[1:]:
            key=','.join([row[self.column_index[arg]] for arg in args])
            if key in data:
                data[key].append(row)
            else:
                data[key]=[row]
