...
class Systems():
    ...
    def CalculateActiveEntities(self) -> pg.Rect:
        _VIEWPORTRECT = self.camera.get_view_rect()
        self._ACTIVE = [print(entity) if "SpriteComponent" in entity and self._VIEWPORTRECT.colliderect(entity["SpriteComponent"].rect) else self.renderSystem._CleanUp(entity) for entity in _ENTITYMAP.values()]
