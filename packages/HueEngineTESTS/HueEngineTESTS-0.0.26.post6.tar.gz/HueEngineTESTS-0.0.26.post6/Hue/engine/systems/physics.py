...
class PhysicsSystem:
    ...
    def CalcFriction(self, componentV:VelocityComponent):
        if self.frictionX:
            if (componentV.x > 0.0):
                componentV.x -= self.f
    ...
    def ApplyGravity(self, _ACTIVE:list=[], dt:float=1.0) -> None:
    ...
    def AABB(self, _ACTIVE:list=[]) -> None:
        for eFor in _ACTIVE:
    ...
    def Update(self, _ACTIVE:list=[], dt:float=1.0) -> None:
        stepDelta = dt / self.SPF
        for step in range(self.SPF):
            self.ApplyGravity(_ACTIVE, dt=stepDelta)
            self.PositionalUpdates(_ACTIVE, dt=stepDelta)
            self.AABB(_ACTIVE)
