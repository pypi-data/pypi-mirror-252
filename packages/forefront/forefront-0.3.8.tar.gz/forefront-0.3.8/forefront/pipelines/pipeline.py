...
class Pipelines:
    ...
    def create(self, name: str, sample: List[str] = []):
...
class Pipeline():
    ...
    # def get_samples(self, limit: int = 10):
    #     # Check if an event loop is already running
    #     try:
    #         loop = asyncio.get_running_loop()
    #     except RuntimeError:  # If no loop is running, create one
    #         return asyncio.run(self.get_samples_async(limit))

    #     # If a loop is already running, create a new task in the existing loop
    #     if loop.is_running():
    #         return loop.create_task(self.get_samples_async(limit))
    #     else:  # If the loop is not running, start a new one
    #         return asyncio.run(self.get_samples_async(limit))
