...
class Haver:
    ...
    def read_df(self, haver_codes: list) -> pd.DataFrame:
        ...
        for haver_code in haver_codes:
            ...
            content_json = haver.read(database=database, series=series)
...
if __name__ == '__main__':
    ...
    haver = Haver(private_token=os.getenv('HAVER_TOKEN'),
                  verify=where(),
                  proxies={'http': 'http://ap-python-proxy:x2o7rCPYuN1JuV8H@app-gw-2.ecb.de:8080',
                           'https': 'http://ap-python-proxy:x2o7rCPYuN1JuV8H@app-gw-2.ecb.de:8080'})
    haver.get_series(database='EUDATA')
    tt = haver.read_df(['N997CE@EUDATA', 'N025CE@EUDATA'])
