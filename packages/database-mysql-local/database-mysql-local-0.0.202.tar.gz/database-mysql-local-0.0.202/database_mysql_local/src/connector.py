...
from .constants import logger_connector_object
from .cursor import Cursor

load_dotenv()

logger = Logger.create_logger(object=logger_connector_object)
connections_pool = {}
...
class Connector:
    ...
    def reconnect(self):
        ...
        try:
            ...
            use_query = f"USE `{self.schema}`;"
            try:
                self._cursor.execute(use_query)
                logger.info(f"set schema to database {self.schema} successfully")
            except mysql.connector.Error as err:
                logger.exception(use_query, object=err)
                logger.end()
                raise
        except mysql.connector.Error as err:
    ...
    def _connect_to_db(self):
        ...
        try:
            ...
            try:
                ...
                self._cursor.execute(use_query)
                logger.info(f"set schema to schema {self.schema} successfully")
            except mysql.connector.Error as err:
                logger.exception(use_query, object=err)
                logger.end()
                raise
    ...
    def close(self) -> None:
        ...
        try:
            ...
            logger.exception("connection.py close()", object=exception)
