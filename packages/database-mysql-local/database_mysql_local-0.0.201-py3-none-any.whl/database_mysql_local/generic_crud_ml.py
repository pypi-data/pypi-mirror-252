...
from .constants import DEFAULT_SQL_SELECT_LIMIT, logger_crud_ml_object
from .generic_crud import GenericCRUD
from .utils import validate_select_table_name

logger = Logger.create_logger(object=logger_crud_ml_object)
...
class GenericCRUDML(GenericCRUD):
    ...
    def get_id_by_name(self, name: str, lang_code: LangCode = None,
                       id_column_name: str = None, order_by: str = "") -> int | None:
        ...
        try:
            ...
            try:
                ...
                logger.warning("select_one_tuple_by_where was not successful for name and lang_code. "
                               "Error: " + str(exception), object={"name": name, "lang_code": lang_code})
                logger.exception("Error selecting data_json", object=exception)
                raise
        finally:
            logger.end(object={"result": result})
        if result:
    ...
    def get_ml_id_by_name(self, name: str, lang_code: LangCode = None,
                          id_column_name: str = None, order_by: str = "") -> int:
        logger.start(object={"name": name, "lang_code": lang_code})
    ...
    def delete_by_name(self, name: str,
        ...
        try:
            ...
            try:
                ...
                logger.warning("delete_by_where was not successful for name and lang_code. "
                               "Error: " + str(exception), object={"name": name, "lang_code": lang_code})
                logger.exception("Error deleting data_json", object=exception)
                raise
        finally:
            logger.end()
    ...
    def select_one_by_value_with_none_option(
        ...
        try:
            ...
            logger.exception(self._log_error_message(message="Error selecting data_json",  # noqa501
                                                     sql_statement=select_query), object=exception)  # noqa501
            logger.end()
            raise
