...
# TODO: verify that schema, table, and column names do not contain ; to prevent SQL injection
class GenericCRUD:
    ...
    def insert(self, *, schema_name: str = None, table_name: str = None, data_json: dict = None,
        ...
        self.set_schema(schema_name=schema_name or self.schema_name)
        if ignore_duplicate:
        ...
        if table_name in table_definition:
            if table_definition[table_name]["is_number_column"]:
                ...
                number = NumberGenerator.get_random_number(schema_name=self.schema_name, view_name=view_name)
                data_json["number"] = number
        else:
            logger.warning("Table not found in table_definition_table")
    ...
    def select_multi_tuple_by_id(self, *, schema_name: str = None, view_table_name: str = None,
                                 ...
                                 id_column_name: str = None,
                                 id_column_value: Any = None,
                                 limit: int = DEFAULT_SQL_SELECT_LIMIT, order_by: str = "") -> list:
    ...
    @staticmethod
    def _get_view_name(table_name: str) -> str | None:
        if table_name:
    ...
    def get_test_entity_id(self, *, entity_name: str, insert_function: callable, insert_kwargs: dict = None,
        ...
        Example: get_test_entity_id(entity_name='person_id', entity_creator=Person, insert_function=PersonsLocal.insert)
        """
        self.set_schema(schema_name=schema_name or self.schema_name)
        view_name = view_name or self.default_view_table_name

        fetched_result = self.select_one_dict_by_id(view_table_name=view_name,
                                                    id_column_name='is_test_data',
                                                    id_column_value='1',
                                                    select_clause_value=entity_name)
        if fetched_result:
            test_entity_id = fetched_result[entity_name]
        else:
            if entity_creator:
    ...
    # TODO Why is this commented? Do you think we should uncomment?
    # Answer: This is commented because we are not using it. I am not sure if we should delete it or not.
    # def _has_number_column(self, table_name: str) -> bool:
    #     try:
    #         table_data_dict = json.loads(self.table_definition_table_json_string)
    #         index = table_data_dict["table_name"].index(table_name)
    #         has_number_column = table_data_dict["is_number_column"][index]
    #         return has_number_column
    #     except Exception:
    #         return False

    # # TODO Currently we are not calling/using this method, we should
    # #   delete this when not in use anymore
    # @staticmethod
    # def _is_ml_table(table_name: str) -> bool:
    #     """ Returns True if the table is an ML table, False otherwise."""
    #     logger.warning("This function is deprecated")
    #     table_name_of_ml_child = table_name.endswith("_ml_table")
    #     # TODO Please logger.warning() if table is_ml_child_table <>  table_name_of_ml_child
    #     # TODO logger.end("...", table_name_of_ml_child)
    #     return table_name_of_ml_child
