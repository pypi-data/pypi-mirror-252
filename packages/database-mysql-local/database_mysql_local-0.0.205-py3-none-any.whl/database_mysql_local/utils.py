...
def validate_select_table_name(database_object_name: str) -> None:
    if os.getenv("environment_name") not in ("prod1", "dvlp1") and not database_object_name.endswith(
            "_view"):  # noqa501
        raise Exception(
...
def validate_none_select_table_name(database_object_name: str) -> None:
    if os.getenv("environment_name") not in ("prod1", "dvlp1") and not database_object_name.endswith(
            "_table"):  # noqa501
        raise Exception(
            f"Table name must end with '_table' in this environment  (got {database_object_name})")  # noqa501
...
def process_insert_data_json(data_json: dict | None) -> tuple[str, str, dict]:
    ...
    filtered_data_json = {key: value for key, value in data_json.items() if
                          not isinstance(value, ToSQLInterface)}  # noqa501
