...
<template>
  <div>
    ...
    <div v-else @pointerover="showAllScopes = true" @pointerleave="showAllScopes = false">
      <pre v-if="showAllScopes" class="ws-pre-wrap d-inline mb-0">{{ allScopes }}</pre>
      <div v-else>
        <pre class="ws-pre-wrap d-inline mb-0" :class="`d-${hoverTextBreakpoint}-block`">{{ firstShownScopes }}</pre>
        <strong class="ml-2" :class="`ml-${hoverTextBreakpoint}-0`">{{ scopesHoverText }}</strong>
      </div>
    </div>
  </div>
</template>
