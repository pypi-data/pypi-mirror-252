...
def calendar_conventional_commit_foxy(version: ScmVersion) -> str:
    return _conventional_commit_foxy(version, guess_next=guess_next_simple_calendar)


def semver_conventional_commit_foxy(version: ScmVersion) -> str:
    return _conventional_commit_foxy(version, guess_next=guess_next_simple_semver)


def _conventional_commit_foxy(version: ScmVersion, guess_next: Callable[Concatenate[ScmVersion, _P], str]) -> str:
    if version.exact:
...
def guess_next_simple_calendar(version: str, *, retain: int, increment: bool = True) -> str:
    try:
    ...
    if parts[-1] > CALENDAR_DECEMBER_MONTH and retain == VERSION_MINOR:
        parts[-2] += 1
        parts[-1] = 1
    while len(parts) < CALENDAR_DECEMBER_MONTH:
        parts.append(0)
...
if __name__ == "__main__":
    version = guess_next_simple_calendar(version="2024.12.1", retain=VERSION_MINOR)
    print(version)
