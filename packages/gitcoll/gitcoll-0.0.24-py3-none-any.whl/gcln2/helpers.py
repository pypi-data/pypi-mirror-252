...
def timestamp2isotime(timestamp: float):
    return datetime.datetime.utcfromtimestamp(timestamp).isoformat(timespec="seconds") + "Z"
        ...
        if dry_run:
            ...
            if cp.returncode and not allow_nonzero:
                if retry >= retries:
                    ...
                    retry += 1
                    logging.error(f"Error: {args} got return code {cp.returncode}, retry {retry}/{retries}")
            else:
                return cp.returncode
...
def uid_get_url_from_abs_path(repo_abs_url:str, parent_rem_url: str) -> Optional[str]:
    """return absolute url from absolute path, ie, add protocol/hostname. Or None if cannot be found"""
...
def rmtree_with_permissions(tree_path: str):
    ...
    shutil.rmtree(tree_path, onerror=set_rw)
