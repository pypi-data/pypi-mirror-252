...
class DkLangEtcUtils(object):
    ...
    @classmethod
    def f_remove_comments_v1(cls, py: str, *, 
                             _removeAstSentence:bool=False, 
                             _removeATVast:bool=False,
                             _removeSingleQuoteMuilti:bool=False) -> str:
        '''
        行尾的注释需要保留
    ...
    @classmethod
    def find_words(cls, texts: List[str], keyFun:Callable[[str],Any], *, minLen:int=6) -> Set[str]:
    ...
    def handle_files_100(cls, fs:List[Path], keyFun:Callable[[str],Any])->None:
        ...
        assert sorted(['ab', 'ba', 'caa'], key=lambda x: x[::-1]
                      ) == ['caa', 'ba', 'ab']
