...
PALETTIZABLE_MODULES = (nn.Linear, nn.Conv1d, nn.Conv2d, nn.Conv3d, nn.Embedding)
...
class Palettizer(ABC):
    ...
    def profile_mixed_bit_response(self,
        ...
        if top_k_in_default_dtype is not None:
            ...
            logger.info(f"Preserving precision of {len(layers_in_default_dtype)} layers with default dtype")
    ...
    def _sanity_check_per_layer_results(self):
        """ Ensure per-layer results consistently increase with increasing precision
        """
        inverted = 0
        total = 0
    ...
    def apply_recipe_coreml(self, recipe_key: str) -> None:
        ...
        def get_tensor_hash(tensor: np.ndarray) -> float:
            """ Ad-hoc method to hash np.float16 np.ndarrays to a np.float64 value
            """
        ...
        def op_selector(const):
            ...
            if parameter_tensor.dtype != np.float16:
                # These are the tensors that were compressed to constexpr in previous passes
                return False
            ...
            if pdist.min() != 0:
                logger.info(
                    f"\tParameter tensor with shape={parameter_tensor.shape}"
                    " was not found in the recipe, stays in float16!"
                )

                if tensor_hash not in float16_tensors:
                    float16_tensors.add(tensor_hash)
            ...
            # 16 is the only recipe entry which implies no palettization
            if target_nbits == 16 and hashes[matched] not in float16_tensors:
                float16_tensors.add(hashes[matched])
                precision_stats[16]['num_tensors'] += 1
                precision_stats[16]['numel'] += np.prod(parameter_tensor.shape)
                logger.info(
                    f"\t{target_module}:\tRecipe specifies float16, skipping palettization.")

            return False

        active_nbits = [int(nbits) for nbits in set(recipe.values()) if nbits in SUPPORTED_NBITS]

        for nbits in active_nbits:
            logger.info(f"\tProcessing tensors targeting {nbits}-bit palettes")
            current_nbits = nbits

            config = ct.optimize.coreml.OptimizationConfig(
               global_config=ct.optimize.coreml.OpPalettizerConfig(
                   mode="kmeans", nbits=nbits, weight_threshold=None),
               is_deprecated=True,
               op_selector=op_selector,
            )
            self.coreml_model = ct.optimize.coreml.palettize_weights(
                self.coreml_model, config=config)
            logger.info(
                f"\t{precision_stats[nbits]['num_tensors']} "
                f"tensors are palettized with {nbits} bits")
...
def _get_compressible_modules(model: torch.nn.Module,
    ...
    for name, module in model.named_modules():
        if hasattr(module, 'weight') and isinstance(module, PALETTIZABLE_MODULES):
            module_numel = getattr(module, 'weight').data.numel()
            if module_numel > min_numel:
    ...
    logger.debug(
        ...
        f"({compressible_percent * 100:.1f}% compressible)"
    )
