...
def fetch_html(
    ...
    # Convert.
    data_html = data_df.fetch_html(col_space=50, index=False, justify="center")
...
def fetch_csv(
    ...
    # Save file.
    data_df.fetch_csv(rfile.path, header=header, index=False, mode="a")
...
def fetch_excel(
    ...
    for sheet_name, sheet_df in sheets_table:
        sheet_df.fetch_excel(excel, sheet_name, index=False)
    excel.close()
