...
from typing import Any, Callable, Generic, Protocol, TypeAlias, cast, overload
...
class Dispatch(Protocol, Generic[State, Action, Event]):
    ...
    def __call__(
        self: Dispatch,
        callback: Callable[[State | None], Action | Event | list[Action | Event]],
    ) -> None:
        ...
...
def create_store(
    ...
    def dispatch(
        *parameters: DispatchParameters[Action, Event]
        | Callable[[State | None], DispatchParameters[Action, Event]],
    ) -> None:
        items = [
            item
