...
class EnginelessDataflow(Dataflow):
    ...
    @track(get_logger, custom_dimensions={'app_name': _APP_NAME})
    def _to_pyrecords(self, use_new_flow=False):
        # need to default to native preppy flow until core releases download code handling rslex records
        if use_new_flow:
            ex = get_rslex_executor()
            error = None
            record_count = None
