...
import pyrogramv1
from pyrogramv1 import __version__, __license__
from pyrogramv1 import raw
from pyrogramv1 import utils
from pyrogramv1.crypto import aes
from pyrogramv1.errors import CDNFileHashMismatch
from pyrogramv1.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    AuthBytesInvalid, BadRequest
)
from pyrogramv1.handlers.handler import Handler
from pyrogramv1.methods import Methods
from pyrogramv1.session import Auth, Session
from pyrogramv1.storage import Storage, FileStorage, MemoryStorage
from pyrogramv1.types import User, TermsOfService
from pyrogramv1.utils import ainput
...
class Client(Methods, Scaffold):
    ...
    def load_config(self):
        ...
        if self.bot_token:
            ...
            self.bot_token = parser.get("pyrogramv1", "bot_token", fallback=None)

        if self.api_id and self.api_hash:
            pass
        else:
            if parser.has_section("pyrogramv1"):
                self.api_id = parser.getint("pyrogramv1", "api_id")
                self.api_hash = parser.get("pyrogramv1", "api_hash")
            else:
    ...
    async def get_file(
        ...
        try:
            ...
            if not isinstance(e, pyrogramv1.StopTransmission):
                log.error(e, exc_info=True)
