...
def register(
    ...
    **kwargs,
    ...
    # ensure the adata object is validated
    validated = validate(
        adata,
        organism=organism,
    ...
    file = ln.Artifact.from_anndata(
        adata, description=description, field=lb.Gene.ensembl_gene_id, organism=organism
    )
    file.save()

    # link validated obs metadata
    for feature, labels in feature_labels:
        file.labels.add(labels, feature)

    logger.print(
        "\n\n🎉 successfully registered file in LaminDB!\nview it in the hub"
        f": https://lamin.ai/{ln.setup.settings.instance.identifier}/record/core/Artifact?uid={file.uid}\n\n"
    )
