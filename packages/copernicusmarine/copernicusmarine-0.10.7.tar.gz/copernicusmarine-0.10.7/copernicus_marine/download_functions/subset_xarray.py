...
from copernicus_marine.core_functions import sessions
from copernicus_marine.core_functions.exceptions import (
...
def _update_dataset_coordinate_valid_minmax_attributes(
    ...
    for coordinate_label in COORDINATES_LABEL:
        for coordinate_alias in COORDINATES_LABEL[coordinate_label]:
            if coordinate_alias in dataset.coords:
...
def check_dataset_subset_bounds(
    ...
    if service_type in [
        ...
        dataset_coordinates = sessions.open_zarr(
        ...
        dataset_coordinates = xarray.open_dataset(store).coords
    for coordinate_label in COORDINATES_LABEL["latitude"]:
        if coordinate_label in dataset_coordinates:
    ...
    for coordinate_label in COORDINATES_LABEL["longitude"]:
        if coordinate_label in dataset_coordinates:
            ...
            _check_coordinate_overlap(
                dimension="longitude",
                user_minimum_coordinate_value=longitude_modulus(
                    dataset_subset.minimum_longitude
                )
                if dataset_subset.minimum_longitude
                else longitudes.min(),
                user_maximum_coordinate_value=longitude_modulus(
