...
class generalized_stiefel_jax(basic_manifold_jax):
    def __init__(self, var_shape, B = lambda X:X ,train_mode = False, device = None) -> None:
        n = var_shape[0]
        p = var_shape[1]
        self._n = n
        self._p = p
        ...
        if not train_mode:
            super().__init__('Generalized_Stiefel',(n,p), (p,p), device = self.device)

        self.Ip = jax.device_put(jnp.eye(self._p) , self.device)
