...
class grassmann_jax(basic_manifold_jax):
    def __init__(self, var_shape, train_mode = False, device = None) -> None:
        if len(var_shape) >= 2:
            self._n = var_shape[-2]
            self._p = var_shape[-1]
        ...
        self.device = device
        if not train_mode:
            super().__init__('grassmann',var_shape, (*self.dim, self._p,self._p), device=self.device)
        
        self.Ip = jax.device_put(jnp.reshape(jnp.outer(jnp.ones(self.dim), jnp.eye(self._p)),(*self.dim, self._p,self._p)), self.device)
