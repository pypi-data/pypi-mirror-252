...
class grassmann_torch(basic_manifold_torch):
    def __init__(self, var_shape, device = torch.device('cpu'), dtype = torch.float64) -> None:
        ...
        super().__init__('Grassmann',var_shape, (*self.dim, self._p,self._p),   device= self.device ,dtype= self.dtype)
    ...
    def JA(self, X: Tensor, G: Tensor) -> Tensor:
        return torch.matmul(G , ( self.Ip - 0.5 * self.C(X) ))  - torch.matmul(X , self.Phi(torch.matmul(X.transpose(-2,-1), G)) )

    def JA_transpose(self,X: Tensor, G: Tensor) -> Tensor:
        # JA is self-adjoint
        return self.JA(X,G)

    def hessA(self, X: Tensor, gradf: Tensor, D: Tensor) -> Tensor:
    ...
    def JC(self, X: Tensor, Lambda: Tensor) -> Tensor: 
    ...
    def generate_cdf_fun(self, obj_fun : Callable, beta: float):
        def local_obj_fun(X):
            CX = self.C(X)
            AX = X - 0.5 * X@ CX
    ...
    def generate_cdf_grad(self, obj_grad: Callable, beta: float):
        def local_grad(X):
            CX = self.C(X)
    ...
    def generate_cdf_hess(self, obj_grad: Callable, obj_hess: Callable, beta: float):
        def local_hess(X, D):
            CX = self.C(X)
            AX = X - 0.5 *  torch.matmul(X,CX)
            gradf = obj_grad(AX)
    ...
    def generate_cdf_hess_approx(self, obj_grad: Callable, obj_hess: Callable, beta: float):
        def local_hess(X, D):
            CX = self.C(X)
            AX = X - 0.5 *  torch.matmul(X,CX)
            gradf = obj_grad(AX)
