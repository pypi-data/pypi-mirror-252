...
class backbone_torch:
    def __init__(self,*args, **kwargs) -> None:
        '''
            Initialize the backbone with the specified device and data type, defaulting to CPU and float64.
        '''
        ...
        # Define common tensor operations for convenience.
    ...
    def auto_diff_vjp(self, fun, X: Tensor, D:Tensor):
        # Compute the vector-Jacobian product (backward mode autodiff) for function `fun`.
        fun_vjp, val = torch.autograd.functional.vjp(fun, X, D, create_graph=True)
        # val.requires_grad = True
        return val

    def auto_diff_jvp(self, fun, X: Tensor , D: Tensor):
        # Compute the Jacobian-vector product (forward mode autodiff) for function `fun`.
        val, jvp_result = torch.autograd.functional.jvp(fun, X, v = D, create_graph=True)
        # jvp_result.requires_grad = True
        return jvp_result


    def auto_diff_jacobian(self, fun, X: Tensor):
        # Compute the full Jacobian matrix for function `fun`.
    ...
    def linear_map_adjoint(self, fun,D: Tensor):
    ...
    def autodiff(self, obj_fun, obj_grad = None, manifold_type = 'S'):
        # Automatically differentiate the objective function `obj_fun`. If gradients are provided, use them; otherwise compute using autodiff.
    ...
    def array2tensor(self, X_array: np.ndarray):
        # Convert a NumPy array to the variables of the manifold with the specified device and data type.
        X = torch.as_tensor(X_array).to(device = self.device, dtype = self.dtype)
        X.requires_grad = True 
        return X

    def tensor2array(self, X_tensor: Tensor, np_dtype = np.float64):
        # Convert the variables of the manifold to a NumPy array with the specified data type.
