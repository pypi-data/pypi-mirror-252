...
class stiefel_range_constraints_np(basic_manifold_np):
    def __init__(self, var_shape, e_vec = None, tau = 1) -> None:
        self._n = var_shape[-2]
        self._p = var_shape[-1]
        self.dim = var_shape[:-2]

        super().__init__('Stiefel with range constraints', var_shape, (self._n + self._p ** 2,))
        ...
        if e_vec is None:
            self.e_vec = np.ones((self._n,1))/np.sqrt(self._n)
        else:
            self.e_vec = e_vec
    ...
    def C(self, X:np.ndarray):
        XX = X.swapaxes(-2,-1) @ X
        eX = (self.e_vec).swapaxes(-2,-1) @ X
        return np.concatenate(((XX-self.Ip).flatten(),  self.tau * (X @ eX.swapaxes(-2,-1) - self.e_vec).flatten() ),axis = 0)
    ...
    def JC(self, X, Lambda):
        T = np.reshape(Lambda[:self._p **2], (self._p, self._p) )
        gamma = np.reshape(Lambda[self._p **2:], (self._n, 1))
        return 2 * X@ T  + self.e_vec @ (gamma.T @ X) + self.tau * gamma @ ((self.e_vec).swapaxes(-2,-1) @ X) 
        


    def Init_point(self, Xinit: np.ndarray = None):
        if Xinit is None:
            Xinit = np.random.randn(*self.var_shape)
            Xinit[...,:,0] = np.reshape(self.e_vec, (self._n, ))
        

        if self.Feas_eval(Xinit) > 1e-6:
            UX, SX, VX = np.linalg.svd(Xinit, full_matrices= False)
            Xinit = np.matmul(UX, VX.swapaxes(-2,-1))
        return Xinit