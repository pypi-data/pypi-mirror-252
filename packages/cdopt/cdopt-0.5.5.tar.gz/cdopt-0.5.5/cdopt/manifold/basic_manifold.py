...
class basic_manifold(metaclass=abc.ABCMeta):
    """
    Abstract base class representing a basic manifold structure.

    Parameters:
    - name (str): The name of the manifold.
    - variable_shape (Tuple[int]): The shape of the variables on the manifold.
    - constraint_shape (Tuple[int]): The shape of the constraints on the manifold.
    - backbone (str): The autodiff package used for gradient computation. Defaults to 'torch'.
    - regularize_value (float): Regularization value used in computations to ensure numerical stability.
    - safeguard_value (float): Safeguard value to avoid division by zero in computations.

    Attributes:
    - var_num (int): Total number of variables, computed as the product of variable_shape dimensions.
    - con_num (int): Total number of constraints, computed as the product of constraint_shape dimensions.
    - manifold_type (str): Type of manifold, here initialized as 'S' (could represent a sphere).
    - var_type (str): The type of variables used, depending on the backbone.
    
    Methods:
    - v2m, m2v: Methods to convert between vector and manifold representations.
    - array2tensor, tensor2array: Methods to convert between numpy arrays and the backbone's tensor representation.
    - A: Method to compute the constraint dissolving mapping corresponding to the manifold.
    - JA, JC: Methods to compute the Jacobian actions.
    - JA_transpose, JC_transpose: Methods to compute the transposed Jacobian actions.
    - C_quad_penalty: Method to compute the quadratic penalty of constraints.
    - hessA, hess_feas: Methods to compute Hessians.
    - Feas_eval: Method to evaluate feasibility of a point on the manifold.
    - Post_process: Method to post-process points by projecting them on the manifold.
    - generate_cdf_fun, to_cdf_fun: Methods to generate and transform constraint dissolving functions.
    - generate_cdf_grad, to_cdf_grad: Methods to generate and transform gradients of constraint dissolving functions.
    - generate_cdf_hess, generate_cdf_hess_approx: Methods to generate Hessians for constraint dissolving functions.
    
    The class "basic_manifold" requires implementation of methods C, v2m, m2v, and Init_point in derived classes.
    All the other essential methods are automatically generated based on automatic differentiation provided by self.backbone. 
    """
    ...
    def __init__(self, name: str, variable_shape: Tuple[int], constraint_shape: Tuple[int], backbone = 'torch',  regularize_value: float = 0.01, safeguard_value:float = 0, **kwargs) -> None:
        self.name = name
    ...
    def A(self, X):
        # The default approach for constructing the constraint dissolving mapping, from C and its Jacobians.
        local_JC =  lambda y: self.C(self.v2m(y)).flatten()
        JC_mat = self.jacobian(local_JC,self.m2v(X))
        C_vec = (self.C(X)).flatten()
...
class complex_basic_manifold(metaclass=abc.ABCMeta):
    """
    Abstract base class for manifolds defined with complex variables. 

    Parameters:
    - name (str): Name of the manifold.
    - variable_shape (Tuple[int]): Shape of the variables on the manifold.
    - constraint_shape (Tuple[int]): Shape of the constraints on the manifold.
    - backbone (str): The autodiff package used for gradient computation.
    - regularize_value (float): Regularization value used for numerical stability.
    - safeguard_value (float): Safeguard value to prevent numerical issues.

    Attributes:
    - var_num (int): Total number of variables on the manifold.
    - con_num (int): Total number of constraints on the manifold.
    - manifold_type (str): Type of manifold, initialized as 'S'.
    - var_type (str): Type of variables, depending on the backbone.
    ...
    The class "complex_basic_manifold" requires implementation of methods C, v2m, m2v, and Init_point in derived classes.
    All the other essential methods are automatically generated based on the automatic differentiation provided by self.backbone. 
    The employed backbone must support the automatic differentiation for complex functions. 
    """
