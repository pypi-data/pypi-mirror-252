...
class symp_stiefel_torch(basic_manifold_torch):
    def __init__(self, var_shape, device = torch.device('cpu'), dtype = torch.float64) -> None:
        ...
        super().__init__('Symp_Stiefel',(n,p), (p,p),   device= self.device ,dtype= self.dtype)
    ...
    def Init_point(self, Xinit = None):
        ...
        for jl in range(10):
            X = X - 0.5/( torch.linalg.norm(X.T @ X, 2) )   * self.JC(X,self.C(X))
        ...
        for jl in range(10):
            ...
            if feas < 1e-2:
                ...
                X = torch.linalg.solve(TTT.T, X.T ).T
            if feas < 1e-8:
                break
