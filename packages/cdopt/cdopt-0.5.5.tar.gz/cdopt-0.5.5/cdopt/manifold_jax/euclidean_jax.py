...
class euclidean_jax(basic_manifold_jax):
    def __init__(self, var_shape, train_mode = False, device = None) -> None:

        self.dim = jnp.prod(jnp.array(var_shape))
        ...
        self.zero_scalar = 0
        if not train_mode:
            self.zeroX = jnp.zeros(self.var_shape)
            super().__init__('eclidean',var_shape, (1,), device= self.device)
