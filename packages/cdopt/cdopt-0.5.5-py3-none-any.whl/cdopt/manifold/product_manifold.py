...
class product_manifold(basic_manifold):
    """
        This class represents a product manifold, which is formulated by the outer product (Cartesian product) of a list of individual manifolds.

        Attributes:
        - name (str): The name of the manifold, set to 'Product Manifold'.
        - manifold_type (str): The type of manifold, set to 'P' to indicate a product manifold.
        - list_manifold (Tuple): A tuple containing the individual manifolds that make up the product manifold.
        - var_shape (Tuple): A tuple of variable shapes for each individual manifold in the product manifold.
        - vectorized_length (Tuple): A tuple containing the lengths of vectorized forms of individual manifolds.
        - dim_tuple (Tuple): A tuple containing dimensions for each individual manifold.
        - split_index (np.array or int): Indices to split a concatenated vector into vectors corresponding to each individual manifold.
        - var_num (int): Total number of variables across all manifolds in the product.
        - shape (Tuple): The shape of the product manifold, which is a single dimension tuple of var_num.
        - backbone: The autodiff backbone used for the product manifold, inherited from the first manifold in the list.

        Methods:
        - v2m: Converts a list of vectors into a tuple of manifold points corresponding to each individual manifold.
        - m2v: Converts a list of points on individual manifolds into a tuple of vectors.
        - array2tensor: Splits a concatenated vector and converts each part to the tensor format of the respective manifold's backbone.
        - tensor2array: Converts a list of tensors from the backbone's format to a concatenated NumPy array.
        - A: Applies the retraction operation to each point in the list for the corresponding manifold.
        - JA: Applies the Jacobian action for each individual manifold to its corresponding elements in X_list and G_list.
        - JA_transpose: Assumes JA is self-adjoint and applies it to each element in X_list and G_list.
        - hessA: Computes the Hessian action for each manifold given points X_list, gradients G_list, and directions D_list.
        - JC: Computes the Jacobian action of constraints for each manifold with respect to X_list and Lambda_list.
        - C: Computes constraints for each point in X_list on their respective manifolds.
        - C_quad_penalty: Computes the sum of quadratic penalties of constraints across all manifolds for points in X_list.
        - hess_feas: Computes the Hessian of feasibility conditions for each manifold given points X_list and directions D_list.
        - Feas_eval: Evaluates the feasibility of points on the product manifold by computing the square root of C_quad_penalty.
        - Init_point: Initializes points on each individual manifold, either based on provided Xinit or default initial points.
        - Post_process: Applies post-processing to points on each individual manifold.
        - generate_cdf_fun: Generates a constraint dissolving function for the product manifold using an objective function and penalty parameter beta.
        - generate_cdf_grad: Generates the gradient of a constraint dissolving function for the product manifold using an objective gradient and penalty parameter beta.
        - generate_cdf_hess: Generates the Hessian of a constraint dissolving function for the product manifold using an objective gradient, objective Hessian, and penalty parameter beta.

        The class "product_manifold" is designed to handle operations across multiple manifolds simultaneously by leveraging their individual properties and operations. 
    """



    def __init__(self, list_manifold: Tuple) -> None:
        ...
        self.vectorized_length = tuple(M.vectorized_length for M in list_manifold)
        self.dim_tuple = tuple(vectorized_length for vectorized_length in self.vectorized_length)
        if len(list_manifold) > 1:
            self.split_index = np.array( tuple(np.sum(self.dim_tuple[:i+1]) for i in range(len(self.dim_tuple) )) )
        else:
            self.split_index = 1

        self.var_num = np.sum(self.dim_tuple)
        self.shape = (self.var_num,)
    ...
    def Init_point(self, Xinit = None):
        if Xinit is None:
            ...
            return tuple(M.Init_point(Xinit_local) for M, Xinit_local in zip(self.list_manifold, Xinit))
    ...
    # 2024/01/25
    # Modified self.generate_cdf_grad() and self.generate_cdf_hess()
