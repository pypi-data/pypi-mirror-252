...
from ...nn.modules.utils import get_quad_penalty
...
def get_quad_penalty_call(
    ...
    if (
        ...
        raise RuntimeError("The stateless API can't be used with Jitted modules")
    with _reparametrize_module(module, parameters_and_buffers):
        out = get_quad_penalty(module)
    return out


def functional_quad_penalty_call(
    module: 'torch.nn.Module',
    parameters_and_buffers: Dict[str, Tensor],
    args: Tuple,
    kwargs : Dict[str, Any] = None,
    ...
    with _reparametrize_module(module, parameters_and_buffers):
        if isinstance(args, tuple):
            ...
            out = module(args, **kwargs)
        quad_penalty = get_quad_penalty(module)
    return out, quad_penalty