...
class Substation:
    ...
    def __init__(self, name: str):
        self.app_division = name
        self.request_store = f"_SP_REQUEST_{name}"
        self.response_store = f"_SP_RESPONSE_{name}"
    ...
    def make_request(self, input_data: Dict[str, Any]) -> Dict[Any, Any]:
        ...
        request_data = {"request": resp.json()}
        return request_data
            ...
            if kv_store.exists(self.response_store, request["id"]):
                ...
                if type(data["output"]) is list:
                    ...
                    if "https://replicate.delivery" in output:
                        ...
                        obj_name = f"{msg.meta['_seaplane_request_id']}.{output.split('.')[-1]}"
            ...
            if kv_store.exists(self.request_store, data["id"]):
                ...
                if type(data["output"]) is list:
                    ...
                    if "https://replicate.delivery" in output:
                        ...
                        obj.upload(bucket, obj_name, response.content)
                        data["output"] = {"bucket": bucket, "object": obj_name}
                for key in ("logs", "urls", "version", "webhook", "webhook_events_filter"):
                    data.pop(key, "")
...
def substation_dag(app: App, name: str, input: Optional[EdgeFrom] = None) -> Any:
    dag = app.dag(name)
    if input is None:
        input = app.input()
    substation = Substation(app.name)
    ...
    request = dag.task(make_request, [input])

    response_sub = f"{substation.results_stream()}.{app.name}.>"
    response = dag.task(
