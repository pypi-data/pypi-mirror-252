...
class UMnetinfo(UMnetdb):
    ...
    def get_network_admins(self, netname:str, entity_types:list=[], rel_types:list=[]):
        """
        ...
        if rel_types:
            rel_where = " or ".join([f"i_a.ENTITYITEMRELTYPCD='{rel}'" for rel in rel_types])
    ...
    def get_device_admins(self, name_or_ip:str, entity_types:list=[], rel_types:list=[]):
        """
    ...
    def get_arphist(self, query:str, no_hist=False, no_device=False):
        """
        ...
        # IPv6 is stored in IP6NEIGHBOR
            ...
            raise ValueError(f"Unsupported input {query}, must be a MAC, a subnet, or an IP")
