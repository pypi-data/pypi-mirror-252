...
USAGE
    repos [st]                  # Lists all chanaged repos
    repos ls                    # Lists all repos
...
class Cli:
    ...
    def findCmd(self, *args):
        pattern = args[1]
        self.repos.list()
        found = self.repos.findNames(pattern)
        for name in found:
            print(f"- {name}")
    ...
    #     repo = Repo(path)
    #     # configFile = os.path.join(path, ".git", "repo.yaml")
    ...
    def configCmd(self, *args):
        if len(args) < 2:
            self.repos.showConfigs()
        elif len(args) < 3:
            names = args[1].split(",")
            self.repos.showConfig(names)
        elif len(args) < 4:
            names = args[1].split(",")
            self.repos.getConfig(names, args[2])
        else:
            repos = args[1].split(",")
            for repo in repos:
                self.repos.setConfig(repo, args[2], args[3])
...
def findTopDir(root: str) -> str:
    ...
    if repo.git:
        ...
        # print(f"\033[33;1mWarning:\033[0m You are inside a git repo. Using \033[32;1m{root}\033[0m instead.", file=sys.stderr)
