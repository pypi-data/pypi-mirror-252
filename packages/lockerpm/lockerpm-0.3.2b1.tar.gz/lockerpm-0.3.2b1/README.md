...
### Get a secret value
This func will get the secret value by a key. If the Key does not exist, SDK will return the default_value
...
You could get a secret value by secret key and specific environment name by `environment_name` parameter. 
...
Use `.create()` function to create new secret

```
secret = locker.create(key="YOUR_NEW_SECRET_KEY", value="YOUR_NEW_SECRET_VALUE")
```

Create new secret with specific environment:
...
To create new environment, use `.create_environment()`
