...
class PortfolioApi(object):
    ...
    def portfolio_model_retrieve(self, **kwargs):  # noqa: E501
        ...
        ## Portfolio Model Data API |입력변수              |설명                                                    |필수  |기본값      |  |:--------------------|:-------------------------------------------------------|:-----|:-----------|  |identity_name        |포트폴리오 모델 identity name                            |O     |            |  |end                  |받을 데이터의 마지막 인덱스(날짜)                          |X     |request 시점 | |code_format          |데이터 칼럼 형식: ccid, isin, short_code 중 택1           |X     |ccid        | |tail                 |받을 데이터의 row 개수: 마지막 인덱스부터 tail 값 만큼, 1 이상 1000 이하값으로 제한됨 |X     |20 |      |출력변수  |설명                 |타입                 |  |:--------|:--------------------|:--------------------|  |unix_timestamp||int| |pm       |포트폴리오 모델 데이터 |string (json-string) | |meta     |출력값 메타 정보      |dict                 |   ### meta dict 구성  - identity_name - code_format - tail - start_date - last_date   ### 'pm'으로 받은 포트폴리오 모델 문자열 데이터프레임 변환 - pd.read_json(pm, orient='index')  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
