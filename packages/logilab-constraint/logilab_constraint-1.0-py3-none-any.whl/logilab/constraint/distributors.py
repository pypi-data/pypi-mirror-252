...
class AbstractDistributor:
    ...
    def findSmallestDomain(self, domains):
        ...
        domlist = [
            (dom.size(), variable)
            for variable, dom in domains.items()
            if dom.size() > 1
        ]
        domlist.sort()
        return domlist[0][1]
    ...
    def _distribute(self, *args):
        ...
        raise NotImplementedError(
            "Use a concrete implementation of " "the Distributor interface"
        )
...
class NaiveDistributor(AbstractDistributor):
    ...
    def _distribute(self, dom1, dom2):
        ...
        if self.verbose:
            print("Distributing domain for variable", variable, "at value", values[0])
...
class RandomizingDistributor(AbstractDistributor):
    ...
    def _distribute(self, dom1, dom2):
        ...
        if self.verbose:
            print("Distributing domain for variable", variable, "at value", distval)
...
class SplitDistributor(AbstractDistributor):
    ...
    def _distribute(self, *args):
        ...
        nb_elts = max(1, len(values) * 1.0 / nb_subspaces)
        slices = [
            (int(math.floor(index * nb_elts)), int(math.floor((index + 1) * nb_elts)))
