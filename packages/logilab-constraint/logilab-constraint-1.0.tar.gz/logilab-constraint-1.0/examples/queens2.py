    ...
    for r1 in range(size):
        ...
        for r2 in range(r1 + 1, size):
            ...
            c = fd.make_expression(
                (q1, q2),
                f"{D['q1']} != {D['q2']} and {D['diag']} != abs({D['q1']}-{D['q2']})",
            )
            constraints.append(c)

    r = Repository(variables, domains, constraints)
    Distrib = distributors[distrib]
    yield from Solver(Distrib()).solve_all(r, verbose)
    ...
    for o, v in opts:
        if o == "-d":
            ...
            if v in distributors:
                ...
                raise RuntimeError(
                    f"Distributor must be one of {', '.join(distributors.keys())}"
                )
    count = 0
