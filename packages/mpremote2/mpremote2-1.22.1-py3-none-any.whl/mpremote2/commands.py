        ...
        if dev == "list":
            ...
            for p in sorted(serial.tools.list_ports.comports()):
                if p.vid is not None and p.pid is not None:
                    try:
                        state.transport = SerialTransport(p.device, baudrate=115200, hard_reset=state.hard_reset)
                        return
            ...
            if dev.startswith("port:"):
                dev = dev[len("port:") :]
            state.transport = SerialTransport(dev, baudrate=115200, hard_reset=state.hard_reset)
            return
    except TransportError as er:
        msg = er.args[0]
...
def do_hard_reset(state, _args=None):
