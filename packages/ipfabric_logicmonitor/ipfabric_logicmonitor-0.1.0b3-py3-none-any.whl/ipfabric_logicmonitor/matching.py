...
class DeviceMatching:
    ...
    def report(self, company: str):
        ipf_unmatched = [
            vars(MergedDevice(**d.merge())) for d in self.ipf.unmatched.values()
        ]
        lm_unmatched = [
            vars(MergedDevice(**d.merge(company))) for d in self.lm.unmatched
        ]
        matched = [
            vars(MergedDevice(**d.merge(), **self.lm.matched[s].merge(company)))
            for s, d in self.ipf.matched.items()
        ]
