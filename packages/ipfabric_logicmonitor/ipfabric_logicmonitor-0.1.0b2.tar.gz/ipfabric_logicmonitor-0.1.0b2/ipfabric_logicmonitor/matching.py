...
from ipfabric_logicmonitor.models import LMDevice, MergedDevice
...
class DeviceMatching:
    ...
    def report(self, company: str):
        ipf_unmatched = [MergedDevice(**d.merge()) for d in self.ipf.unmatched.values()]
        lm_unmatched = [MergedDevice(**d.merge(company)) for d in self.lm.unmatched]
        matched = [MergedDevice(**d.merge(), **self.lm.matched[s].merge(company)) for s, d in self.ipf.matched.items()]
        return ipf_unmatched, lm_unmatched, matched
