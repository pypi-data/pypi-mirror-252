...
class LMAttributes:
    ...
    url: Optional[
        str
    ] = None  # https://company.logicmonitor.com/santaba/uiv3/device/index.jsp#tree/-17-d-5
...
class IPFDevice(Device, BaseModel):
    ...
    def merge(self) -> dict:
        return {
            "ipf_hostname": self.hostname,
            "ipf_fqdn": self.fqdn,
            "ipf_login_ip": str(self.login_ip.ip) if self.login_ip else None,
            "ipf_site": self.site,
            "ipf_vendor": self.vendor,
            "ipf_family": self.family,
            "ipf_platform": self.platform,
            "ipf_model": self.model,
            "ipf_serial_number": self.sn,
            "ipf_hw_serial_number": self.sn_hw,
            "ipf_version": self.version,
            "ipf_device_type": self.dev_type,
        }
