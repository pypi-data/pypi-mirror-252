...
class LMDevice(Device):
    ...
    @property
    def url(self) -> str:
        group = self.host_group_ids.split(",")[0]
        return f"/santaba/uiv3/device/index.jsp#tree/-{group}-d-{self.id}"
    ...
    def merge(self, company: str) -> dict:
        url = urljoin(f"https://{company}.logicmonitor.com", self.url)
        return {
            "lm_display_name": self.display_name,
            "lm_name": self.name,
            "lm_hostname": getattr(self.sys, "hostname", None),
            "lm_sysname": getattr(self.sys, "sysname", None),
            "lm_description": self.description,
            "lm_host_status": self.host_status,
            "lm_groups": ",".join(list(getattr(self.sys, "groups", {}))),
            "lm_id": self.id,
            "lm_network_address": getattr(self.auto, "network_address", None),
            "lm_endpoint_serial_number": getattr(
                self.auto, "endpoint_serial_number", None
            ),
            "lm_entphysical_serialnum": getattr(
                self.auto, "entphysical_serialnum", None
            ),
            "lm_url": url,
        }
