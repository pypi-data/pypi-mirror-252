...
from . import config
from .data_object import AccInfo, KlineData, Position, TradeRecord
...
class BaseStrategy(ABC):
    ...
    async def get_params(self) -> dict:
        return await self._db.get_strategy_params(
            acc_id=self.acc_id, code=self.code, strategy_id=self.STRATEGY_ID
        )

    async def save_params(self, params: dict):
        await self._db.update_addnew_strategy_params(acc_id=self.acc_id, code=self.code, strategy_id=self.STRATEGY_ID,
                                                     params=params)
    ...
    async def get_acc_info(self) -> AccInfo:
        return await self.trade.get_acc_info(self.acc_id)

    async def get_position_count(self) -> int:
        return await self.trade.get_position_count(self.acc_id, self.STRATEGY_ID)

    async def get_position_list(self) -> list[Position]:
        return await self.trade.get_position_list(self.acc_id, self.STRATEGY_ID)

    async def get_trade_record(
            self, start: datetime = None, end: datetime = None, limit=5
    ) -> pd.DataFrame:
        return await self.trade.get_trade_record(
            acc_id=self.acc_id,
            code=self.code,
    ...
    async def buy(self, price: float, qty: int, remark: str = None, trade_count: int = None) -> TradeRecord:
        return await self.trade.buy(
            code=self.code,
            price=price,
            qty=qty,
            acc_id=self.acc_id,
    ...
    async def sell(self, price: float, qty: int, remark: str = None, trade_count: int = None) -> TradeRecord:
        return await self.trade.sell(
            code=self.code,
            price=price,
            qty=qty,
            acc_id=self.acc_id,
