...
class IsoBuilderDirs:
    ...
    def mkdirs(self):
        ...
        for dir in [self.source_mount, self.build_dir, self.output_dir]:
...
class IsoBuilder:
    ...
    def _run_command(
        ...
        try:
            ...
            return subprocess.run(
                shlex.split(command), check=check, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
    ...
    def write_ioslinux_files(self) -> None:
        ...
        (self._dirs.build_dir / "isolinux" / "isolinux.bin").write_text(ISOLINUX_CFG)
