...
def regression_score(data, label_true, label_pred, score_type='r2',
                     tweedie_power=1.5, quantile_alpha=0.5):
    r"""
    Compute regression scores w.r.t. different types of objective functions.
    """
    conn = data.connection_context
    require_pal_usable(conn)
    label_true = arg('label_true', label_true, str, required=True)
    label_pred = arg('label_pred', label_pred, str, required=True)
    score_type = arg('score_type', score_type, str)
    quantile_alpha = arg('quantile_alpha', quantile_alpha, float)
    quantile_alpha = 0.5 if score_type == 'ae' else quantile_alpha
    tweedie_power = arg('tweedie_power', tweedie_power, float)
    score_type = score_type.lower()
    if score_type not in ['r2', 'r2-log', 'ae', 'quantile', 'tweedie', 'gamma']:
        msg = f"Input objective type '{score_type}' not supported!"
        logger.error(msg)
        raise ValueError(msg)
    data_ = data.select(label_true, label_pred).rename_columns(
        ['ACTUAL', 'PREDICTED']).cast('ACTUAL', 'DOUBLE').cast('PREDICTED', 'DOUBLE')
    if data_.empty() or data_.hasna():
        msg = 'Input dataframe is empty or has NULL values.'
        logger.error(msg)
        raise ValueError(msg)
    base_val = f'SELECT AVG(TO_DOUBLE(ACTUAL)) AS AV FROM ({data_.select_statement})'
    if score_type == 'r2':
        pred_dev = 'SUM(POWER(TO_DOUBLE(ACTUAL) - TO_DOUBLE(PREDICTED), 2))'
    # total sum of squares
        base_dev = 'SUM(POWER(TO_DOUBLE(ACTUAL) - TO_DOUBLE(AV), 2))'
    elif score_type == 'r2-log':
        base_val = f'SELECT AVG(LN(TO_DOUBLE(ACTUAL) + 1)) AS AV FROM ({data_.select_statement})'
        pred_dev = 'SUM(POWER(LN(TO_DOUBLE(ACTUAL) + 1) - LN(TO_DOUBLE(PREDICTED) + 1), 2))'
    # total sum of squared-logs
        base_dev = 'SUM(POWER(LN(TO_DOUBLE(ACTUAL) + 1) - AV, 2))'
    elif score_type in ['ae', 'quantile']:
        base_val = f'SELECT PERCENTILE_CONT({quantile_alpha}) WITHIN ' +\
        f'GROUP (ORDER BY ACTUAL) AS BV FROM ({data_.select_statement})'
        pred_dev = f'SUM((0.5 + 0.5 * SIGN(PREDICTED - ACTUAL) - {quantile_alpha}) * (PREDICTED - ACTUAL))'
        base_dev = f'SUM((0.5 + 0.5 * SIGN(BV - ACTUAL) - {quantile_alpha}) * (BV - ACTUAL))'
    elif score_type in ['gamma', 'tweedie']:
        if score_type == 'gamma':
            if any(data_.filter(f'{col} > 0').count() != data_.count() for col in ['ACTUAL', 'PREDICTED']):
                msg = 'Score function for Gamma regression only applies when actual ' +\
                'and predicted values are all positive.'
                logger.error(msg)
                raise ValueError(msg)
            pred_dev = 'SUM(LN(TO_DOUBLE(PREDICTED) / TO_DOUBLE(ACTUAL)) + TO_DOUBLE(ACTUAL) / TO_DOUBLE(PREDICTED) - 1)'
            base_dev = 'SUM(LN(AV / TO_DOUBLE(ACTUAL)) + TO_DOUBLE(ACTUAL) / AV - 1)'
        else:
            p = tweedie_power
            p1 = (1.0 - p) * (2.0 - p)
            dcnt = data_.count()
            if data_.filter('ACTUAL >= 0').count() != dcnt or data_.filter('PREDICTED > 0').count() != dcnt:
                msg = 'Score function for tweedie regression only applies when actual values are ' +\
                'non-negative, and predicted values are all positive.'
                logger.error(msg)
                raise ValueError(msg)
            pred_dev = f'SUM(POWER(TO_DOUBLE(ACTUAL), 2 - {p}) / {p1} - ' +\
            f'ACTUAL * POWER(TO_DOUBLE(PREDICTED), 1 - {p}) / (1 - {p}) + '+\
            f'POWER(TO_DOUBLE(PREDICTED), 2 - {p}) / (2 - {p}))'
            base_dev = f'SUM(POWER(TO_DOUBLE(ACTUAL), 2 - {p}) / {p1} - ' +\
            f'ACTUAL * POWER(TO_DOUBLE(AV), 1 - {p}) / (1 - {p}) + '+\
            f'POWER(TO_DOUBLE(AV), 2 - {p}) / (2 - {p}))'
    else:
        msg = f"Input objective type '{score_type}' not supported!"
        logger.error(msg)
        raise ValueError(msg)
    score_select = ('SELECT 1 - {rss}/{tss} FROM ({data}) as data, ' +
                    '({base_val}) as avg_tbl').format(
                    rss=pred_dev,
                    tss=base_dev,
                    data=data_.select_statement,
                    base_val=base_val)
    try:
        with conn.connection.cursor() as cur:
            execute_logged(cur, score_select, conn.sql_tracer, conn)
...
def r2_score(data, label_true, label_pred):
    ...
    return regression_score(data, label_true, label_pred, score_type='r2')
...
def binary_classification_debriefing(data, label_true, label_pred, auc_data=None, positive_label=1, negative_label=0):
    ...
    return {
        ...
        'RECALL': tp / (tp + fn) if tp + fn != 0 else None, # TPR
        'SPECIFICITY': tn / (tn + fp) if tn + fp !=0 else None, # True Negative Rate
        ...
        'SELECTION RATE': (tp + fp) / (tp + tn + fp + fn) if tp + tn + fp +fn != 0 else None
    }
