...
def _printer(
    ...
    """
    ** The complete C source code of the module with the tree function numpy compatible. **
...
def _printer_kernel(
    ...
    """
    ** The source code of the C function's kernel. **
    ...
    for out, expr in (tree[:-1] if isinstance(tree[-1][1], Tuple) else tree):
        ...
        context |= new_context
        alloc |= new_alloc
        kernel_lines.extend([f"    {l}" for l in new_lines])
    if alloc:
...
def _printer_parser(
    ...
    ** Write the code of the C python parser. **
    ...
    # thread management
    parser += "  if (syscall(SYS_gettid) == getpid()) {\n"
...
def _pow(
    ...
    if exp.is_number:
        ...
        # a**(n/m) <=> (a**n)**(1/m)
        if (cfunc := { # a**(1/2) <=> sqrt(a) or a**(1/3) <=> cbrt(a)
                (2, "float"): "sqrtf",
                (2, "double"): "sqrt",
                (2, "long double"): "sqrtl",
...
def _pow_pos_integer(
    ...
    """
    ** Special case of power where the exponant is a positive integer. **
