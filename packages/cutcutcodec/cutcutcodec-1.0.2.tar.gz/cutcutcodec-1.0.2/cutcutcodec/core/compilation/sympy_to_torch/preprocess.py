...
def _expr_to_atomic(expr: Basic, *, _symbols=None) -> list[tuple[Symbol, Basic]]:
    ...
    ** Apply ``cse`` and split the sub patterns. **
    ...
    if _symbols is None:
        ...
        rep.append(((Symbol("_") if isinstance(expr, Tuple) else next(_symbols)), final))
    ...
    for var, sub_expr in rep:
        ...
        for arg in sub_expr.args:
            if (
                arg not in subs # we don't do the same calculs several times
                and (
                    not arg.is_Atom # replace if sub expr is not atomic
                    or (arg.is_Number and isinstance(sub_expr, Tuple)) # no numbers in Tuple
                )
            ):
        ...
        if isinstance(sub_expr, Tuple): # make sure no duplicate and no intermediate variables
            args = [] # this ensures the independence of the output variables
            for arg in sub_expr.args:
                if not re.fullmatch(r"_\d+", str(arg)) or arg in args:
...
def _isolate_cst_dyn(
    ...
    ** Isolate the constant subexpressions. **
    ...
    for symb, expr in symb_expr:
        if not expr.is_number and all(s in cst_args or s in csts for s in expr.free_symbols):
            csts.add(symb)
    ...
    ** Optimises memory by reusing as many old variables as possible. **
    ...
    for new_free, (old_symb, old_expr) in zip(all_new_free, symb_expr):
        ...
        symb = min(
            ...
            key=lambda s: ((s != expr), (not str(s).startswith("_")), str(s)),
            default=symb,
        )
        if symb != expr:
            new_tree.append((symb, expr))
...
def evalf(expr: Basic) -> Basic:
    ...
    ** Numerical eval and simplification of the expression. **
    ...
    if sub := expr.atoms(NumberSymbol):
        expr = expr.xreplace({s: s.evalf(n=25) for s in sub})
    return expr.evalf(n=25) # the float128 mantissa is 80 bits => n = ceil(log10(2**80)) = 25
