...
def _node_to_ast(
    ...
    * Input / Ouput pickalisable, no modification inplace for alow parralelisation.
    ...
    def get_filter_audio_delay_1(filter_audio_delay_2: Node) -> FilterAudioDelay:
        """** Creation of the node 'filter_audio_delay_1'. **"""
    ...
    def get_generator_audio_noise_1() -> GeneratorAudioNoise:
        ...
        generator_audio_noise_1.setstate([], state={'seed': 0.0, 'profile': 'stereo'})
    ...
    body.append(ast.Expr(value=ast.Constant(f"** Creation of the node {repr(node_name)}. **")))
...
def graph_to_ast(graph: networkx.MultiDiGraph) -> ast.Module:
    ...
    ** Autogenerated project exportation script. **
    ...
    tree_body.insert(0, ast.Expr(value=ast.Constant("** Retrives the complete assembly graph. **")))
    body.append(
        ast.FunctionDef(
