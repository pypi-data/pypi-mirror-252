...
def _optimal_rate_generator_audio_equation(
    ...
    def estimate_freq_max(
        ...
        if isinstance(signal, Add): # fourier transform is linear
            return max(estimate_freq_max(s, t_symb, t_min, t_max) for s in signal.args)
        if isinstance(signal, Mul): # fourier transform of prod is convolution, max is sum
            return sum(estimate_freq_max(s, t_symb, t_min, t_max) for s in signal.args)
        ...
        if isinstance(signal, Pow) and signal.exp.is_integer and signal.exp > 0: # recursive Mul
            return float(signal.exp) * estimate_freq_max(signal.base, t_symb, t_min, t_max)
        return estimate_freq_max_by_diff(signal, t_symb, float(t_min), float(t_max))

    def abs_max_expr(expr, t_symb: Symbol, t_min: float, t_max: float) -> float:
        """
    ...
    def estimate_freq_max_by_diff(signal, t_symb: Symbol, t_min: float, t_max: float) -> float:
        """
        ** Match the max freq sin by differencials. **

        Hypotesis : signal = s = a*sin(2*pi*f*t)
        ...
        return math.sqrt(abs_max_expr(max_freq, t_symb, t_min, t_max) / (4*math.pi**2))

    t_symb = Symbol("t", real=True, positive=True)
    freq = max(estimate_freq_max(signal, t_symb, t_min, t_max) for signal in stream.node.signals)
...
def optimal_rate_audio(
    ...
    choices: typing.Optional[set[int]]=None,
) -> int:
    """
    ** Finds the optimal sampling rate for a given audio stream. **
    ...
    if (estimator := RATE_ESTIMATORS.get(stream.node.__class__, None)) is not None:
        rate = estimator(stream, t_min, t_max)
    else:
        rate = max((optimal_rate_audio(s, *t) for s, *t in time_backprop(stream, t_min, t_max)
            if s.type == "audio"), default=0)

    # select the most appropriate rate among the choices
    if not choices or not rate:
        return min(choices) if choices else rate
