...
def optimal_rate_video(
    ...
    """
    ** Finds the optimal frame rate for a given video stream. **
    ...
    if (estimator := FPS_ESTIMATORS.get(stream.node.__class__, None)) is not None:
        ...
        fps = max((optimal_rate_video(s, *t) for s, *t in time_backprop(stream, t_min, t_max)
            if s.type == "video"), default=0)
