    ...
    ** Allows you to write the output file to disk. **
    ...
    def __init__(self,
            ...
            For audio streams, here is the format to follow:
                {
                    ...
                    "bit_rate": int, # (optional) the flow in bits/s (ex 1024000)
                }
            For video streams, here is the format to follow:
                {
                    ...
                    "bit_rate": int, # (optional) the flow in bits/s (ex 6400000)
                    "pix_fmt": str, # (optional, default yuv420p) pixel format (ex "yuv444p10le")
        ...
        for stream, settings in zip(in_streams, streams_settings):
            ...
            settings["bit_rate"] = settings.get("bit_rate", None)
            if settings["bit_rate"] is not None:
                settings["bit_rate"] = round(parse_to_number(settings["bit_rate"]))
                assert settings["bit_rate"] >= 0, settings["bit_rate"]
            if stream.type == "video":
                ...
                settings["pix_fmt"] = settings.get("pix_fmt", "yuv420p")
                assert isinstance(settings["pix_fmt"], str), settings["pix_fmt"].__class__.__name__
            ...
            for stream, settings in zip(self.in_streams, self.streams_settings):
                if stream.type == "audio":
                    ...
                    stream_av = container_av.add_stream(
                        settings["encodec"], rate, layout=stream.profile.name
                    )
                    stream_av.options = settings["options"] | {"strict": "experimental"}
                    if settings["bit_rate"]:
                        stream_av.bit_rate = settings["bit_rate"] # int
                    ...
                    stream_av.pix_fmt = settings["pix_fmt"] # str, default yuv420p
                    if settings["bit_rate"]:
                        stream_av.bit_rate = settings["bit_rate"] # int
                    streams_av.append(stream_av)
