        ...
        while True:
            ...
            try:
                with self.lock:
                    ...
                    except TypeError: # if self.scheduler set to None in background
                        continue
            ...
            with self.lock:
                if self.position is None:
                    self.position = [frame.time, time.time()]
                else:
                    date_theo = self.position[1] + frame.time - self.position[0]
                    if (sleep := date_theo - time.time()) > 0:
                        time.sleep(sleep) # overrun -> real time
                    elif sleep < -0.01: # if big underun (>10ms), assume delay not catchable
                        date_theo -= sleep # reset counter for next frame, (= time.time())
    ...
    ** Non-blocking thread allowing to quietly extract frames in the background. **
    ...
    def refresh(self):
        ...
        if stream := self.dynamic_scheduler.get_stream_video():
            ...
            shape = shape[0] if shape else (1080, 1920)
            if shape == "default":
                shape = optimal_shape_video(stream) or (1080, 1920)
            self.dynamic_scheduler.state["shape"] = tuple(shape)
    ...
    def set_pause(self):
        """
        ** Stops reading and updates icon. **
        """
