...
class EditFilterAudioFIR(EditBase):
    """
    ** Allows to view and modify the properties of a filter of type ``FilterAudioFIR``.
    """
    ...
    def init_fir(self, grid_layout, ref_span=0):
        """
        ** Displays and allows to modify the impulsional resp. **
        """
        ...
        self.rep_textedit.textChanged.connect(self.validate_fir)
        grid_layout.addWidget(self.rep_textedit, ref_span, 1)
        return ref_span + 1

    def init_rate(self, grid_layout, ref_span=0):
        """
        ** Displays and allows to modify the samplerate. **
        """
        ...
        self.rate_textbox.textChanged.connect(self.validate_rate)
        grid_layout.addWidget(self.rate_textbox, ref_span, 1)
        return ref_span + 1

    def validate_fir(self):
        """
        ** Checks than the numbers are correct and update the list. **
        """
    ...
    def validate_rate(self, text):
        """
        ** Checks than the framerate is correct and update it. **
        """
