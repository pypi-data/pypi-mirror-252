...
def fit_logistic_regression(X, y, columns, method):
    try:
        model = sm.Logit(y, sm.add_constant(pd.DataFrame(X[columns]))).fit(method=method)
        return model
    except Exception as e:
        logging.error(f"Error fitting logistic regression model using {method}: {e}")
        return None

def drop_worst_feature(X, y, included_features, threshold_out, dropped_variables, method):
    model = fit_logistic_regression(X, y, included_features, method)
    if model:
...
def backward_regression(X, y, initial_list=[], threshold_in=0.01, threshold_out=0.05, include_interactions=True, verbose=True, method='bfgs'):
    ...
    if include_interactions:
        ...
        for i in range(len(included_features)):
            for j in range(i + 1, len(included_features)):
                ...
                model = fit_logistic_regression(X, y, included_with_interactions + [interaction_term], method)
                if model:
