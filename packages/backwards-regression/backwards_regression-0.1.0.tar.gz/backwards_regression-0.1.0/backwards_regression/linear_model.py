...
def fit_linear_regression(X, y, columns):
    try:
...
def drop_worst_feature(X, y, included_features, threshold_out, dropped_variables):
    model = fit_linear_regression(X, y, included_features)
    if model:
    ...
    if include_interactions:
        ...
        for i in range(len(included_features)):
            for j in range(i + 1, len(included_features)):
                ...
                model = fit_linear_regression(X, y, included_with_interactions + [interaction_term])
                if model:
