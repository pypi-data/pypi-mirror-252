...
def _spark_asof_join_features(
    ...
    joined_df = _as_of_join_native(
        df_left=df,
        df_right=feature_and_keys,
        p_keys=df_lookup_keys,
        t_key=df_timestamp_lookup_key,
...
# [ES-966100] This is a new implementation of as_of_join without Tempo
def _as_of_join_native(
    df_left, df_right, p_keys, t_key, lookback_window_seconds: Optional[float] = None
):
    # All rows from the left
    # One or zero rows from the right per each row from the left
    # Row from the right is the latest available, based on t_key col:
    # df_left.key === df_right.key
    # df_left.t_key >= df_right.t_key
    # df_right.t_key = max
    left_keys = df_left.select([F.col(key) for key in p_keys] + [F.col(t_key)])

    right_keys = df_right.select(
        [F.col(key).alias(f"right_pk_{key}") for key in p_keys]
        + [F.col(t_key).alias("right_tk")]
    )

    conditions = [left_keys[key] == right_keys[f"right_pk_{key}"] for key in p_keys]
    cond = reduce(lambda x, y: x & y, conditions)
    cond = cond & (left_keys[t_key] >= right_keys["right_tk"])

    # If the lookback window is specified,
    # only pick the keys from the right side that falls within the window
    if lookback_window_seconds is not None:
        # Convert timestamps to epoch seconds for the lookback calculation
        left_epoch = unix_timestamp(left_keys[t_key]).alias("left_epoch")
        right_epoch = unix_timestamp(right_keys["right_tk"]).alias("right_epoch")
        cond = cond & ((left_epoch - right_epoch) <= lookback_window_seconds)

    # Observations dataset (left one) should be small compared to FS set (right one)
    # We make broadcast join here
    all_keys = F.broadcast(left_keys).join(right_keys, on=cond, how="left")

    # For each time key from left we need a latest time key from the right
    final_keys = (
        all_keys.withColumn(
            "final_rtk",
            F.max("right_tk").over(
                Window.partitionBy([F.col(pk) for pk in p_keys])
                .orderBy(F.col(t_key))
                .rangeBetween(Window.unboundedPreceding, 0)
            ),
        )
        .select(
            *[F.col(key) for key in p_keys],
            F.col("final_rtk"),
            F.col(t_key),
        )
        .distinct()  # to allow inner join
    )

    pk_conditions = [df_right[key] == final_keys[key] for key in p_keys]

    # Add the condition for t_key
    t_key_condition = df_right[t_key] == final_keys["final_rtk"]

    # Combine all conditions using logical AND
    cond = reduce(lambda x, y: x & y, pk_conditions) & t_key_condition

    # Apply the lookback window condition
    if lookback_window_seconds is not None:
        cond = cond & (
            (F.unix_timestamp(df_left[t_key]) - F.unix_timestamp(df_right[t_key]))
            <= lookback_window_seconds
        )

    # We need make two inner joins
    # Final keys is small table of the size of the left one and limited amount of cols
    final_right_data = df_right.join(
        F.broadcast(
            final_keys
        ),  # Because it is the table with keys only, only two cols
        on=cond,
        how="inner",
    )

    # Dropping columns from final_keys and df_right
    for key in p_keys:
        final_right_data = final_right_data.drop(final_keys[key])

    final_right_data = final_right_data.drop(final_keys["final_rtk"]).drop(
        df_right[t_key]
    )

    # Final join of tables.
    # Cannot use broadcast here because there may be additional columns
    # in the right table
    return df_left.join(final_right_data, on=p_keys + [t_key], how="left")
