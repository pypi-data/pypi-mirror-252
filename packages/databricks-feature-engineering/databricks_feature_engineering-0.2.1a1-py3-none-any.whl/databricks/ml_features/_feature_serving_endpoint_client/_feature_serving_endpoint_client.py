...
class FeatureServingEndpointClient:
    ...
    def create_feature_serving_endpoint(
        self, name: str, config: EndpointCoreConfig, client_name: str
    ) -> FeatureServingEndpoint:
        ...
        if config.servables is not None:
            ...
            return self._create_endpoint_with_servables(name, config, client_name)
        else:
            self._verify_endpoint_not_exists(name)
    ...
    def _create_model(
        ...
        training_set = self._create_training_set(
            df=self._create_dummy_training_df(features, client_name),
            label=None,
    ...
    def _create_dummy_training_df(
        self, features: List[Union[FeatureLookup, FeatureFunction]], client_name: str
    ):
