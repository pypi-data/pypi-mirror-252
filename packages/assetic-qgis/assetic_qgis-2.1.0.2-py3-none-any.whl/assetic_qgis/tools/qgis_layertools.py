...
class QGISLayerTools(LayerToolsBase):
    ...
    def get_geom_wkt(self, crs, wkt_type, geometry):
        shape_types = {
        ...
        # check layer geometry shape to determine if we need a centroid
        shape_type = shape_types[wkt_type]

        # define the CRS transformation we want for upload
        crs_dest = QgsCoordinateReferenceSystem("EPSG:4326")

        # create a transformation object
        crs_transform = QgsCoordinateTransform(crs, crs_dest, QgsProject.instance())

        # apply the transformation (in place), returns 0 if error
        if geometry.transform(crs_transform) != 0:
            self.messager.new_message("Failed to apply transformation to geometry CRS for bulk upload".format(asset_id))
            return None
        ...
        if shape_type == "Polygon":
            geom_wkt_dict["Centroid"] = geometry.centroid().asWkt()
    ...
    def layer_to_feature_list(self, lyr: QgsVectorLayer):
        """
        ...
        for feat in lyr.getSelectedFeatures():
            ...
            row["geom_wkt"] = self.get_geom_wkt(lyr.crs(), lyr.wkbType(), feat.geometry())
    ...
    def update_assets(self, lyr: QgsVectorLayer, query: str = None
        ...
        feat_list, att_index = self.layer_to_feature_list(lyr)
    ...
    def _new_asset(self, row, lyr_config: dict, fields, layer_crs
