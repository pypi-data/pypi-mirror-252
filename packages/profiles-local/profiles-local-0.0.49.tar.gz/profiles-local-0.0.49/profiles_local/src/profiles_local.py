...
class ProfilesLocal(GenericCRUD):
    ...
    def insert(self, person_id: int, profile_dict: Dict[str, any]) -> int:  # noqa
    ...
    def get_preferred_lang_code_str_by_profile_id(self, profile_id: int) -> str:
        logger.start(object={'profile_id': profile_id})
        preferred_lang_code = self.select_one_dict_by_id(
            view_table_name="profile_view", id_column_value=profile_id).get('preferred_lang_code')
    ...
    def get_test_profile_id(self) -> int:
        ...
        return super().get_test_entity_id(entity_name="profile_id",
                                          insert_function=self.insert,
                                          insert_kwargs={"person_id": person_id})
