...
def _delvewheel_patch_1_5_2():
    ...
    if sys.version_info[:2] >= (3, 8) and not is_conda_cpython or sys.version_info[:2] >= (3, 10):
        ...
        load_order_filepath = os.path.join(libs_dir, '.load-order-scikit_digital_health-0.13.2')
        if os.path.isfile(load_order_filepath):
            with open(os.path.join(libs_dir, '.load-order-scikit_digital_health-0.13.2')) as file:
