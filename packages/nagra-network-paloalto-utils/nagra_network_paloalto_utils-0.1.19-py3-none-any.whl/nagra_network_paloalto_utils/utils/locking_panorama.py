...
def check_lock(url, api_key, firewalls, wait_interval=60, max_tries=60) -> bool:
    ...
    for attempt in range(max_tries):
        if attempt > 0:
            print("Trying in {wait_message} again..")
        ...
        if (
            ...
            log.info("No change but lock is already taken.")
            continue
...
def try_lock(
    url, api_key, firewalls, all_firewalls=False, wait_interval=60, max_tries=60
) -> bool:
    """
    ...
    panorama_instance = Panorama(url, api_key=api_key)
    for _ in range(3):
        # TODO: This code is not safe. We should lock anyway then check for pending changes

        if not check_lock(
            url, api_key, firewalls, wait_interval=wait_interval, max_tries=max_tries
        ):
            return False
        # we can now try and lock.
