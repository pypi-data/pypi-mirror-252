...
class AzKeyVault:
    ...
    def retrieve_secret(self, secret_client, secret_name):
        ...
        with tracer.start_as_current_span("retrieve_secret"):
            try:
                ...
                error_msg = f"Error: {e} retrieving secret: {secret_name} using client_id: {self.client_id} with using client_secret: {len(self.client_secret)}"
                exc_info = sys.exc_info()
                LoggerSingleton.instance(
                    NAMESPACE_NAME, SERVICE_NAME, data_product_id, environment
                ).error_with_exception(error_msg, exc_info)
    ...
    def get_secret(self, secret_name, secret_scope=None):
        """
        ...
        with tracer.start_as_current_span("get_secret"):
            try:
                ...
                logger.info(f"first attempt to retrieve secret for key: {secret_name}")

                try:
                    secret_value = self.retrieve_secret(self.client, secret_name)
                except Exception as ex:
                    logger.warning(
                        f"Failed to retrieve secret for key: {secret_name}. Error: {ex}"
                    )
                    secret_value = None
                ...
                if secret_value is None:
                    if self.running_interactive is True:
                        logger.info(
                            f"second alternate attempt to retrieve secret for key: {secret_name}"
                        )
                        logger.info(
                        ...
                        try:
                            ...
                            ex_message = "Unable to fetch secret from dbutils"
