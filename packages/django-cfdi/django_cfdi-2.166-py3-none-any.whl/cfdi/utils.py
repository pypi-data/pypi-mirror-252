...
def get_xml_object(xml_text, incluir_texto_direccion=False, venta_global_es_combustible=False):
    ...
    if carta_porte.exists:
        ...
        if "IdCCP" in xml.complemento_carta_porte and xml.complemento.timbrefiscaldigital:
            url_validacion = f"https://verificacfdi.facturaelectronica.sat.gob.mx"
            url_validacion += "/verificaccp/default.aspx"
            url_validacion += f"?IdCCP={xml.complemento_carta_porte['IdCCP']}"
            url_validacion += f"&FechaOrig={xml.fecha_str}&FechaTimb={xml.complemento.timbrefiscaldigital.fechatimbrado_str}"""
