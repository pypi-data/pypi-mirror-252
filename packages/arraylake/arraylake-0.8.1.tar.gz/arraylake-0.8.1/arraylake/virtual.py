...
def reformat_kerchunk_refs(
...
) -> tuple[dict[Path, MetaDictT], dict[Path, ReferenceData], dict[Path, bytes]]:
    """
    ...
    """
    from .chunkstore.s3chunkstore import encode_inline_data
    ...
    for k, v in v3_refs.items():
        ...
        if key == "zarr.json":
            ...
            if isinstance(v, list):
                ...
                chunk_refs[key] = ReferenceData(uri=v[0], offset=v[1], length=v[2], hash=None)  # type: ignore
    ...
    return meta_docs, chunk_refs, inlined_refs
