...
def ElectronNumberDensity(snap, get_dependencies=False):
    """
    The electron number density in cm⁻³.
    Not valid in star-forming regions
    """
    if get_dependencies:
        return ['0_Density']
    ...
    electron_number_density = density * hydrogen_abundance / proton_mass
    return electron_number_density.to_physical


def ElectronNumberDensitySFR(snap, get_dependencies=False):
    """
    The electron number density in cm⁻³.
    Tries to account for star-forming regions.

    See https://ui.adsabs.harvard.edu/abs/2003MNRAS.339..289S/abstract
    for details.

    adopted from code by Larissa Tevlin
    """

    if get_dependencies:
        return ['0_StarFormationRate', '0_ElectronNumberDensity',
                '0_ElectronAbundance']
    ...
    # mean weight assuming 0 free electrons
    meanweight = 4. / (1. + 3. * x_h)

    arepo_internal_energy_unit = snap.get_paicos_quantity(1, 'InternalEnergy',
                                                          'voronoi_cells')

    # Conversion factor between Kelvin and Arepo's internal energy units
    Tfac = 1 / ((snap.gamma - 1) * arepo_internal_energy_unit
                * meanweight * mproton).to('K') * arepo_internal_energy_unit

    rho = snap['0_Density'].to('g cm^-3').to_physical

    # get electron number density assuming all hydrogen to be ionized
    ne = snap['0_ElectronNumberDensity']

    # above which thermal instability sets in -> gas goes from hot phase to cool cloud
    PhysDensThresh = 0.1 * const.m_p.to(u.g) / u.cm**3

    # SN evaporation parameter
    FactorEVP = snap.Parameters['FactorEVP']

    # get all the star-forming cells
    i = snap['0_StarFormationRate'] > 0.

    # density dependent SN evaporation parameter (equation (20) in
    # Springel&Hernquist 2003)
    facEVP = FactorEVP * (rho[i] / PhysDensThresh)**(-4 / 5)

    # T_cloud=10^3 K to internal energy of cloud
    EgySpecCold = 1e3 * u.K * Tfac

    # T_SN=5.73e7K to internal energy of SN
    EgySpecSN = 5.73e7 * u.K * Tfac

    # Equilibrium temperature, assuming cooling instab regime and deviations decay fast
    # (equ (11) Springel&Hernquist 2003)
    EgyHot = EgySpecSN / (1. + facEVP) + EgySpecCold

    # calculate fraction of internal energy that is in cold clouds
    xCloud = (snap['0_InternalEnergy'][i] - EgyHot) / (EgySpecCold - EgyHot)

    # modfiy star-forming cells in ne: subtract this fraction from total electron
    # number density, assuming 76% H and 23% He
    ne[i] = ((1. - xCloud) * (0.76 + 0.23 / 4. * 2.)
             * rho[i] / const.m_p.to(u.g))
    return ne
