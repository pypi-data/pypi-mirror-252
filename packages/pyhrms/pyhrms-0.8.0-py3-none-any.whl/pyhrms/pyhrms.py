...
def gen_DDA_ms2_df(ms1, ms2, i_threshold=0, profile=True, opt=False, more_info=False, message=''):
    ...
    for i, scan in tqdm(enumerate(ms2), desc='Collecting MS2 info', colour='Green', leave=False):
        ...
        s = pd.Series(data=intensity, index=mz, dtype='float64').sort_values(ascending=False).iloc[
            :20]  # Explicit dtype
        s = s[s > i_threshold]
        ...
        for i in tqdm(range(len(unique)), desc='Starting DIA ms2 matching:', leave=False):
            ...
            if len(match_result) == 0:  # 匹配失败
                ...
                for j in range(len(match_result)):
                    ...
                    try:
                        iso_result = isotope_matching(iso_info, formula, adduct=adduct)
                        iso_score = score = round(1 - sum(iso_result['dis_diff']), 3)
                    except:
                        iso_score = 0
...
def precursor_frag_peak_area(files_unique, path_to_mzml, company, rt_error=0.1,
                             ...
                             precursor_ion_start_mass=99.5, mz_overlap=1):
    """
...
def peak_checking_area_precursor_frag_swath(peak_ref, ms2, precursor_ion_start_mass=99.5, mz_overlap=1, message=''):
    """
    ...
    for i, (k, v) in enumerate(items):
        ...
        # . xxxxxx
        peak_area = peak_checking_area_split(ref_all, target_ms2, 'area', message=message1)
...
def gen_ref_swath(files_excel, rt_error=0.1, ms1_error=0.01, ms2_error=0.015):
    ...
    for idx, scaled_point in tqdm(enumerate(scaled_data),
                                  desc='Aligning all rt_mz pairs', leave=False, colour='Green'):
        if idx in visited:
            continue
...
def compare_ms_spectra(spec_db, spec_obs, error=0.015):
    ...
    s2_nor = s2_new / max(s2_new) if max(s2_new) != 0 else s2_new
    if max(s2_new) == 0:
        score = 0
    else:
        diff = sum([abs(s1_nor.values[i] - s2_nor.values[i]) for i in range(len(s1_nor))]) / len(s1_nor)
        score = 1 - diff
    return score
