...
class PlaywrightWebDriver(WebDriver[Locator]):
    ...
    def wait_for_page_ready_state(self):
        ...
        if not self._network_monitor:
            ...
            self._current_page.on("request", self._network_monitor.track_request)
            self._current_page.on("requestfinished", self._network_monitor.track_response)
            self._current_page.on("requestfailed", self._network_monitor.track_response)
        else:
            # Reset the network monitor to clear the logs
            self._network_monitor.reset()

        try:
            self._current_page.wait_for_load_state("networkidle", timeout=6000)
        except PlaywrightTimeoutError:
            self._determine_load_state(self._network_monitor)
