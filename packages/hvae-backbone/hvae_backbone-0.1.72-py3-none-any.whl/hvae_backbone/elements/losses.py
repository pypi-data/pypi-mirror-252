...
class LogProb(nn.Module):
    ...
    def forward(self, targets, distribution: Distribution):
        ...
        avg_loss = torch.sum(avg_per_example_loss) / (batch_size * np.log(2))
...
class BernoulliLoss(nn.Module):
    ...
    def forward(self, targets, logits):
        ...
        scalar = batch_size * np.prod(self.data_shape)
        loss = torch.sum(per_example_loss) / scalar
        avg_loss = torch.sum(per_example_loss) / batch_size
        return loss, avg_loss
...
class KLDivergence(nn.Module):
    ...
    def forward(self, prior: Distribution, posterior: Distribution):
...
class StructureSimilarityIndexMap(nn.Module):
    ...
    def forward(self, targets, outputs, global_batch_size):
        ...
        if len(targets.size()) == 5:
            targets = torch.stack([targets[:, i] for i in range(targets.size()[1])], dim=1)
            outputs = torch.stack([outputs[:, i] for i in range(outputs.size()[1])], dim=1)
