...
class hSequenceVAE(hVAE):
    ...
    def extrapolate(self, inputs: int, seq_len: int) -> (tensor, dict):
        with torch.no_grad():
        ...
        computed['output'] = torch.cat([torch.stack(outputs, dim=1), computed['output']], dim=1)
        distributions['output'] = distributions['output'].extend(output_distributions)
        return computed, distributions
