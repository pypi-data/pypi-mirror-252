...
def compute_loss(targets: tensor, distributions: dict, logits: tensor = None, step_n: int = 0) -> dict:
    ...
    feature_matching_loss, avg_feature_matching_loss = params.reconstruction_loss(targets, output_distribution)
    ...
    for block_name, dists in distributions.items():
        ...
        loss, avg_loss = params.kl_divergence(prior, posterior)
        global_variational_prior_losses.append(loss)
        avg_global_var_prior_losses.append(avg_loss)
        block_names.append(block_name)

    kl_divs = {f"kl_div_{block_name}": loss for block_name, loss
               in zip(block_names, global_variational_prior_losses)}
    ...
    global_variational_prior_loss = kl_div \
        ...
        else params.gamma_schedule(global_variational_prior_losses,
                                   avg_global_var_prior_losses,
                                   step_n=step_n)
...
def extrapolate(net, loader: DataLoader, n_samples):
    net.eval()
    original, output_samples, output_means = None, None, None
    for val_step, val_inputs in enumerate(loader):
        n_samples -= params.eval_params.batch_size
        val_inputs = val_inputs.to(params.device, non_blocking=True)
        computed, distributions = net.extrapolate(val_inputs, seq_len=n_samples)
