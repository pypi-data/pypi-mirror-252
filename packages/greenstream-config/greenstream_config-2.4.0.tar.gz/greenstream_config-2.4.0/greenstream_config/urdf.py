...
from greenstream_config.types import Camera, CameraOverride, GreenstreamConfig, Mode
from greenstream_config.greenstream_config.generate_greenstream_launch_description import merge_camera_overrides
...
def get_cameras_urdf(cameras: List[Camera], config : GreenstreamConfig) -> Tuple[List[Link], List[Joint]]:
    ...
    links = List[Link]
    joints = List[Joint]
    cameras = merge_camera_overrides(cameras, config.camera_overrides)
