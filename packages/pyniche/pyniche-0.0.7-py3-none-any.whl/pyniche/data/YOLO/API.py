...
class YOLO_API:
    ...
    def shuffle_train_val(self, n):
        ...
        ------
        n: int or float
            int: number of images to be included in the training set
            float: ratio of images to be included in the training set
        """
        if isinstance(n, float):
            n = int(n * len(self.ls_train_images_all))
        random.shuffle(self.ls_train_images_all)
        self.ls_train_images = self.ls_train_images_all[:n]
        self.ls_val_images = self.ls_train_images_all[n:]
