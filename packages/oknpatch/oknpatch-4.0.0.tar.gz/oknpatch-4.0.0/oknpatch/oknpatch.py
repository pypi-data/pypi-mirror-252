...
def rerun_recording(main_folder_dir, opmdc, es, uc, di, okndc, okndl, pil, ril, overwritten=False,
...
# di = direction_input, okndc = okn_detector_config_location, okndl = okn_detector_location,
# pil = plot info location, ril = rule info location
def fix_neon_csv_and_rerun(main_folder_dir, es, uc, okndc, okndl, pil, ril):
    if not os.path.isdir(main_folder_dir):
        print("Recording folder input to be rerun does not exist.")
        return
    ...
    if type_input:
        if str(type_input) == "trial_data_lost":
            ...
            if directory_input is None:
                ...
                print("oknpatch -t rerun_recording -d directory_to_be_rerun -okndl okn_detector_location")
                print("(or)")
                print("oknpatch -t rrr -d directory_to_be_rerun -okndl okn_detector_location")
                print("")
                return
            ...
            if okn_detector_location:
                ...
                if not okn_detector_location_exist:
                    print("")
                    print("Invalid okn detector location input.")
                    print("")
                    return
            else:
                print("")
            ...
            if directory_input is None:
                print("")
                print("The directory to be rerun is missing in the commandline.")
                print("oknpatch -t fix_neon_csv_and_rerun -d directory_to_be_rerun -okndl okn_detector_location")
                print("(or)")
                print("oknpatch -t fncar -d directory_to_be_rerun -okndl okn_detector_location")
                print("")
                return
