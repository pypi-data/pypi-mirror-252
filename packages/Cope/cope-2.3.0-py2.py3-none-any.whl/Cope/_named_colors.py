...
Parsed with:
    ...
    regex = group(+(word + optional(space)), name="name") + ow + group('#' + hexDigit*6, name='color')

    def hex2rgb(hex:str):
        return int(hex[1:3], 16), int(hex[3:5], 16), int(hex[5:7], 16)

    # outstr = ''
    # for name, color in re.findall(str(regex), colors_str):
    #     outstr += f'{name.replace(' ', '_')} = {hex2rgb(color)}\n'

    outstr = 'named_colors = {\n'
    for name, color in re.findall(str(regex), colors_str):
        outstr += f'    "{name}": {hex2rgb(color)},\n'
    outstr += '}'
