...
class BarplotSeries(BaseSeries):
    def __init__(self, x, y, label="", width=0.8, **kwargs):
        super().__init__(**kwargs)
...
def draw_box(renderer, data):
    ...
    # handle = p._ax.boxplot(data, **s.rendering_kw)
...
def plot_bars(data, labels=[""], **kwargs):
    ...
    show = kwargs.get("show", True)
    backend = kwargs.get("backend", TWO_D_B)
    width = kwargs.get("width", 1 / (len(data) + 1))
    ...
    if hasattr(data[0], "__iter__"):
        ...
        for i, d in enumerate(data):
            ...
            offset = start + width * mult
            x = (array(f.observation) + offset).tolist()
            series.append(BarplotSeries(x, f.frekvens, labels[i], width=width))
            mult += 1
        ...
        x = f.observation
        p = backend(BarplotSeries(x, f.frekvens, labels, width), **kwargs)
    if show:
        p.show()
    return p
...
def plot_hist(data, groups, **kwargs):
    ...
    series = BarplotSeries(x, y, width=w)
