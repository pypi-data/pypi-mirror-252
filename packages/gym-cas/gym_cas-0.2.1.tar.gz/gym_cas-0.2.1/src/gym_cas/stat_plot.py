...
def plot_bars(data, labels=[""], **kwargs):
    ...
    if hasattr(data[0], "__iter__"):
        ...
        # kwargs.setdefault("axis_center", (global_min - gap, 0))
        # kwargs.setdefault("axis_center", "None")
        ...
        # kwargs.setdefault("axis_center", (min(data) - 0.8*width, 0))
        p = backend(BarplotSeries(x, f.frekvens, x, labels, width), **kwargs)
    # p.ax.spines["left"].set_position(("data", 0))
    if show:
        p.show()
    return p
...
def plot_hist(data, groups, **kwargs):
    ...
    series = BarplotSeries(x, y, x, width=w)
