...
class Command(BaseCommand):
    help = """Loads NCAA Game Period data from JSON."""
    ...
    def handle(self, *args, **options):
        ...
        season_obj, _ = Season.objects.get_or_create(
            season_name=season.season, season_type=season.type, label=self.derive_label_from_file_name(file_name))

        for game_period in season.weeks:
            game_period_obj, created = GamePeriod.objects.update_or_create(
                sport_radar_period_id=game_period.id, defaults={'number': game_period.number, 'season': season_obj})
        ...
        for game in game_list:
            game_obj, created = Game.objects.update_or_create(
                game_id=game.id, defaults={
                    ...
                    'home': cls.team_from_id(game.home),
                    'away': cls.team_from_id(game.away),
                    'status': game.status,
