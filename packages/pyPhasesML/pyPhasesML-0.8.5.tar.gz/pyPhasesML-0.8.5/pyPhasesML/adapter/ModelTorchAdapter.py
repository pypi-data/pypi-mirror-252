...
class ModelTorchAdapter(ModelAdapter):
    ...
    def validate(self, validationData):
        ...
        model = self.model
        model.eval()
        s = ScorerTorch(self.config.numClasses, trace=True)
        # s = ScorerTorchEvent(self.config.numClasses, trace=True) if self.useEventScorer else ScorerTorch(self.config.numClasses, trace=True)
    ...
    def mapOutputForLoss(self, output, mask=None):
        output = output.reshape(-1, self.config.numClasses) if self.oneHotDecoded else output.flatten()
