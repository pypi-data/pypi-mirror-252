...
def extract_sleep_entry(timestamps, data, idx, page):
    ...
    end = dateutil.parser.isoparse(properties["start_date"]["date"]["start"])
    timestamps.append((start, end))
    duration = properties["duration"][0]["text"]["content"]
...
def get_basic_sleep_dataframe(
    ts: list[tuple],
    durations: list[str],
    target_sleep_start: str = "22:30",
    target_sleep_end: str = "06:30",
):
    N = len(ts)
    starts, ends = zip(*ts)
