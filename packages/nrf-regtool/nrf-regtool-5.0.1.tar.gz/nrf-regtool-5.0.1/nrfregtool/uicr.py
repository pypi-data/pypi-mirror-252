...
def _mboxes_are_valid(mbox_nodes: Dict[str, ParsedDTNode]) -> bool:
    ...
    rx mbox is local (rx mbox has the "interrupts" property).
    ...
    tx_mbox_is_remote: bool = "interrupts" not in mbox_nodes["tx"].properties
    rx_mbox_is_local: bool = "interrupts" in mbox_nodes["rx"].properties
...
def _remote_owner_id_get(mbox_nodes: Dict[str, ParsedDTNode]) -> OwnerID:
    ...
    """
    if "domain" not in mbox_nodes["tx"].properties:
        raise RuntimeError(
            f"Missing domain property in node {mbox_nodes['tx'].path}, {mbox_nodes['tx'].labels}"
        )

    domain_id: DomainID = DomainID(mbox_nodes["tx"].properties.get_val("domain"))
