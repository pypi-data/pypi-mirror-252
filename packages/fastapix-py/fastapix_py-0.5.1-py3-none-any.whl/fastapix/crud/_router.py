...
from fastapix.crud._selecter import (
    ...
    sqlmodel_to_selector, sql_operator_docs
)
...
class CrudRouterManager:
    ...
    def get_schema_router(
        ...
        :return:
        """
        router = APIRouter(prefix=f"/schema/{cls.crud.name.lower()}", tags=['schema'])
    ...
    def create_object_router(
        ...
        router = APIRouter(prefix=f"/{cls.crud.name.lower()}", tags=[cls.crud.name])
        docs = []
