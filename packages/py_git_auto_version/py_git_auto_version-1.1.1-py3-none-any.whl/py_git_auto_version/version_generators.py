...
class LocalBuildVersionGenerator(LocalVersionGenerator):
    ...
    def generate_version_string(
        ...
        if len(current_branch_name.strip()) > 0:
            git_ref = GitRef(
            ...
            current_tag_name = Git.Tag.points_at(current_hash)
            if len(current_tag_name.strip()) > 0:
                ...
                raise RuntimeError("Could not determine which git ref to use. Detached head and no "
