...
from .functions import qtpoint2point, get_coo_list, split_line, split_line_2
...
class Network:
    ...
    def create_network(self, tess_lane_types, netiface=None):
        ...
        try:
            Proj(proj_string.strip())
            attrs["proj_string"] = proj_string
        except:
            print("The proj_string is not valid!")
        # 记录属性，如：路网来源
    ...
    def create_links(self, netiface, roads_info: Dict, connector_mapping: Dict,
...
# 处理过短连接段
def process(netiface):
    # 连接段的最小长度
    MIN_CONNECTOR_LENGTH = 1
    ...
    connector_data = []
    # 需要裁剪的路段
    link_need_split = {"start":set(), "end":set()}

    # =========================================================================
    # 1.遍历连接段，找到过短的连接段，记录连接关系
    ...
    for connector in netiface.connectors():
        ...
        if need_process:
            ...
            # 记录需要处理的路段信息
            link_need_split["start"].add(to_link_id)
            link_need_split["end"].add(from_link_id)
    link_need_split_all = link_need_split["start"] | link_need_split["end"]
    ...
    for connector in netiface.connectors():
        ...
        to_link_id = connector.toLink().id()
        # 涉及到需要重新创建的路段，记录连接段
        if link_need_split_all & {from_link_id, to_link_id}:
            from_lane = [lane_connector.fromLane().number() + 1 for lane_connector in connector.laneConnectors()]
            to_lane = [lane_connector.toLane().number() + 1 for lane_connector in connector.laneConnectors()]
    ...
    for link in netiface.links():
        ...
        link_points_left = np.array(qtpoint2point(link.leftBreakPoint3Ds()))
        link_points_right = np.array(qtpoint2point(link.rightBreakPoint3Ds()))
        link_points_center = ((link_points_left + link_points_right) / 2).tolist()
        ...
        if link_id in link_need_split["end"]:
            link_points_center, end_info = split_line(link_points_center, SPLIT_LENGTH, "end")
        if link_id in link_need_split["start"]:
            link_points_center, start_info = split_line(link_points_center, SPLIT_LENGTH, "start")

        for lane in link.lanes():
            # 车道断点
            lane_center = qtpoint2point(lane.centerBreakPoint3Ds())
            lane_left = qtpoint2point(lane.leftBreakPoint3Ds())
            lane_right = qtpoint2point(lane.rightBreakPoint3Ds())

            if end_info:
                lane_center = split_line_2(lane_center, end_info, "end")
                lane_left = split_line_2(lane_left, end_info, "end")
                lane_right = split_line_2(lane_right, end_info, "end")
            if start_info:
                lane_center = split_line_2(lane_center, start_info, "start")
                lane_left = split_line_2(lane_left, start_info, "start")
                lane_right = split_line_2(lane_right, start_info, "start")
        ...
        link_data[link_id] = {
            'link_points_center': link_points_center,
    ...
    # 创建路段
    link_mapping = {}
    for link_id, link_data in link_data.items():
        link_points_center = get_coo_list(link_data["link_points_center"])

        lane_points = link_data["lane_points"]
        for i, lane_point in enumerate(lane_points):
            lane_points[i]["center"] = get_coo_list(lane_points[i]["center"])
            lane_points[i]["left"] = get_coo_list(lane_points[i]["left"])
            lane_points[i]["right"] = get_coo_list(lane_points[i]["right"])

        lane_types = link_data["lane_types"]

        link = netiface.createLink3DWithLanePointsAndAttrs(
            link_points_center,
            lane_points,
            lane_types,
        )
        if link is None:
            print("创建失败！")
        link_mapping[int(link_id)] = link.id()

    # 创建连接段
    for connector_data in connector_data:
        from_link_id = link_mapping.get(connector_data["from_link"]) or connector_data["from_link"]
        to_link_id = link_mapping.get(connector_data["to_link"]) or connector_data["to_link"]
        netiface.createConnector(
            from_link_id,
            to_link_id,
            connector_data["from_lane"],
            connector_data["to_lane"]
        )

    # print(link_need_split)
    # print(link_mapping)