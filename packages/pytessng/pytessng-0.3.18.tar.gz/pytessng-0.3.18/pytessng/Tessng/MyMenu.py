...
class Tools():
    # ico图标
    icon_path = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), "Files", "Ico", "TESSNG.ico"))

    # 获取电脑屏幕的尺寸
    screen_width, screen_height = scene_size()

    # 默认打开和保存的位置
    # default_path = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop') # 电脑桌面
    # default_path = os.getcwd() # 主进程路径
    default_path = os.path.join(os.getcwd(), "WorkSpace", "Examples") # 样例文件夹
    ...
    def network_import(widget, params):
        ...
        if path:
            Tools.default_path = os.path.dirname(path)
    ...
    def network_export(widget):
        ...
        if not file_path:
            return
        # 更改默认路径
        Tools.default_path = os.path.dirname(file_path)
    ...
    def send_message(path, message):
        # 用于唯一标识
        UUID_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "Files", "UUID")
...
class MyMenu(QMenu):
    def __init__(self, *args, extension=False):
        super().__init__(*args)
        # 文件路径
        self.instruction_path = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), "Files", "Doc", "用户使用手册.pdf"))
        self.examples_path = "file:\\" + os.path.join(os.getcwd(), "WorkSpace", "Examples")
    ...
    def open_instruction(self):
        open_file(self.instruction_path, new=2)

    # 5.2.打开样例
    def open_examples(self):
        open_folder(['explorer', self.examples_path], shell=True)
...
class TrajectoryExport(QWidget):
    memory_params = {
        ...
        "coord_lon": None,
        "coord_lat": None,
        "json_path": os.path.join(os.getcwd(), "WorkSpace", "Data"),
        "kafka_ip": None,
        "kafka_port": None,
        "kafka_topic": None
    }
    ...
    def check_kafka(self):
        ...
        kafka_pull_is_ok, message = self.check_kafka_pull(ip, port)
    ...
    def check_kafka_pull(self, ip, port):
        ...
        try:
            ...
            if message:
                ...
                message = "Kafka连通测试测试失败：未接收到消息"
        except Exception as e:
            message = f"Kafka连通测试测试失败：{e}"
        try:
            consumer.close()
        except:
            pass

        return kafka_pull_is_ok, message
