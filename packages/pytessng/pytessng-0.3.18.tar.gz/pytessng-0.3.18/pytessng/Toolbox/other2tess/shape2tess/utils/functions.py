    ...
    for idx, lane in enumerate(all_lanes):
        ...
        for other_idx, other_lane in enumerate(all_lanes):
            ...
            if idx != other_idx:
                ...
                # 判断起点和其他车道的起点是否满足条件
                if calculate_distance(start_point, other_start) <= threshold_distance_max:
                    found_connection_start = True
                
                # 判断终点和其他车道的终点是否满足条件
                if calculate_distance(end_point, other_end) <= threshold_distance_max:
        ...
        for other_idx, other_lane in enumerate(all_lanes):
            ...
            if idx != other_idx:
                ...
                if distance < threshold_distance_min:
                    print(f"-距离过近！  distance: {distance:.1f}m")
                    return False
...
def check_lane_length(all_lanes, threshold_length):
    ...
    if max_lane_length - min_lane_length <= threshold_length:
        ...
        print(f"-max: {max_lane_length:.1f}m   min: {min_lane_length:.1f}m")
        return False
