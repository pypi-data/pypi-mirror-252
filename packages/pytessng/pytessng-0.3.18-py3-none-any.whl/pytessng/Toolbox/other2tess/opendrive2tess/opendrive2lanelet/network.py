...
from .conversion_lanelet_network import ConversionLaneletNetwork
...
class Network:
    ...
    def load_opendrive(self, opendrive: OpenDrive):
        ...
        for road in pgd.progress(opendrive.roads, '数据解析中（2/13）'):
    ...
    def export_lanelet_network(
        ...
        progress = 0
        for parametric_lane in pgd.progress(self._planes, '数据解析中（3/13）'):
            ...
            # 传入的是整条参考线点序列，需要减去初始值得到section对应的lane的点序列,length只是长度序列，对左右来向是一样的，不会影响计算
    ...
    def _add_junctions(self, opendrive):
        ...
        for junction in opendrive.junctions:
            for connection in junction.connections:
                ...
                if not (incoming_road and connecting_road):
                    continue
