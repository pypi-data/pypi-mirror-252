...
def convert_roads_info(opendrive: OpenDrive, step_length: int,
    ...
    """
    roads_info = {}
    for road in pgd.progress(opendrive.roads, '数据解析中（1/13）'):
...
def convert_lanes_info(opendrive: OpenDrive, scenario: Scenario, roads_info: Dict) -> Dict:
    ...
    for road in pgd.progress(opendrive.roads, '数据解析中（4/13）'):
    ...
    # 中心车道未转换，用参考线代替
    for lane in pgd.progress(scenario.lanelet_network.lanelets, '数据解析中（5/13）'):
        # center_lane
        # 获取所在路段
