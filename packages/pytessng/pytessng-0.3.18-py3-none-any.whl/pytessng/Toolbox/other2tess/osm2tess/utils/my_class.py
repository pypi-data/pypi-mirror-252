...
from .config import osm_data_before_process_save_path, osm_data_after_process_save_path
try:
...
class OSMData:
    ...
    def __init__(self, params:dict):
        ...
        # 设置缓存路径和取消API限速
        ox.settings.cache_folder = osm_data_before_process_save_path
        ox.settings.overpass_rate_limit = False
        # 创建保存文件夹
        os.makedirs(osm_data_before_process_save_path, exist_ok=True)
        os.makedirs(osm_data_after_process_save_path, exist_ok=True)
    ...
    def _get_osm_graph_object(self)->None:
        if self.params.get("osm_file_path"):
            ...
            print("No initialization information!\n")
            return

        print("Graph object obtaining is finished!\n")
    ...
    def _get_osm_graph_object_by_osm_file_path(self)->None:
        try:
            ...
            print(e)
            print("The OSM file cannot be parsed!\n")
    ...
    def _get_osm_graph_object_by_bounding_box(self)->None:
        ...
        try:
            ...
            print("Due to network connectivity issues, OpenStreetMap data acquisition failed!\n")
    ...
    # 从外部获取数据
    def get_osm_data(self, save_file_name:str=None)->dict:
        osm_data = {
        ...
        if save_file_name:
            if self.G:
                save_path = os.path.join(osm_data_after_process_save_path, f'{save_file_name}.json')
                with open(save_path, 'w', encoding="utf-8") as json_file:
                    json.dump(osm_data, json_file, indent=4, ensure_ascii=False)
                print("Data saving is finished!\n")
            else:
                print("The graph object has not been initialized!\n")
        return osm_data
...
class Link:
    ...
    def cut(self, distance:int, save:str)->None:
        ...
        if distance <= 1:
            print(f"Warning: Cutting distance {distance} is too short for the line{self.id}'s length {line.length}!")
            distance = 1
        # 防止溢出
        elif distance >= line.length-1:
            print(f"Warning: Cutting distance {distance} is too long for the line{self.id}'s length {line.length}!")
...
class Turn:
    ...
    def _init_turn(self):
        ...
        if left_count>1 or straight_count>1 or right_count>1:
            print(f"Warning: Too much turn: [left]-[{left_count}], [straight]-[{straight_count}], [right]-[{right_count}].")
            self.error = True
    ...
    def _connect_links(self):
        ...
        for node_id, node in network_nodes.items():
            ...
            if link1.is_oneway != link2.is_oneway:
                print(f"Warning: The is_oneway of link{link_id_1} is different from that of link{link_id_2}.")
                continue
            ...
            if link1.lane_count != link2.lane_count:
                print(f"Warning: The lane count of link[{link_id_1}] ({link1.lane_count}) is different from that of link[{link_id_2}] ({link2.lane_count}).")
...
class NetworkCreator:
    ...
    def _create_TESSNG_network(self):
        ...
        # print("Network attrs: ", self._netiface.netAttrs().otherAttrs())
    ...
    def _create_link(self):
        ...
        if network_links:
            self.error_message = f"一共需要创建{len(network_links)}条路段，其中{error_count}条路段创建失败。"
        else:
            self.error_message = "这个区域没有路段数据。"
