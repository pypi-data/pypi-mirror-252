...
from mcli.api.finetuning_runs.api_stop_finetuning_runs import stop_finetuning_runs
...
@cli_error_handler('mcli stop finetuning-run')
def stop_finetuning_run(
    ...
    **kwargs,
    ...
    finetuning_runs = get_finetuning_runs_with_filters(
    ...
    if not finetuning_runs:
        extra = '' if stop_all else ' matching the specified criteria'
        logger.error(f'{WARN} No finetuning runs found{extra}.')
        return 1

    # skip runs that have already been stopped or don't need to be stopped
    filtered_runs = [r for r in finetuning_runs if not r.status.is_terminal()]

    skipped_runs = len(finetuning_runs) - len(filtered_runs)
    if not filtered_runs:
        if skipped_runs == 1:
            logger.error(f'{WARN} The finetuning run has already been stopped or has already finished')
        else:
            logger.error(f'{WARN} These finetuning runs have already been stopped or have already finished')
        return 1
    elif skipped_runs == 1:
        logger.error(f'{WARN} 1 finetuning run has already been stopped or has already finished. '
                     'Skipping stop request for this finetuning run')
    elif skipped_runs:
        logger.error(f'{WARN} {skipped_runs} finetuning runs already been stopped or have already finished. '
                     'Skipping stop request for these finetuning runs')

    if not force and not confirm_run_update(filtered_runs, 'stop'):
        logger.error(f'{FAIL} Canceling stop finetuning runs')
        return 1

    with console_status('Stopping finetuning runs...'):
        stop_finetuning_runs(filtered_runs, reason=reason)

    logger.info(f'{OK} Stopped finetuning runs')
    return 0
...
def add_stop_parser(subparser: argparse._SubParsersAction, is_admin: bool = False):
    ...
    # Stop finetuning runs
    stop_finetuning_run_parser = subparsers.add_parser(
        'finetuning-run',
        aliases=['ft', 'finetuning-runs'],
        help='Stop finetuning runs',
    )
    stop_finetuning_run_parser.set_defaults(func=stop_finetuning_run)

    if is_admin:
        stop_finetuning_run_parser.add_argument('--reason',
                                                dest='reason',
                                                default=None,
                                                help='Reason for stopping the run')

    configure_finetuning_submission_filter_argparser('stop', stop_finetuning_run_parser)

    stop_finetuning_run_parser.add_argument('-y',
                                            '--force',
                                            dest='force',
                                            action='store_true',
                                            help='Skip confirmation dialog before stopping runs')
