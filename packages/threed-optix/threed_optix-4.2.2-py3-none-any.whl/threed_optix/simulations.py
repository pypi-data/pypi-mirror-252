...
class Setup(object):
    ...
    def _get_parts(self):
        ...
        def create_part(id):
            try:
                part = tdo_parts.create_part(_setup=self, id=id)
                self._parts.append(part)
            except Exception as e:
                self._part_creation_errors.append(e)
            return None

        print_thread = threading.Thread(target=au._print_getting_parts)
        print_thread.start()
        ...
        for part in parts:
            thread = threading.Thread(target=create_part, args=(part['id'],))
    ...
    def run(self, analyses = None):
        ...
        if isinstance(analyses, tdo_analyses.Analysis):
            results = self._api._run_analysis(analyses)
            return copy.deepcopy(results)

        if isinstance(analyses, list):

            if not all([isinstance(analysis, tdo_analyses.Analysis) for analysis in analyses]):
                raise TypeError("Analyses must be a list of `tdo.Analysis` objects.")

            with ThreadPoolExecutor() as executor:
                futures = {executor.submit(self._api._run_analysis, analysis): analysis for analysis in analyses}
            results = []
            for future in as_completed(futures):
                analysis = futures[future]
                result = copy.deepcopy(analysis.results)
