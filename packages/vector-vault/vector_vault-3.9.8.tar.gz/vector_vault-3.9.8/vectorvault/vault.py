...
class Vault:
    ...
    def make_3d_map(self, highlight_id: int = None, return_html: bool = False):
        ...
        hover_texts = ['ID: {}'.format(i) for i in range(len(vectors))]

        # Trace for all points
        trace_all = go.Scatter3d(
            ...
            text=hover_texts,  # set the hover text
            hoverinfo='text',  # correctly combine the hover info
            name='All points'
        )
        ...
        if highlight_id is not None:
            trace_highlight = go.Scatter3d(
                ...
                text=['Highlighted ID: {}'.format(highlight_id)],  # hover text for the highlighted point
                hoverinfo='text',
                name='Highlighted point'
            )
            data = [trace_all, trace_highlight]
        else:
            data = [trace_all]

        # Layout for the 3D plot
        layout = go.Layout(
            title='3D Visualization of Annoy Index Data',
            scene=dict(
