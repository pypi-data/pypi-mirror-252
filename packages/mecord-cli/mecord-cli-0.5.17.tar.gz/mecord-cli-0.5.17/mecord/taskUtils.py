...
def idlingNotify(isProduct, cnt):
    env = "test"
    if isProduct:
        env = "[us,sg]"
...
def onlineNotify(isProduct):
    ...
    notifyWechatRobot(env, {
        "msgtype": "text",
        "text": {
            "content": f"机器<{machine_name}[{device_id}]>[{ver}] 上线{env}"
        }
    })

def offlineNotify(isProduct):
    env = "test"
    if isProduct:
        env = "[us,sg]"
    device_id = utils.generate_unique_id()
    ...
    notifyWechatRobot(env, {
        "msgtype": "text",
        "text": {
            "content": f"机器<{machine_name}[{device_id}]>[{ver}] 下线"
        }
    })
