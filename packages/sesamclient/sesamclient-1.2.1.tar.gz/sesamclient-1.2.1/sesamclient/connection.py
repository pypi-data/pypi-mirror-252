...
class Connection:
    ...
    def _get_requests_kwargs(self, extra_kwargs):
        kwargs = {}
        if self.timeout:
            kwargs["timeout"] = self.timeout
        kwargs.update(extra_kwargs)

        # The requests library will automatically set Content-Type to application/json if the json parameter
        # is set to a non-None value. But None is also a valid value, so we need to handle that use-case ourselves.
        if "json" in extra_kwargs and extra_kwargs["json"] is None:
            ...
            if "data" in kwargs:
                raise AssertionError("Both the 'json' and 'data' arguments were specified, but they are mutuallly "
                                     "exclusive!")
            del kwargs["json"]
            kwargs["data"] = "null"
        return kwargs

    def do_put_request(self, url, allowable_response_status_codes=(200,), retries=None, retry_delay=None,
                       **kwargs):
        assert url is not None
        session_put_kwargs = self._get_requests_kwargs(kwargs)
        ...
        while True:
            try:
                try:
                    response = self.session.put(url, **session_put_kwargs)
                    ...
                    self._oauth2_token = None
                    data = session_put_kwargs.get("data")
                    files = session_put_kwargs.get("files")
    ...
    def do_get_request(self, url, allowable_response_status_codes=(200, 404), retries=None, retry_delay=None, **kwargs):
        assert url is not None
        session_get_kwargs = self._get_requests_kwargs(kwargs)
    ...
    def do_post_request(self, url, allowable_response_status_codes=(200,), retries=None, retry_delay=None,
                        **kwargs):
        assert url is not None
        session_post_kwargs = self._get_requests_kwargs(kwargs)
        ...
        while True:
            try:
                try:
                    ...
                    # refreshed, but if the local clock is off that can mess things up.
                    self._oauth2_token = None
                    data = session_post_kwargs.get("data")
                    files = session_post_kwargs.get("files")
                    self._rewind_request_data_if_needed(data, files)
                    response = self.session.post(url, **session_post_kwargs)
    ...
    def do_delete_request(self, url, allowable_response_status_codes=(200,), retries=None, retry_delay=None, **kwargs):
        assert url is not None
        session_delete_kwargs = self._get_requests_kwargs(kwargs)
    ...
    def get_system_logs_url(self, system_id):
        return self.get_system_url(system_id) + "/logs"
    ...
    def get_status(self, **kwargs):
        ...
        """
        status_response = self.do_get_request(self.status_url, allowable_response_status_codes=[200], **kwargs)
    ...
    def get_ping(self, **kwargs):
        ...
        """
        status_response = self.do_get_request(self.ping_url, allowable_response_status_codes=[200], **kwargs)
        return utils.parse_json_response(status_response)
    ...
    def get_datasets_streaming(self, include_internal_datasets=True, verbose=False, **kwargs):
        # Note: the default for getting datasets is to get all
        # including the internal ones
        # This is opposite behaviour to pipes/systems
        datasets = []
        ...
        for dataset_json in entity_json.parse_json_stream(response.raw, do_transit_decoding=False):
            yield Dataset(self, dataset_json)
    ...
    def get_metadata(self, **kwargs):
        ...
        response = self.do_get_request(self.node_metadata_url, allowable_response_status_codes=[200], **kwargs)
        return utils.parse_json_response(response)
    ...
    def delete_all_sink_datasets(self, **kwargs):
        response = self.do_post_request(self.pipes_url + "/delete-all-sink-datasets",
    ...
    def get_license(self, **kwargs):
        ...
        response = self.do_get_request(self.node_license_url, allowable_response_status_codes=[200], **kwargs)
    ...
    def get_config_all(self, **kwargs):
        """Returns the configuration of the sesam node. Includes configs from all config-groups.
        """
        url = self.sesamapi_base_url + 'config-all'
    ...
    def upload_config(self, config,
                      ...
                      upload_all_config_groups=False,
                      **kwargs):
        """
        ...
        :param upload_all_config_groups: If set to True, the config is allowed to contain multiple config-groups,
                                         regardless of the 'config_group' parameter.
        ...
        request_params = {"force": "true" if force else "false"}
        if upload_all_config_groups is True:
            url = self.sesamapi_base_url + 'config-all'
        else:
            url = self.get_config_url(config_group)
        try:
            if _use_multipart_form_data:
                ...
                response = self.do_put_request(url,
                    ...
                    if not isinstance(data, bytes):
                        raise TypeError(
                            "The content-type is 'application/zip', but the data isn't of type 'bytes': type(data):%s"
                                % (type(data),))
                response = self.do_put_request(url, data=data,
                                               params=request_params,
                                               # we will deal with any 400-errors later
                                               allowable_response_status_codes=[200, 400],
    ...
    def wait_for_restore_to_finish(self, restore_id, timeout=1800, poll_interval=0.5, **kwargs):
