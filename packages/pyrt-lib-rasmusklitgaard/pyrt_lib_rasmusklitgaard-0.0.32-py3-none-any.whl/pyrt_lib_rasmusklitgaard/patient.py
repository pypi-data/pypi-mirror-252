...
class Patient():
	...
	def dvh_above_let_value(self, let_value, organ, pydicom_rtdose="this", relative_volume=True, dose_is_let = False, delta_dose = 0.1):
		import time
		t00 = time.time()
		...
		organ_inds = self.structure_indices[self.get_actual_structure_names(self.wanted_structures)[organ]]
		t1 = time.time()
	...
	def calculate_dvh_parameter(self, structure : str, dvh_parameter : str) -> float:
		# Parsing string.
		...
		match dvh_parameter[0]:
			...
			case "D":
				...
				dvh = self.dvh_above_let_value(0,structure)
				doses = dvh[:,0]
				vols = dvh[:,1]
	...
	def set_unkelbach_rbe_weighed_dose(self):
		...
		rbe = var_rbe_calculator(dose_grid, letd_grid, "unkelbach")
