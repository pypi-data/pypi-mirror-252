...
def fut_market_data(start_date, end_date, path, table, n=20):
...
def fut_market(start_date, end_date, path, corr_n=20, table='fut_index_wind'):
    ...
    data_tr1 = tr.rolling(5)
    data_tr2 = tr.rolling(20)
    data_tr3 = tr.rolling(60)
    ...
    for t in range(start_index, len(calendar)):
        ...
        sigma_series1.append(data_chg1.loc[calendar[t], vol_index.index].std(ddof=1))
        sigma_series2.append(data_chg2.loc[calendar[t], vol_index.index].std(ddof=1))
        sigma_series3.append(data_chg3.loc[calendar[t], vol_index.index].std(ddof=1))
        trend1 = data_chg1.loc[calendar[t], vol_index.index] / data_tr1.loc[calendar[t], vol_index.index]
        trend_series1.append((sum(trend1 > 0.7) + sum(trend1 < -0.7)) / trend1.count())
        trend2 = data_chg2.loc[calendar[t], vol_index.index] / data_tr2.loc[calendar[t], vol_index.index]
        trend_series2.append((sum(trend2 > 0.7) + sum(trend2 < -0.7)) / trend2.count())
        trend3 = data_chg3.loc[calendar[t], vol_index.index] / data_tr3.loc[calendar[t], vol_index.index]
        trend_series3.append((sum(trend3 > 0.7) + sum(trend3 < -0.7)) / trend3.count())
        # mean_series.append(data_chg.loc[calendar[t], vol_index.index].mean())
    data = pd.DataFrame(
        {
            't_date': corr_date,
            ...
            '截面波动率（短期）': sigma_series1,
            '截面波动率（中期）': sigma_series2,
            '截面波动率（长期）': sigma_series3,
            '短期趋势品种占比': trend_series1,
            '中期趋势品种占比': trend_series2,
            '长期趋势品种占比': trend_series3,
...
def fut_market_trend(start_date, end_date, path, table='fut_index_wind', n_list=None):
    ...
    data_chgabs = []
    for n in n_list:
        data_chg_list.append(data_chg[data_vol > 20000].rolling(n).sum())
        data_chgabs.append(
            # data_chg_list[-1].abs().rolling(n).sum()
            ((tr_max - tr_min) / data_close.shift(1))[data_vol > 20000].abs().rolling(n).sum()
        )
