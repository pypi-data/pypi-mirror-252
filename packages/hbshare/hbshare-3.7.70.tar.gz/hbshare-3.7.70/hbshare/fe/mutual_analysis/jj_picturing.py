...
def mutual_pool_picture(pool_name,asofdate1,asofdate2,latest_date,fool_version=False,bmk_id='930950'):
    ...
    if (pool_name == '公募核心池'):
        ws = wb.sheets['池对齐情况_基于持仓']
        for pic in ws.pictures:
            pic.delete()

        ws["B2"].options(pd.DataFrame, header=1, index=False, expand='table').value = core_pool_ind_dis
        ws["A37"].options(pd.DataFrame, header=1, index=False, expand='table').value = core_pool_theme_dis
        ws["A57"].options(pd.DataFrame, header=1, index=False, expand='table').value = core_pool_style_dis
        ...
        ws.pictures.add(newpath + r"\穿透后行业对齐.png", left=ws.range('J3').left,
                        top=ws.range('J3').top,
                        width=600, height=300)
        data, layout = plot.plotly_jjpic_bar (core_pool_theme_dis[['theme',  '基准暴露', '池暴露']].set_index('theme'),'穿透后主题对齐情况',legend_x=1)
        plot.save_pic2local(data, layout, newpath + r"\穿透后行业对齐")
        ws.pictures.add(newpath + r"\穿透后行业对齐.png", left=ws.range('J36').left,
                        top=ws.range('J36').top,
                        width=600, height=300)
        data, layout = plot.plotly_jjpic_bar (core_pool_style_dis[['风格类型',  '基准暴露', '池暴露']].set_index('风格类型'),'穿透后风格对齐情况',legend_x=1)
        plot.save_pic2local(data, layout, newpath + r"\穿透后行业对齐")
        ws.pictures.add(newpath + r"\穿透后行业对齐.png", left=ws.range('J57').left,
                        top=ws.range('J57').top,
                        width=600, height=300)
    ...
    barra_ret_sector.index.name='个基暴露'
    ...
    barra_ret_style.index.name='个基暴露'
...
if __name__ == '__main__':
    ...
    # mutual_pool_picture('公募核心池','20230630', '20230831', '20231228',fool_version=True)
    # mutual_pool_picture('公募30池偏股基金', '20230630', '20230831', '20231228',fool_version=True)
    # mutual_pool_picture('公募机构池', '20230630', '20230831', '20231228',fool_version=True)
