...
class Plot:
    ...
    def ploty_heatmap_new(self,df,title,font=10,if_reversed=True):

        fig_width, fig_height = self.fig_width, self.fig_height

        fig=ff.create_annotated_heatmap(z=df.values,x=df.columns.tolist(),y=df.index.tolist())
        if(if_reversed):
            fig['layout']['yaxis']['autorange'] = "reversed"

        fig.layout.width=fig_width
        fig.layout.title=title
        fig.layout.font=dict(size=font)
