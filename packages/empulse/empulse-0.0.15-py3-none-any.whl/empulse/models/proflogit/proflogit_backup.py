...
from .common import FLOAT_TYPES, NUM_TYPE
from .common import default_formula
from .rga import RGA
...
class ProfLogitCCP:
    ...
    score_kws : dict (default: None)
        Parameters passed to ``score_function``.
    ...
    def __init__(
        ...
        # Check score function parameters
        if score_kws is None:
            score_kws = {}
        assert isinstance(score_kws, dict), (
            "``score_kws`` must be a dict, "
            "where keys are parameters."
        )
    ...
    def fit(self, X, y):
        ...
        # Init
        score_function = partial(self.score_function, y_true=y, **self.score_kws)
        func_args = (X, self.reg_kws, score_function, self.intercept)
    ...
    def score(self, X, y_true):
        ...
        score = self.score_function(y_true, y_score, **self.score_kws)
        return score
...
class ProfLogitClassifier(ProfLogitCCP):
    """
    ProfLogit for churn
    ===================
    ...
    *args, **kwargs are passed to class `proflogit.base.ProfLogitCCP`.
    ...
    def score(self, X, y_true):
        ...
        score = self.score_function(y_true, y_pred, **self.score_kws)
        return score
