...
class PTSampler(object):
    ...
    def get_Z(self, discard=1, coef=3, largo=3):
        sampler_dict={'flat':True, 'discard':discard}
        logl_r = np.array(sampler.get_func('get_blobs', kwargs=sampler_dict))
        itera = len(logl_r[0])//largo

        chunks = extract_chunks(logl_r)

        zz_ = []
        zze_ = []
        for subset in chunks:
            logls = get_mean_logls(subset, coef=coef)
            
            Nsamp = len(subset[0])
            z, zerr = hand_calc_z(logls)
            zz_.append(z)
            zze_.append(zerr)
