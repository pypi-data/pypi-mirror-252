        ...
        for browser in browsers:
            ...
            with sync_playwright() as playwright:
                ...
                for test in tests:
                    ...
                    # grouping of results should improve
                    ...
                    for event, setup in validation_setup.items():
                        ...
                        scenario_results[event] = scenario_results.get(event, []) + [result]
    ...
    def run_tests(self,
        ...
        for test in tests:
            ...
            test_instance.run(headless=headless,
                              ...
                              origin=origin)
            results[test.name] = test_instance.results
    ...
    def test_report_data(self,
        ...
        for test_name, test_r in self.test_results.items():
            ...
            for scenario_name, scenario_results in test_r.items():
                scenario_data = {}
                for event_name, event_results in scenario_results.items():
                    scenario_data[event_name] = [r.test_report_data() for r in event_results]
                test_data = [scenario_name] = scenario_data
            tests_results[test_name] = test_data

        return {
            'time': time.strftime('%Y-%m-%d %H:%M:%S'),
            'starttime': self.tests_starttime.astimezone(timezone),
            'endtime': self.tests_endtime.astimezone(timezone),
            'n_errors': n_errors,
