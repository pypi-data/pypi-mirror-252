...
class CHORUSReport:
    ...
    def harmonizeRepositoryNames(self):
        '''
           Some repositories have changed names. This function standardizes the repository names throughout the dataset report. 
        '''
        repository_synonyms = [         # List of synonyms: the key is the correct name with a list of synonyms to replace with the name
            {'Dryad':    ['Dryad Digital Repository']},
            {'Figshare': ['figshare']},
            {'ICPSR - Interuniversity Consortium for Political and Social Research': ['Inter-University Consortium for Political and Social Research']},
            {'PANGAEA': ['PANGAEA_-_Data_Publisher_for_Earth_&_Environmental_Science']}
        ]
        data_df = self.data_d['data']
        if 'Dataset Repository Name' in self.data_d['data'].columns:            # check to make sure there is a column named 'Dataset Repository Name'
            for s_d in repository_synonyms:                                     # loop the synonyms
                for bestName, synonym_l in s_d.items():
                    print(f"harmonizing repository names for {bestName}. Currently {len(data_df[data_df['Dataset Repository Name'] == bestName])} rows for {bestName}")
                    for s in synonym_l:
                        print(f"replacing {len(data_df[data_df['Dataset Repository Name'] == s])} rows with {s} with {bestName}")
                        data_df['Dataset Repository Name'] = data_df['Dataset Repository Name'].str.replace('^' + s + '$', bestName, regex=True)
                        print(f"Now {len(data_df[data_df['Dataset Repository Name'] == bestName])} rows with {bestName}, {len(data_df[data_df['Dataset Repository Name'] == s])} with {s} ")
...
class CHORUSRetrieval:
    ...
    def makeStandardNames(self):                                # call makeStandardNames for each report
        for dt in self.dataTypes():
            if 'standardNames' in dataTypes_d[dt]:
                self.report_d[dt].makeStandardNames()
    ...
    def harmonizeRepositoryNames(self):
        for dt in self.dataTypes():                             # loop all data types
            self.report_d[dt].harmonizeRepositoryNames()
