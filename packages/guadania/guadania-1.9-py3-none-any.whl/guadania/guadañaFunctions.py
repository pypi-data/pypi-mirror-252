...
def compliance_section_summary(prismasession, complianceStandard, return_json=False):
    """
    ...
    res = []
    for summary in rawResponse["complianceDetails"]:
        res.append(summary)
...
def compliance_section_by_policy(prismasession, complianceStandard, accountId = None, accountGroup = None, return_json=False):
    ...
    for section in section_summary:
        ...
        for index, pol in policies.iterrows():
            for comp in pol['complianceMetadata']:
                if(section['name'] == comp['sectionId'] and comp['standardName'] == complianceStandard):
                    section_req[section["name"]] = comp["requirementName"]
                    failed_resources[comp['sectionId']][pol['name']] = {'failed':0, 'total':section['totalResources'], 'severity':pol['severity']}
        ...
        for index, row in section_alerts.iterrows():
            ...
            for comp in pol_comp:
                if comp['standardName'] == complianceStandard and comp['sectionId'] == section['name']:

                    polname = policies[policies['policyId'] == comp['policyId']]['name'].item()
                    failed_resources[comp['sectionId']][polname]['failed'] +=1
    ...
    df = {"accountGroup":[], "subscriptionId":[], "standard": [], "requirement": [], "section": [], "policy": [], "severity":[], "failed": [], "totalSection": []}
    for sec in failed_resources.keys():
        for pol in failed_resources[sec].keys():
            df["accountGroup"].append(accountGroup)
            df["subscriptionId"].append(accountId)
