...
from endi_celery.models import Job
...
from endi.utils.renderer import set_close_popup_response
...
class BaseView:
    def __init__(self, context, request=None):
        self.logger = logging.getLogger("endi.views.__init__")
        self.logger.debug(f"Using view {self}")
        if request is None:
...
class BaseListClassWithForm(BaseListClass):
    ...
    def get_filter_button(self):
        ...
        """
        self.logger.debug("Building the filter button : %s" % self.filter_button_label)
    ...
    def _collect_appstruct(self):
        ...
        if self.schema is not None:
            ...
            try:
                ...
                self.error = e
                # https://framagit.org/endi/endi/-/issues/2789
                # raise e
...
class BaseCsvView(BaseListClassWithForm):
    ...
    def _init_writer(self):
        self.logger.debug("# Initializing a writer %s" % self.writer)
        self.logger.debug(" + For the model : %s" % self.model)
    ...
    def _build_return_value(self, schema, appstruct, query):
        ...
        self.logger.debug(" + Streaming rows")
        for item in self._stream_rows(query):
            writer.add_row(item)
        self.logger.debug(" + Writing the file to the request")
...
class BaseFormView(FormView, PopupMixin):
    ...
    **Methods that you may implement**
    ...
    def get_schema(
        ...
        if self.schema is not None:
            schema = self.schema
            if inspect.isclass(schema) or inspect.isfunction(schema):
                schema = schema()
            return schema
        else:
...
class DisableView(BaseView):
    ...
    def __call__(self):
        if getattr(self.context, self.active_key):
            setattr(self.context, self.active_key, False)
            ...
            setattr(self.context, self.active_key, True)
            self.dbsession.merge(self.context)
...
class BaseRestView(BaseView):
    ...
    def _submit_datas(self, submitted, edit=False):
        self.logger.debug(" + Submitting %s" % submitted)
        submitted = self.pre_format(submitted, edit)
        self.logger.debug(" + After pre format %s" % submitted)
        ...
        self.logger.debug(" + After deserialize : %s" % attributes)
        if edit:
...
class TreeMixin(metaclass=TreeMixinMetaClass):
    ...
    def get_back_url(cls, request):
        logger.debug("Asking for the parent url : {0}".format(cls))
        if cls.parent_view is not None:
            return cls.parent_view.get_me_as_back_url(request)
        else:
            return None
...
def redirect_to_index_view(request, context):
    """
    Vue redirigeant l'utilisateur vers l'index d'enDI (utilisée lorsque l'on
    remappe les urls)
    """
    return HTTPFound("/")
...
class AsyncJobMixin:
    ...
    def initialize_job_result(self, job_class, **kwargs) -> Job:
        ...
        logger.debug(f"    + The job {job.id} was initialized")
        logger.debug("    + Delaying the task to celery")
        return job
...
class RestListMixinClass(BaseListClass):
        ...
        if schema is not None:
            try:
                ...
                if submitted:
                    ...
                    self.logger.debug("Submitted filter datas : {}".format(submitted))
    ...
    def collection_get(self):
        ...
        if query is not None:
            ...
            self.logger.debug(f"{count} items {query.count()}")
        else:
            count = 0
...
def endi_add_route(config, route_name, **kwargs):
