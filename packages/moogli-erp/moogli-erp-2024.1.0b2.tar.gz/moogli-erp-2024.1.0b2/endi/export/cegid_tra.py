...
from endi.interfaces import ITreasuryWriter, ITreasuryInvoiceWriter
...
class TraExporter:
    ...
    def format_general_entry(
        ...
        if compte_tiers is not None and compte_tiers != "":
            quadra_line += compte_tiers.rjust(8)
        else:
            quadra_line += compte_cg.rjust(8)

        # Code journal de 2 caractères (à priori inutile)
        quadra_line += code_journal.ljust(2)

        # Folio
        quadra_line += "000"

        # Date
        quadra_line += date.ljust(6)
