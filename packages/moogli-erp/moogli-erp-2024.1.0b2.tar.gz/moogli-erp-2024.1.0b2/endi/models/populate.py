...
GROUPS = (
    ...
    },
    {
        ...
        "label": "Peut valider ses propres factures d'avoir",
    },
    {
    ...
    },
    {
        ...
        "label": "Peut valider ses propres commandes fournisseur",
    },
    {
        "name": "supplier_invoice_validation",
        "label": "Peut valider ses propres factures fournisseur",
    },
    {
        "name": "payment_admin",
        "label": "Peut saisir/modifier/supprimer les paiements de ses factures",
    },
    {
        "name": "trainer",
        "label": "Formateur",
...
def populate_training_types(session):
    ...
    if (
        ...
        session.add(
            TrainingTypeOptions(label="Petit groupe (moins de 5 personnes)", order=2)
        )
...
def populate_groups(session):
    ...
    for group in GROUPS:
        if session.query(Group.id).filter(Group.name == group["name"]).count() == 0:
...
def populate_system_user(session):
    ...
    """
    from endi.models.user import User, Login
...
def populate_accounting_balance_sheet_types(session):
    ...
    from endi.models.accounting.balance_sheet_measures import (
...
def populate_doctype_label_override(session):
    ...
    """
    from endi.models.project.naming import LabelOverride
    from endi.models.project.types import BusinessType
...
def populate_notification_types_and_channels(session):
    from endi.models.notification import NotificationEventType, NotificationChannel
