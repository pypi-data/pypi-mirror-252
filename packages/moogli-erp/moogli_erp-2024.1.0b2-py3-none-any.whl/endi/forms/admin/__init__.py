...
from endi.utils.accounting import ACCOUNTING_SOFTWARES
...
CONFIGURATION_KEYS = {
    ...
    "internalcode_journal_frns": {
        "title": (
            "Code journal utilisé pour les factures et paiements fournisseurs internes"
        ),
    },
    "internalcae_general_supplier_account": {
        "title": "Compte fournisseur général de la CAE",
        "description": (
            "Pour tous les fournissseurs internes de toutes "
            "les enseignes. "
            "une valeur spécifique pour une enseigne peut être paramétrée au "
            "niveau de l'enseigne. une valeur spécifique à un fournisseur "
...
class ActivityTypeConfig(colander.MappingSchema):
    ...
    label = colander.SchemaNode(
        colander.String(), title="Libellé", validator=colander.Length(max=100)
    )
