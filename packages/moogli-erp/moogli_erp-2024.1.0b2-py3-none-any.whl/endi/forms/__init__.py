...
from endi.utils.fileupload import FileTempStore
...
def get_fileupload_widget(
    store_url, store_path, session, default_filename=None, filters=None
):
    """
        return a file upload widget

    :param list filters: Callable filters than can filter the uploaded files
    """
    ...
    return deform.widget.FileUploadWidget(tmpstore, template="fileupload.pt")
...
def public_file_appstruct(request, config_key, file_object):
    ...
    obj file_object: A :class:`endi.models.files.ConfigFile` instance
    :rtype: dict
    """
...
def file_appstruct(request, file_id):
    ...
    :param obj request: The Pyramid request
    :param int file_id: The id of a :class:`endi.models.files.File`
    instance
    :rtype: dict
    """
    if file_id is None:
...
def get_deferred_model_select_validator(model, id_key="id", filters=[]):
    ...
    def deferred_validator(node, kw):
        ...
        """
        query = model.query().options(load_only(id_key))
        for filter_def in filters:
            query = _add_filter_to_model_query(node, kw, model, query, filter_def)
...
def get_model_select_option_values(
    node, bind_params, model, keys, filters=(), add_default=True, empty_filter_msg=""
):
    """
    ...
    query = model.query()
    for filter_def in filters:
        query = _add_filter_to_model_query(node, bind_params, model, query, filter_def)
...
def get_deferred_model_select_checkbox(
    ...
    def deferred_widget(node, bind_params):
        ...
        """
        values = get_model_select_option_values(
            node, bind_params, model, keys, filters, add_default=False
        )
...
class CustomModelSchemaNode(colander.SchemaNode):
    ...
    def objectify(self, value):
        ...
        res = (
            DBSESSION()
            .query(self.model)
            .filter(getattr(self.model, self.name) == value)
            .first()
        )
        if res is None:
            res = colander.null
...
def get_sequence_child_item(
    model, required=False, child_attrs=("id", "label"), filters=[]
):
    """
    ...
    return [
        get_sequence_child_item_id_node(
            widget=get_deferred_model_select(model, keys=child_attrs, filters=filters),
            missing=missing,
            model=model,
            validator=get_deferred_model_select_validator(model, filters=filters),
        )
    ]
...
def get_model_checkbox_list_node(model, model_attrs=("id", "label"), filters=[], **kw):
    ...
    return CustomModelSequenceSchemaNode(
        ...
        widget=get_deferred_model_select_checkbox(
            model, keys=model_attrs, filters=filters, widget_options=widget_options
        ),
        **kw,
    )
