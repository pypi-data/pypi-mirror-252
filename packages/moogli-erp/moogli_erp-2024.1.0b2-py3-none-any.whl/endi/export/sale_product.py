...
from endi import version
from endi.export.utils import JSONExportSQLAlchemySchemaNode
from endi.models.company import Company
from endi.models.sale_product import (
    BaseSaleProduct,
    SaleProductTraining,
    SaleProductWork,
)
...
class EndiSalesCatalogExportSchema(colander.MappingSchema):
    """
    ...
    data_format = colander.SchemaNode(
        colander.String(),
        validator=colander.OneOf([version(strip_suffix=True)]),
    )
...
def serialize_catalog(company: Company) -> dict:
    ...
    """
    export_schema = EndiSalesCatalogExportSchema()
    queries = {
    ...
    out = {
        "data_format": version(strip_suffix=True),
        "data": {},
    }
