...
def run_anything(
    ...
    design = load_user_default_design().value_or(instances()) + design + load_user_overrides_design().value_or(instances())
...
@safe
def load_design_from_paths(paths, design_name) -> Result[Design]:
    res = instances()
    for path in paths:
        if path.exists():
