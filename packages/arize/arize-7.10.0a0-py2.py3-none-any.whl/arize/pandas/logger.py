...
from .spans.columns import SPAN_OPEN_INFERENCE_COLUMN_NAMES
from .spans.utils import jsonify_dataframe
from .spans.validation.validator import SpansValidator
...
class Client:
    ...
    def log_spans(
        ...
        # NOTE: Validation:
        # - validate_direct_input_types
        #    - validate convertible to string: model_id, model_version
        #    - validate dataframe is of type dataframe
        # - validate_dataframe
        #    - validate dataframe index is well sorted
        #    - validate dataframe column names are conforming with open inference
        #    - validate dataframe column contain the right python data structure
        # - validate_arrow_types
        # - validate_values

        spans_validator = SpansValidator()
        if verbose:
            logger.info("Performing direct input type validation.")
        errors = spans_validator.validate_direct_input_types(
            dataframe=dataframe,
            model_id=model_id,
        ...
        if validate:
            ...
            errors = spans_validator.validate_dataframe_values(dataframe)
            if errors:
                for e in errors:
                    logger.error(e)
                raise err.ValidationFailure(errors)

        # NOTE(Kiko): Should we do this?
        if verbose:
            logger.info("Converting dictionaries to JSON objects.")
        dataframe = jsonify_dataframe(dataframe)

        if verbose:
            logger.info("Removing unnecessary columns.")
        dataframe = self._remove_extraneous_columns(
            df=dataframe, open_inference_column_list=SPAN_OPEN_INFERENCE_COLUMN_NAMES
        )
