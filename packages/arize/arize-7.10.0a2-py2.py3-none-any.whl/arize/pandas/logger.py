...
from .spans.utils import (
    ...
    convert_timestamps,
    replace_indetermine_math_values_with_none,
)
...
class Client:
    ...
    def log_spans(
        ...
        if ROOT_LEVEL_SPAN_KIND_COL_NAME in df.columns and SPAN_KIND_COL_NAME not in df.columns:
            if verbose:
                logger.info("Moving span kind to atributes")
            df.rename(columns={ROOT_LEVEL_SPAN_KIND_COL_NAME: SPAN_KIND_COL_NAME}, inplace=True)
        if verbose:
            logger.info("Replacing indtermined math values with None")
        df = replace_indetermine_math_values_with_none(df)
    ...
    def _log_arrow(
        ...
        try:
            if verbose:
                logger.info(f"Writing table to temporary file: {tmp_file}")
