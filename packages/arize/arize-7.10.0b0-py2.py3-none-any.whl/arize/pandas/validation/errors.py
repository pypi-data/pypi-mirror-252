...
class InvalidPredActColumnNamesForModelType(ValidationError):
    ...
    def error_message(self) -> str:
        ...
        if self.allowed_columns is not None:
            allowed_col_msg = f" Only allowed columns {log_a_list(self.allowed_columns, 'and')}"
        return (
            f"Cannot use following columns: {log_a_list(self.wrong_columns, 'or')}  for "
            f"{self.invalid_model_type} model type." + allowed_col_msg
        )
...
class InvalidNumClassesMultiClassMap(ValidationError):
    ...
    def __init__(self, invalid_col_num_classes: Dict[str, List[str]]) -> None:
        self.invalid_col_num_classes = invalid_col_num_classes

    def error_message(self) -> str:
        err_msg = ""
        for col, num_classes in self.invalid_col_num_classes.items():
            err_msg += (
                f"Multi-Class dictionary for the following column: {col} had {len(num_classes)} rows"
                ...
                f"invalid number of classes: {log_a_list(num_classes, 'and')}\n"
            )
        return err_msg
...
class InvalidMultiClassClassNameLength(ValidationError):
    ...
    def __init__(self, invalid_col_class_name: Dict[str, List[str]]) -> None:
    ...
    def error_message(self) -> str:
        ...
        for col, class_names in self.invalid_col_class_name.items():
            err_msg += (
                f"Found invalid class names: {log_a_list(class_names, 'and')} in the {col} column. Class"
...
class InvalidMultiClassPredScoreValue(ValidationError):
    ...
    def __init__(self, invalid_col_class_scores: Dict[str, List[str]]) -> None:
    ...
    def error_message(self) -> str:
        ...
        for col, scores in self.invalid_col_class_scores.items():
            err_msg += (
                f"Found invalid scores: {log_a_list(scores, 'and')} in the {col} column that was "
...
class InvalidMultiClassThresholdClasses(ValidationError):
    ...
    def __init__(
        self, name: str, prediction_class_sets: List[set], threshold_class_sets: List[set]
    ) -> None:
        self.name = name
        self.prediction_class_sets = prediction_class_sets
        self.threshold_class_sets = threshold_class_sets

    def error_message(self) -> str:
        err_msg = (
            "Multi-Class Prediction Scores and Threshold Scores Dictionaries must contain the same "
        )
        f"classes. Found {len(self.prediction_class_set)} rows with mismatches."
        for index, prediction_class_set in enumerate(self.prediction_class_sets):
            err_msg += (
                "The following classes of the Prediction Scores Dictionary are not in the Threshold "
                f"Scores Dictionary: {prediction_class_set.difference(self.threshold_class_sets[index])}"
                "\nThe following classes of the Threshold Scores Dictionary are not in the Prediction Scores "
                f"Dictionary: {self.threshold_class_sets[index].difference(prediction_class_set)}\n"
            )
        return err_msg
