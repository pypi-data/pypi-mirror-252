...
from copilotsdk.ws import websocketWrapper

class WebexCopilot:
    def __init__(self, server):
        self.sesssion_id = str(uuid.uuid4())
        self.websocket_uri = f"ws://{server}:9001/{self.sesssion_id}"
        self.upload_uri = f"http://{server}:9000/upload/"
    ...
    async def _generate_anomaly_report_from_url(self,download_link):
        msg = {
            "target": "webex_agent",
            "instruction": "open-log-from-link",
