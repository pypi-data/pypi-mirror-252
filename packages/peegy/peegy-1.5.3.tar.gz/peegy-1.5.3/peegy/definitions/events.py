...
class Events(object):
    ...
    def get_events(self, code=None, include_bad_events: bool = False):
        if code is None:
            return np.array([_ev for _ev in self.events(include_bad_events=include_bad_events)])
        else:
            return np.array([_ev for _ev in self.events(include_bad_events=include_bad_events) if _ev.code == code])

    def get_events_index(self, code=None, fs=None, include_bad_events: bool = False):
        if code is None:
            return np.array([int(_ev.time_pos * fs) for _ev in self.events(include_bad_events=include_bad_events)])
        else:
            return np.array([int(_ev.time_pos * fs) for _ev in self.events(include_bad_events=include_bad_events)
                             if _ev.code == code])

    def get_events_time(self, code=None, include_bad_events: bool = False):
        if code is None:
            ...
            return np.array([_ev.time_pos.to(u.s).value for _ev in self.events(include_bad_events=include_bad_events)
                             if _ev.code == code]) * u.s

    def get_events_duration(self, code=None, include_bad_events: bool = False):
        if code is None:
    ...
    def get_events_code(self, code=None, include_bad_events: bool = False):
        if code is None:
