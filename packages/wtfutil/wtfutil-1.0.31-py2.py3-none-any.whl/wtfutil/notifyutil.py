...
def bark(title: str, content: str) -> None:
    ...
    response = util.requests_session().get(url).json()
...
def dingding_bot(title: str, content: str) -> None:
    ...
    response = util.requests_session().post(
...
def feishu_bot(title: str, content: str) -> None:
    ...
    response = util.requests_session().post(url, data=json.dumps(data)).json()
...
def gotify(title: str, content: str) -> None:
    ...
    response = util.requests_session().post(url, data=data).json()
...
def iGot(title: str, content: str) -> None:
    ...
    response = util.requests_session().post(url, data=data, headers=headers).json()
...
def chat(title: str, content: str) -> None:
    ...
    response = util.requests_session().post(url, data=data)
...
def pushplus_bot(title: str, content: str) -> None:
    ...
    response = util.requests_session().post(url=url, data=body, headers=headers).json()
    ...
    if response["code"] == 200:
        ...
        response = util.requests_session().post(url=url_old, data=body, headers=headers).json()
...
def qmsg_bot(title: str, content: str) -> None:
    ...
    response = util.requests_session().post(url=url, params=payload).json()
...
class WeCom:
    ...
    def get_access_token(self):
        ...
        req = util.requests_session().post(url, params=values)
    ...
    def send_text(self, message, touser="@all"):
        ...
        respone = util.requests_session().post(send_url, send_msges)
...
def pipehub(title:str, content: str) -> None:
    ...
    if not push_config.get("PIPEHUB_KEY"):
        print("pipehub 服务的 PIPEHUB_KEY 未设置!!\n取消推送")
        return
    print("pipehub 服务启动")
    if title:
        content = f"{title}\n\n{content}"
    response = util.requests_session().post(f'https://api.pipehub.net/send/{push_config.get("PIPEHUB_KEY")}', data=content.encode('utf-8'))
    if response.status_code == 200:
        print("pipehub 推送成功！")
    else:
        print(f"pipehub 推送失败！{response.status_code} {response.text}")
...
def xtuis(title: str, content: str) -> None:
    ...
    if not push_config.get("XTUIS_KEY"):
        print("xtuis 服务的 XTUIS_KEY 未设置!!\n取消推送")
        return
    print("xtuis 服务启动")
    response = util.requests_session().post(f'https://wx.xtuis.cn/{push_config.get("XTUIS_KEY")}.send', data={
        "text": title,
        "desp": content,
    })
    if response.status_code == 200:
        ...
        print(f"xtuis 推送失败！{response.status_code} {response.text}")
...
def aiops_phone(title: str, content: str) -> None:
    ...
    if not push_config.get("AIOPS_KEY"):
        print("aiops 服务的 AIOPS_KEY 未设置!!\n取消推送")
        return
    print("aiops 服务启动")
    import uuid
    response = util.requests_session().post("http://api.aiops.com/alert/api/event", json={
    ...
    if response.status_code == 200:
        ...
        print(f"aiops 推送失败！{response.status_code} {response.text}")
...
def chronocat(title: str, content: str) -> None:
    ...
    for chat_type, ids in [(1, user_ids), (2, group_ids)]:
        ...
        for chat_id in ids:
            ...
            response = util.requests_session().post(url, headers=headers, data=json.dumps(data))
            if response.status_code == 200:
                if chat_type == 1:
...
def custom_notify(title: str, content: str) -> None:
    ...
    response = util.requests_session().request(
...
def one() -> str:
    ...
    res = util.requests_session().get(url).json()
