...
class Query:
    ...
    def _to_query_sql(self, **kwargs) -> str:
        ...
        if self.partition_type == PartitionType.DATE.value:
            ...
            return f"{self.query}".replace('#PARTITION-DATE#', self.partition_date.strftime('%d.%m.%Y'))
        else:
            raise NotImplementedError(
                f"Partition type {self.partition_type} not implemented"
            )
    ...
    def proceed(self, **kwargs):
        ...
        if self.config.partition_type == PartitionType.DATE.value:
            file_uri = self.storage_data.put_dataframe(
                f"{self.partition_date.strftime('%Y')}/{self.partition_date.strftime('%m')}/{self.partition_date.strftime('%d')}/{self.config.name}",
                self.data,
            )
        else:
            file_uri = self.storage_data.put_dataframe(
        ...
        # push to warehouse
        if debug:
            print("Partition Column", self.config.partition_type, PartitionType.DATE.value)
        table_name = self.config.warehouse_table if self.config.warehouse_table else self.config.name
        if self.config.partition_type == PartitionType.DATE.value:
            table_name = f"{self.config.name}_{self.partition_date.strftime('%Y%m%d')}"
        self.warehouse_adapter.push(table_name, file_uri)
        return self

    @property
    def stats(self) -> str:
        return f"{self.total_rows} rows"
