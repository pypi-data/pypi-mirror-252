...
from extended_algo import TradeType
...
class VectorStrategy(ABC):
    ...
    def __init__(self, symbol: str, start: dt.datetime, end: dt.datetime, profit_target=100., stop_loss=100.,
                 breakeven_tigger: float = None, breakeven_stop_loss=0.,
        ...
        self.breakeven_tigger = breakeven_tigger
    ...
    def _calculate_sl_pt_bt_bsl(self, df):
        # TODO: I need to make these profit_target, stop_loss, breakeven and breakeven_stop_loss dynamic
        #  I am not sure if force_closeout and signal_timeout needs to be dynamic of if can be static

        # They way I plan to do this is my apply 'pt' 'sl' 'bt' 'bsl', 'force_closeout', 'signal_timeout' as np.where statements
        # I also need to round the price to the closest tick price

        # TODO I think the pt, sl, bt values should be offset numbers

        # TODO: I need to do round the self.profit_target as well as pt_offset to the nearest tick
        #  LONG round down for an earlier fill
        #  SHORT round up for an earlier fill
        #  PT be more conservative with profit targets
        # TOOD: I think you need to handle the error case where someone accidentally enters a negative PT
        ...
        if self.breakeven_tigger:
            ...
            df['bt_offset'] = df['bt_offset'].fillna(self.breakeven_tigger)
        ...
        # print(df[['entry_time', 'pt_offset', 'sl_offset', 'bt_offset', 'bsl_offset', 'entry_price', 'pt', 'sl', 'bt', 'bsl']])
        return df
    ...
    def _apply_stoploss_and_profit_targets(self):
        ...
        if 'entry_type' in df.columns:
            df_stop_entries = df[df.entry_type == TradeType.STP].copy()
            df_limit_entries = df[df.entry_type == TradeType.LMT].copy()
