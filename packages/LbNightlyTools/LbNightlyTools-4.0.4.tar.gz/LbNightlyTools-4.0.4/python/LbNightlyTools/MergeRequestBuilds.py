...
def set_last_commits(slot, ref_date, commits):
    ...
    for name, p in list(slot_repos(slot).items()):
        ...
        logger.debug("Setting checkout for '{}' to be '{}'".format(name, commit))
...
def get_model_slot(target_branches, slots):
    ...
    for slot in slots:
        for project in target_branches:
            if (
                hasattr(slot, project)
                and getattr(slot, project).version != target_branches[project]
            ):
                break
        else:
    ...
    if not candidates:
        raise RuntimeError(
            'Cannot determine Slot model configuration from target branches "{}".'.format(
                target_branches
            )
        )
    elif len(candidates) > 1:
        raise RuntimeError(
            'More than one Slot model configuration for target branches "{}".'.format(
                target_branches
            )
        )
    return candidates[0]
...
def create_mr_slots(
    ...
    if not force_model:
        ...
        if not model_slots:
            raise RuntimeError("cannot find model slot {}".format(force_model))
...
def post_gitlab_feedback(ref_slot, test_slot, flavour, mr_slots_config):
    ...
    ref_id = "{flavour}/{ref_slot}/{ref_build}".format(
        flavour=flavour,
        ref_slot=ref_slot.name,
        ref_build=ref_slot.build_id,
    )
    test_id = "{flavour}/{test_slot}/{test_build}".format(
        flavour=flavour,
