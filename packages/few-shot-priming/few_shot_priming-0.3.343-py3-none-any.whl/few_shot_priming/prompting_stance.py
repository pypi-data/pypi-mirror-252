...
def format_prompt( dataset, tokenizer, labels_text_map, token_limit, few_shot_size, logger, alpaca_examples=False, record=None):
    label_str = ""
    for i, label in enumerate(labels_text_map):
        if i == len(labels_text_map) - 1:
            label_str = label_str + " or " +  labels_text_map[label][0]
        else:
            label_str = label_str + ", " + labels_text_map[label][0]
    examples = ""
    if record:
    ...
    return promet_head + examples + prompt_head_2 + template_no_stance
...
def run_experiment_prompting(config=None, experiment="ibmsc", model=None, offline=False, validate=True, splits=None, logger=None, openai=False, topic_count=None, sampling_strategy=None, similarity=None):
    ...
    for i, record in test_dataset.iterrows():
        ...
        prompt_to_predict = format_prompt(training_dataset, tokenizer, labels_text_map, model_input_limit, few_shot_size, logger, record, few_shot_size)
