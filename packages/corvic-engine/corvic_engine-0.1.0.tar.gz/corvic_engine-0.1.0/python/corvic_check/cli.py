...
@cli.command()
@click.option(
    "--profile", type=click.Choice(list(Profile)), default=Profile.DEBUG_RELEASE
)
@click.option(
...
@cli.command()
@click.option("--profile", type=click.Choice(list(Profile)), default=Profile.OPT_DEV)
@click.option(
    "--default-features/--no-default-features", default=True, show_default=True
)
...
def lint(path: pathlib.Path | None):
    """Lint code and files."""
    cwd = path if path else _project_root()
