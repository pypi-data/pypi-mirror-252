...
from mltraq.storage import models, pyson, serialization
from mltraq.storage.database import Database, next_ulid, pandas_query, sanitize_table_name, short_uuid
...
class Experiment:
    ...
    __slots__ = ("id_experiment", "db", "name", "fields", "runs")
    ...
    def __init__(
        ...
        if name is None:
            name = short_uuid(self.id_experiment)
        self.name = name
    ...
    def load_runs(self, runs_meta):
        """Load runs for experiment from db, considering runs_meta.

        Args:
            runs_meta (_type_): dictionary that specifies which columns have
        ...
        for col_name in runs_meta["serialized"]:
            df[col_name] = df[col_name].map(pyson.deserialize)

        # Reconstruct fields
        def series_to_run(row: pd.Series):
            fields = row[runs_meta["serialized"] + runs_meta["non-serialized"]].to_dict()
    ...
    def load(cls, db: Database, name: str, pickle=False):
        ...
        columns = [
            ...
            Experiment.model_cls.runs_size,
            Experiment.model_cls.runs_meta,
        ...
        with db.session() as session:
            ...
            experiment = Experiment(
                ...
                name=record.name,
                fields=pyson.deserialize(record.fields),
            )

            if record.runs_size > 0:
                experiment.load_runs(runs_meta=pyson.deserialize(record.runs_meta))
    ...
    def execute(
        ...
        Args:
            ...
            config: Fixed arguments to pass to the run.
        ...
        self.runs.execute(steps=steps, config=config, backend=backend, n_jobs=n_jobs)
    ...
    def record(
        ...
        return self.model_cls(
            ...
            runs_meta=serialization.serialize(runs_meta, enable_compression=enable_compression),
        )
    ...
    def info(self):
        ...
        if self.runs:
            run = self.runs.first()
            run_config = list(run.config.keys())
    ...
    def persist(
        ...
        log.debug(f"Persisting experiment (table name: {self.tablename()})")
        ...
        if self.runs:
            ...
            df_runs, runs_meta = serialization.serialize_df(
            ...
            runs_meta = {"serialized": [], "non-serialized": [], "compression": enable_compression}

        # Insert row in "experiments" table.

        with self.db.session() as session:
            session.add(
                self.record(runs_meta=runs_meta, store_pickle=store_pickle, enable_compression=enable_compression)
            )
            session.commit()
        ...
        dtype = {
            ...
            **{col_name: models.LargeBinary for col_name in runs_meta["serialized"]},
        }
