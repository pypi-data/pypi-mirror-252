...
def parallel(steps: List[Callable], n_jobs: int = None) -> List[object]:
    ...
    # sharedmem is required to share the current value of the object mltraq.options.options.
    p = ProgressParallel(n_jobs=n_jobs, total=len(steps))  # , require="sharedmem")
...
class Job:
    ...
    def execute(self):
        ...
        if self.backend == "dask":
            ...
            log.debug(f"Dashboard: {client.dashboard_link}")
