...
"""
This module provide base class for FCE service clients.
"""
...
class FceBaseClient(object):
    ...
    def __init__(self, config, region_supported=True):
        ...
        # just for debug
        self.config = copy.deepcopy(fce_client_configuration.DEFAULT_CONFIG)
    ...
    def _compute_endpoint(self):
        ...
        if self.region_supported:
            return b'%s://%s.%s.%s' % (
                ...
                self.service_id,
                self.config.region,
                flymefce.DEFAULT_SERVICE_DOMAIN)
        else:
            return b'%s://%s.%s' % (
                self.config.protocol,
                self.service_id,
    ...
    def _send_request(self, http_method, path, headers=None, params=None, body=None):
        return fce_http_client.send_request(
            self.config, fce_v1_signer.sign, [handler.parse_error, handler.parse_json],
    ...
    def _add_path_param(self, apiConfig, key, value):
        if value is None:
            raise FceClientError(b"Path param can't be none.")
