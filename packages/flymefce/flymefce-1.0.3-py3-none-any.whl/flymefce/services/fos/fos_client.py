...
import flymefce
from flymefce import fce_client_configuration
from flymefce import utils
from flymefce.auth import fce_v1_signer
from flymefce.fce_base_client import FceBaseClient
...
from flymefce.http import fce_http_client
from flymefce.http import handler
from flymefce.http import http_content_types
from flymefce.http import http_headers
from flymefce.http import http_methods
from flymefce.services import fos
from flymefce.services.fos import fos_handler
from flymefce.services.fos import storage_class
from flymefce.utils import required
...
DEFAULT_FOS_DOMAIN_SUFFIX = b'fos.flymeyun.com'
...
class FosClient(FceBaseClient):
    """
    sdk client
    """

    def __init__(self, config=None):
        FceBaseClient.__init__(self, config)
    ...
    def list_buckets(self, config=None):
        ...
        :param config: None
        :type config: FceClientConfiguration
        :returns: all buckets owned by the user.
        :rtype: flymefce.fce_response.FceResponse
        """
    ...
    def does_bucket_exist(self, bucket_name, config=None):
        ...
        try:
            ...
            return True
        except FceHttpClientError as e:
            if isinstance(e.last_error, FceServerError):
    ...
    def set_bucket_canned_acl(self, bucket_name, canned_acl, config=None):
        ...
        self._send_request(http_methods.PUT,
                           bucket_name,
                           headers={http_headers.FCE_ACL: canned_acl},
                           params={b'acl': b''},
                           config=config)
    ...
    def generate_pre_signed_url(self,
        ...
        specified  protocol in endpoint > protocol > default protocol in config.

        Args:
            bucket_name: Bucket名称
            key: 对象Key
            timestamp: 时间戳
            expiration_in_seconds: 过期时长，单位秒，默认1800秒
            headers: 请求头
            params: 请求参数
            headers_to_sign: 需要加签的请求头
            protocol: 请求协议
            config: Http请求配置
            httpmethod: Http请求方法，默认GET
        ...
        endpoint_protocol, endpoint_host, endpoint_port = utils.parse_host_port(config.endpoint, config.protocol)
        ...
        if config.security_token is not None:
            params[http_headers.FLYME_STS_SECURITY_TOKEN.lower()] = config.security_token

        _, params = fce_v1_signer.sign(config.credentials,
                                       httpmethod,
                                       path,
                                       headers,
                                       params,
        ...
        # params[http_headers.AUTHORIZATION.lower()] = fce_v1_signer.flyme_sign(
        #     config.credentials,
        #     httpmethod,
        #     path,
        #     headers,
    ...
    def _parse_fos_object(http_response, response):
        ...
        prefix = compat.convert_to_string(
            http_headers.FCE_USER_METADATA_PREFIX
        )
        for k, v in headers_list:
    ...
    def get_object(self, bucket_name, key, range=None, traffic_limit=None, config=None):
        ...
        if len(key) == 0 or key.startswith(b"/"):
            raise FceClientError("Key can not be empty or start with '/' .")
    ...
    def restore_object(self, bucket_name, key, days=None, tier="Standard", config=None):
        ...
        headers = {}
        if days is not None:
            headers[http_headers.FOS_RESTORE_DAYS] = days
        if compat.convert_to_string(tier) not in ("Standard", "Expedited", "LowCost"):
            raise ValueError('invalid tier:{} for restore_object.The valid value is \"Standard\" or \"Expedited\" or ' \
                             '\"LowCost\"'.format(tier))
        headers[http_headers.FOS_RESTORE_TIER] = compat.convert_to_bytes(tier)
        return self._send_request(
            http_methods.POST,
            bucket_name,
            key,
    ...
    def get_object_to_file(self, bucket_name, key, file_name, range=None, config=None,
        ...
        return self._send_request(
            ...
            headers=range_header,
            config=config,
            body_parser=lambda http_response, response: FosClient._save_body_to_file(
                http_response,
                response,
                file_name,
    ...
    def append_object(self, bucket_name, key, data,
        ...
        if content_length > fos.MAX_APPEND_OBJECT_LENGTH:
            raise ValueError('Object length should be less than %d. '
                             'Use multi-part upload instead.' % fos.MAX_APPEND_OBJECT_LENGTH)
    ...
    def append_object_from_string(self, bucket_name, key, data,
        ...
        if isinstance(data, str):
            data = data.encode(flymefce.DEFAULT_ENCODING)
    ...
    def put_object(self, bucket_name, key, data,
        ...
        if content_length > fos.MAX_PUT_OBJECT_LENGTH:
            raise ValueError('Object length should be less than %d. '
                             'Use multi-part upload instead.' % fos.MAX_PUT_OBJECT_LENGTH)
    ...
    def copy_object(self,
        ...
        headers = self._prepare_object_headers(
            ...
            user_headers=user_headers,
            traffic_limit=traffic_limit)
        headers[http_headers.FCE_COPY_SOURCE] = utils.normalize_string(
            b'/%s/%s' % (
                compat.convert_to_bytes(source_bucket_name),
                source_key), False)
        if etag is not None:
            headers[http_headers.FCE_COPY_SOURCE_IF_MATCH] = etag
        if user_metadata is not None or content_type is not None:
            headers[http_headers.FCE_COPY_METADATA_DIRECTIVE] = b'replace'
        else:
            headers[http_headers.FCE_COPY_METADATA_DIRECTIVE] = b'copy'
    ...
    def upload_part_copy(self,
        ...
        headers[http_headers.FCE_COPY_SOURCE_RANGE] = range
        if etag is not None:
            headers[http_headers.FCE_COPY_SOURCE_IF_MATCH] = etag
    ...
    def put_super_obejct_from_file(self, bucket_name, key, file_name, chunk_size=5,
        ...
        if chunk_size > 5 * 1024 or chunk_size <= 0:
            raise FceClientError("chunk size is valid, it should be more than 0 and not nore than 5120!")
        left_size = os.path.getsize(file_name)
        # if file size more than 48.8TB, reject
        if left_size > 50000 * 1024 * 1024 * 1024:
            raise FceClientError("File size must not be more than 48.8TB!")
    ...
    def set_object_canned_acl(self, bucket_name, key,
        ...
        :param canned_acl: for header 'x-ic-acl', it's value only is
        ...
        if canned_acl is not None:
            headers = {http_headers.FCE_ACL: compat.convert_to_bytes(canned_acl)}
            num_args += 1
    ...
    def fetch_object(self, bucket_name, key, url,
                     ...
                     config=None):
        """
        fetch object with given url and save to Flyme object storage
        ...
        key = compat.convert_to_bytes(key)
        headers = {}
        headers[http_headers.FOS_FETCH_SOURCE] = compat.convert_to_bytes(url)
        if fetch_mode is not None:
            headers[http_headers.FOS_FETCH_MODE] = fetch_mode
        if storage_class is not None:
            headers[http_headers.FOS_STORAGE_CLASS] = storage_class
        return self._send_request(
            http_methods.POST,
    ...
    def put_object_symlink(self, bucket_name, target_key, symlink, forbid_overwrite=None,
        ...
        headers = self._prepare_object_headers(user_metadata=user_metadata,
                                               content_type=content_type,
                                               storage_class=storage_class)
        headers[http_headers.FOS_SYMLINK_TARGET] = target_key
        if forbid_overwrite is not None:
            if forbid_overwrite:
                headers[http_headers.FOS_FORBID_OVERWRITE] = b'true'
            else:
                headers[http_headers.FOS_FORBID_OVERWRITE] = b'false'
        if target_bucket is not None:
            headers[http_headers.FOS_SYMLINK_BUCKET] = compat.convert_to_bytes(target_bucket)
        return self._send_request(http_methods.PUT,
                                  bucket_name,
                                  symlink,
                                  headers=headers,
    ...
    def select_object(self, bucket_name, key, select_object_args, headers=None, config=None):
        ...
        self._send_request(
            ...
            bucket_name,
            key,
            body=json.dumps({'selectRequest': select_object_args}, default=FosClient._dump_acl_object),
            headers=headers,
            params={b'select': b'', b'type': select_type},
            config=config,
            body_parser=lambda http_response, response: FosClient._parse_select_message(
    ...
    def put_bucket_mirroring(self, bucket_name, mirror_args, config=None):
        ...
        return self._send_request(
            ...
            body=json.dumps({'bucketMirroringConfiguration': mirror_args}, default=FosClient._dump_acl_object),
    ...
    def _prepare_object_headers(
        ...
        if content_sha256 is not None:
            headers[http_headers.FCE_CONTENT_SHA256] = content_sha256
        ...
        if user_metadata is not None:
            ...
            for k, v in iteritems(user_metadata):
                ...
                normalized_key = http_headers.FCE_USER_METADATA_PREFIX + k
                headers[normalized_key] = v
                meta_size += len(normalized_key)
                meta_size += len(v)
            if meta_size > fos.MAX_USER_METADATA_SIZE:
                raise ValueError('Metadata size should not be greater than %d.' % fos.MAX_USER_METADATA_SIZE)
    ...
    def _get_user_header(headers, user_headers, is_copy=False):
        ...
        if not is_copy:
            ...
            user_headers_set = {http_headers.FCE_COPY_SOURCE_IF_NONE_MATCH,
                                http_headers.FCE_COPY_SOURCE_IF_UNMODIFIED_SINCE,
                                http_headers.FCE_COPY_SOURCE_IF_MODIFIED_SINCE}
    ...
    def _get_path(config, bucket_name=None, key=None, use_backup_endpoint=False):
        ...
        if config.cname_enabled or utils.is_cname_like_host(host) or utils.is_custom_host(host, bucket_name):
            return utils.append_uri(fos.URL_PREFIX, key)

        return utils.append_uri(fos.URL_PREFIX, bucket_name, key)
    ...
    def _merge_config(self, config, bucket_name):
        ...
        if bucket_name is not None and not utils.is_cname_like_host(self.config.endpoint):
            ...
            if user_endpoint.endswith(DEFAULT_FOS_DOMAIN_SUFFIX) and len(user_endpoint_split) == 3:
                # split http head
    ...
    def _send_request(
        ...
        if config.security_token is not None:
            headers = headers or {}
            headers[http_headers.FLYME_STS_SECURITY_TOKEN] = config.security_token

        try:
            return fce_http_client.send_request(
                config, fce_v1_signer.sign, [handler.parse_error, body_parser],
...
class SelectResponse(object):
    ...
    def init_from_http_response(self, http_response, response):
        """
        get HttpResponse and FceResponse
        """
            ...
            while not self.finish:
                ...
                if headers_map['message-type'] == 'Records':
                    ...
                    if headers_map["error-code"] != "success":
                        raise FceServerError(headers_map['error-message'], code=headers_map['error-code'],
                                             request_id=self.response.metadata.ic_request_id)
                        return
