...
"""
This module provide some tools for fce client.
"""
...
DEFAULT_CNAME_LIKE_LIST = [b".cdn.fos.flymeyun.com"]
...
def get_flyme_canonical_querystring(params, for_signature):
    """
    Flyme云加签查询参数字符串

    Args:
        params: dict: 请求参数
        for_signature: bool: 是否为了加签
    Returns:
        加签需要的查询参数字符串
    """
    if params is None:
    ...
    for k, v in iteritems(params):
        ...
        # 如果不是加签或者参数名称不是Authorization，则需要计算
        if not for_signature or k.lower() != http_headers.AUTHORIZATION.lower():
            if v is None:
                v = ''
...
def parse_host_port(endpoint, default_protocol):
    ...
    :type: flymefce.protocol.HTTP or flymefce.protocol.HTTPS
    ...
    try:
        ...
        raise ValueError('Invalid endpoint:%s, error:%s' % (endpoint, compat.convert_to_string(e)))

    if parse_result.scheme == compat.convert_to_bytes(flymefce.protocol.HTTP.name):
        protocol = flymefce.protocol.HTTP
        port = flymefce.protocol.HTTP.default_port
    elif parse_result.scheme == compat.convert_to_bytes(flymefce.protocol.HTTPS.name):
        protocol = flymefce.protocol.HTTPS
        port = flymefce.protocol.HTTPS.default_port
    else:
...
def generate_content_md5(body):
    ...
    content = hashlib.md5(body).digest()
    return base64.b64encode(content)


def is_custom_host(host, bucket_name):
    """
    custom host : xxx.fos.flymeyun.com
...
if __name__ == "__main__":
    import io
    s = io.BytesIO(b"hello world 1111")
    print(get_md5_from_fp(s))
