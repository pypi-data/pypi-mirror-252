...
class NativePPOTrainer(BaseTrainer):
    def __init__(
        ...
        *,
        ...
        save_freq: int = -1,  # if save_freg == -1, then disable save, if save_freq == 0, then save at the end of training,otherwise save every save_freq epochs  # noqa: E501
        ...
        for _ in range(epochs):
            ...
            for t in range(self.steps_per_env):
                ...
                if d:
                    ...
                    self.append_eposide_rollout_info(ep_length=ep_len, ep_return=ep_ret)
            ...
            if (self.save_freq and self.epoch % self.save_freq == 0) or self.epoch == epochs - 1:
                tmp_rtn = (
                    -1
                    if self.log_buffer_size == 0
                    else np.mean(self.info_buffer["rollout"]["eposide_return"]).astype(int)
                )
                model_path = os.path.join(
                    self.trainer_montior_table.save_path,
                    f"{self.epoch}_{tmp_rtn}.pth",
                )
                torch.save(self.policy, model_path)
                self.saved_model_pathes.append(model_path)
