# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/03_cgsense_pip_package/91_metrics.ipynb.

# %% auto 0
__all__ = ['epsilon', 'mse', 'rmse', 'nmse', 'nrmse', 'psnr', 'ssim', 'print_metrics']

# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 2
from nbdev.showdoc import *
from fastcore.test import *
from fastcore.utils import *

# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 4
import numpy as np
from prettytable import PrettyTable

# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 5
from skimage.metrics import peak_signal_noise_ratio, structural_similarity
from typing import Optional

# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 6
epsilon = 1e-10

# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 8
def mse(
...
# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 12
def rmse(
    gt: np.ndarray,     # first image (ground truth)
    pred: np.ndarray,   # second image (predicted) 
...
# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 16
def nmse(
    gt: np.ndarray,     # first image (ground truth)
    pred: np.ndarray,   # second image (predicted) 
...
# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 20
def nrmse(
    gt: np.ndarray,     # first image (ground truth)
    pred: np.ndarray,   # second image (predicted) 
) -> np.ndarray:        # NRMSE (normalized root-mean-square error)
...
# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 25
def psnr(
    gt: np.ndarray,     # first image (ground truth)
    pred: np.ndarray,   # second image (predicted)
...
# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 28
def ssim(
    gt: np.ndarray,     # first image (ground truth)
    pred: np.ndarray,   # second image (predicted)
    drange: float=None  # distance between min & max possible values
) -> np.ndarray:        # SSIM (structural similarity index metrix)
...
# %% ../nbs/03_cgsense_pip_package/91_metrics.ipynb 31
def print_metrics(
    gt: np.ndarray,     # first image (ground truth)
    pred: np.ndarray,   # second image (predicted)
):
