...
#   timestamp: 2024-01-02T10:27:42+00:00
...
class TruefoundryImageFull(BaseModel):
    """
    +usage=JupyterLab + All Common ML Libraries with persistent environment (Python 3.11.6)
    """
...
class Codeserver(BaseModel):
    ...
    home_directory_size: conint(ge=5, le=64000) = Field(
        10,
        description="+label=Home Directory Size in GB (Persistent)\n+usage=Size of the home directory for the code server (Persistent Storage)\n+sort=6",
    )
    resources: Optional[Resources] = None
    env: Optional[Dict[str, str]] = Field(
        None,
        description="+label=Environment Variables\n+usage=Configure environment variables to be set in the code server pod(s)\n+sort=10110",
    )
    mounts: Optional[List[Union[SecretMount, StringDataMount, VolumeMount]]] = Field(
        None,
        description="+usage=Configure data to be mounted to code server pod(s)\n+sort=10111",
    )
...
class Notebook(BaseModel):
    ...
    ] = Field(..., description="+usage=Pick a notebook image to deploy\n+sort=2")
    endpoint: Endpoint
    auth: BasicAuthCreds
...
class Application(BaseModel):
    __root__: Union[Service, AsyncService, Job, Notebook, Helm, Volume, Codeserver]
