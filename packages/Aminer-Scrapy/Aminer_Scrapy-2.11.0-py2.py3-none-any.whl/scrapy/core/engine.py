...
from twisted.internet.defer import Deferred, inlineCallbacks, succeed, maybeDeferred
...
class Slot:
    def __init__(
        ...
        self.start_requests: Optional[Iterator[Request]] = start_requests if inspect.isasyncgen(
            start_requests) else iter(start_requests)
...
class ExecutionEngine:
    ...
    async def async_next_request(self):
        while (
                ...
                and (await self.async_next_request_from_scheduler().__anext__()) is not None
        ):
            pass
        if self.slot.start_requests is not None and not self._needs_backout():
            if inspect.isasyncgen(self.slot.start_requests):
    ...
    def sync_next_request(self):
        while (
                not self._needs_backout()
                and self.sync_next_request_from_scheduler() is not None
        ):
            pass
    ...
    def _next_request(self) -> None:
        ...
        if self.paused:
            return None
        if self.is_async_scheduler:
            asyncio.create_task(self.async_next_request())
        else:
            self.sync_next_request()
    ...
    def sync_next_request_from_scheduler(self) -> Optional[Deferred]:
        assert self.slot is not None  # typing
        assert self.spider is not None  # typing
    ...
    async def async_next_request_from_scheduler(self):
        assert self.slot is not None  # typing
        assert self.spider is not None  # typing
    ...
    def crawl(self, request: Request) -> None:
        ...
        if request is None:
            ...
            if self.is_async_scheduler:
                asyncio.create_task(self.async_schedule_request(request, self.spider))
            else:
                self.sync_schedule_request(request, self.spider)
            self.slot.nextcall.schedule()  # type: ignore[union-attr]

    def sync_schedule_request(self, request: Request, spider: Spider) -> None:
        self.signals.send_catch_log(
            signals.request_scheduled, request=request, spider=spider
        )
        enqueued = self.slot.scheduler.enqueue_request(request)
        if not enqueued:  # type: ignore[union-attr]
            self.signals.send_catch_log(
                signals.request_dropped, request=request, spider=spider
            )

    async def async_schedule_request(self, request: Request, spider: Spider) -> None:
        self.signals.send_catch_log(
            signals.request_scheduled, request=request, spider=spider
        )
        enqueued = await self.slot.scheduler.enqueue_request(request)
        if not enqueued:  # type: ignore[union-attr]
            self.signals.send_catch_log(
                signals.request_dropped, request=request, spider=spider
            )
    ...
    def open_spider(
        ...
        self.is_async_scheduler = inspect.iscoroutinefunction(scheduler.next_request)
        self.spider = spider
        if hasattr(scheduler, "open"):
            if inspect.iscoroutinefunction(scheduler.open):
                yield deferred_from_coro(scheduler.open(spider))
            else:
