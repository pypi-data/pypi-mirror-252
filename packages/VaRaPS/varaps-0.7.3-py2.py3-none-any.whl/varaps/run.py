...
def analyse_file(file, PATH_RESULT, PATH_M_MATRIX, NbBootstraps, alphaInit, freezeAlpha):
    ...
    for i in range(NbBootstraps):
        ...
        start = time.time()
        res_CoOcc = VariantsProportionCoOcc.VariantsProportionCoOcc(
            starts_idx_new,
            ends_idx_new,
            muts_data_new,
            M,
        ...
        res_CoOcc()
        res_CoOcc.fit(freezeAlpha=freezeAlpha)
        # if np.abs(pi0 - pi000)<0.001:
        result = res_CoOcc.params

        # order result idx by decreasing order
        idxs = np.argsort(result)[::-1]
        # print result
        # for i in idxs:
        #     print(variants[i], res_CoOcc.params[i])
        # save result
        ...
        resCooc[i, :nV] = result
        resCooc[i, nV] = res_CoOcc.alpha
        resCooc[i, nV + 1] = res_CoOcc.nbIter_alpha_fixed
        resCooc[i, nV + 2] = res_CoOcc.time_alpha_fixed
        resCooc[i, nV + 3] = res_CoOcc.nbIter_alpha
        resCooc[i, nV + 4] = res_CoOcc.time_alpha
        resCooc[i, nV + 5] = res_CoOcc.time_used
        print(top5)
...
def analyze_file_mode2(path_X, path_M, path_result, nb_bootstrap, alpha_init, freeze_alpha, max_workers_=1):
    PATH_X_MATRIXs = path_X
    PATH_RESULT = path_result
    PATH_M_MATRIX = path_M
    NbBootstraps = nb_bootstrap
    alphaInit = alpha_init
    freezeAlpha = freeze_alpha
    files_to_analyze = get_files(PATH_X_MATRIXs)
    max_workers = min(max_workers_, len(files_to_analyze))

    func = partial(analyse_file, PATH_RESULT=PATH_RESULT, PATH_M_MATRIX=PATH_M_MATRIX, NbBootstraps=NbBootstraps, alphaInit=alphaInit, freezeAlpha=freezeAlpha)
