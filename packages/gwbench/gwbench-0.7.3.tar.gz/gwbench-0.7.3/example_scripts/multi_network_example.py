...
mulnet = multi_network.MultiNetwork(network_specs, logger_level='INFO', logger_level_network='WARNING')
mulnet.set_wf_vars(wf_model_name, wf_other_var_dic=wf_other_var_dic, user_waveform=user_waveform)
mulnet.set_net_vars(f=f, inj_params=inj_params, deriv_symbs_string=deriv_symbs_string,
...
mulnet.setup_ant_pat_lpf_psds()

# start the actual analysis
mulnet.calc_det_responses_derivs_num(step=step, method=method, order=order, num_cores=num_cores)
#-----distribute calculated detector reponse variables amongst all networks-----
#-----this is an additional step compared the to (single) Network analysis------
mulnet.dist_det_responses()
#-------------------------------------------------------------------------------
mulnet.calc_snrs(only_net=only_net)
mulnet.calc_errors(only_net=only_net)
mulnet.calc_sky_area_90(only_net=only_net)
