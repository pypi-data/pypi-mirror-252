...
def _resolve_udf_server_url(client_id: Optional[str] = None) -> str:
    if client_id == "_local":
        ...
        if OPTIONS.realtime_client_id is None:
            api = FusedAPI()
            instances = api._list_realtime_instances()
            if len(instances):
                OPTIONS.realtime_client_id = instances[0]["client_id"]

        if OPTIONS.realtime_client_id is None:
            raise ValueError("Failed to detect realtime client ID")
        return f"{OPTIONS.base_url}/realtime/{OPTIONS.realtime_client_id}"
