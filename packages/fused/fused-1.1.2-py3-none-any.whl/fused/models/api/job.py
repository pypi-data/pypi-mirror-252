...
def _common_validate_for_run(
    *,
    ...
    if validate_imports and udf is not None:
        validate_imports_whitelist(udf)
...
class UdfJobStepConfig(JobStepConfig, type="udf"):
    ...
    def run_file(
        ...
        **kwargs,
        ...
        from fused.quick.udf import run as _run_realtime
    ...
    def run_tile(
        ...
        *,
        ...
        from fused.quick.udf import run_tile as _run_realtime_tile
...
class JobConfig(FusedBaseModel):
    ...
    def _generate_code(self, headerfile=False):
        ...
        str_job = "\n".join(jobs)
        if len(job_names) == 1:
            str_multijob = f"job = fused.job({job_names[0]})"
        else:
            str_multijob = f"job = fused.job([{', '.join(job_names)}])"
        # String: Job execution
        str_job_exec = "job.run_local()"
        # Structure cell
        src = f"""
...
def _export(job, path, how, overwrite=False):
    # Validate `how`
    if how not in ("gist", "s3", "local", "zip"):
