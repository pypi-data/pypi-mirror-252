...
def get_udfs(
    ...
    *,
    ...
    api = get_api()
    return api.get_udfs(
        n=n, skip=skip, per_request=per_request, max_requests=max_requests, by=by
    )
...
def list(path: str) -> List[str]:
    ...
    Args:
        path: Parent directory URL, like `fd://folder-name/`
...
def get(path: str) -> bytes:
    ...
    Args:
        path: URL to a file, like `fd://folder-name/file.parquet`
...
def upload(local_path: Union[str, Path, bytes, BinaryIO], remote_path: str) -> None:
    """Upload the local contents to the cloud.
...
def sign_url_prefix(path: str) -> Dict[str, str]:
    ...
    Args:
        path: URL to a prefix, like `fd://folder-name/some_directory/`
