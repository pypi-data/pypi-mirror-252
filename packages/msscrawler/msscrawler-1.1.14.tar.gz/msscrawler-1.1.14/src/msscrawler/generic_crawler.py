...
class GenericCrawler(BaseCrawler, SpiderCacheMixin):
    ...
    def receive_message_callback(self, ch, method, properties, body):
        ...
        try:
            ...
            if record:
                with self.database_client.connection.start_session(
                    ...
                    session.with_transaction(
                        lambda s: self.update_one_data_callback(
                            s,
                            query_obj,
                            self.build_update_object(body_message),
                        ),
                ...
                #     #     ) as session:
                #     #         session.with_transaction(
                #     #             lambda s: self.insert_one_data_callback(
                #     #                 s, self.build_insert_object(body_message)
                #     #             ),
                #     #             read_concern=rc_snapshot,
                #     #             write_concern=wc_majority,
                #     #         )
    ...
    def insert_one_data_callback(self, session, insert_obj):
        self.data_coll.insert_one(
            insert_obj,
            session=session,
        )
