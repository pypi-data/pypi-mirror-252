...
from pyimportcyclefinder.auto_version.git_ref import GitRef
...
class VersionGenerator(ABC):
    ...
    def _helper_method(get_version_from: GitRef, get_branch_from: GitRef):
        ...
        if not m:
            raise ValueError(f"could not parse output from git describe --long: {describe_long_output}")
        cd = m.capturesdict()
...
class LocalBranchBuildVersionGenerator(LocalVersionGenerator):
    @classmethod
    def generate_version_string(cls):
        git_ref = GitRef(("HEAD", 'branch'))
        print(git_ref)
        return cls._helper_method(git_ref, git_ref)


class LocalTagBuildVersionGenerator(LocalVersionGenerator):
    @classmethod
    def generate_version_string(cls):
        git_ref = GitRef(("HEAD", 'tag'))
        print(git_ref)
        return cls._helper_method(git_ref, git_ref)
...
def generate_version_string_for_scenario() -> str:
    if (
        ...
        if len(Git.Tag.points_at("HEAD").strip()) > 0:
            try:
                return LocalTagBuildVersionGenerator.generate_version_string()
            except ValueError:
                ...
            except RuntimeError:
                ...
