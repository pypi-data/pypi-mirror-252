...
class Test:
    ...
    def generate(self, agent: Generator, logged: bool = False):
        ...
        attempt_iterator.set_description(self.name)
    ...
    def evaluate(self, threshold: float = 0.5,  logged: bool = False):
        """
    ...
    def run(self, agent: Generator, detection_threshold: float = 0.5, logged: bool = False):
        ...
        self.evaluate(threshold=detection_threshold, logged=logged)
        self.summarize(threshold=detection_threshold, logged=logged)


    def export(self, path=None, subset=None, threshold=0.5):
        """
...
class PairwiseTest(Test):
    ...
    def evaluate(self):
        ...
        for d in self.detectors:
            ...
            first_half = islice(attempt_iterator, 0, self.step)

            for attempt1 in first_half:
                attempt1.detector_results[d.detectorname] = d.detect(attempt1, self.attempt_results[attempt1.seq + self.step])

                if first_detector:
                    eval_outputs += attempt1.outputs
                eval_results[d.detectorname] += attempt1.detector_results[d.detectorname]
            first_detector = False
