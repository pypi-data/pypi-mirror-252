...
class BaseChat(SyncResource):
    ...
    def read_prompt(self, path: str, name: Union[str, None] = None):
        prompt = Prompt.read(path, name)
    ...
    def read_prompts(self, path: str) -> dict[str, Prompt]:
        ...
        for name, prompt in prompts.items():
            if name in self.client.prompts:
                raise ValueError(
                    f"Prompt {name} already exists. Could not add {name} from {path}."
                )
            self.client.prompts[name] = prompt
        return prompts
