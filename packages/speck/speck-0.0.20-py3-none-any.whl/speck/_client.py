...
class BasePrompt(SyncResource):
    ...
    def read_prompt(self, path: str, id: Union[str, None] = None):
        prompt = Prompt.read(path, id)
        self.client.prompts[prompt] = prompt
        return prompt

    def read_prompts(self, path: str) -> dict[str, Prompt]:
        prompts = Prompt.read_all(path, client=self.client)
        # Add all prompts to self.client.prompts
        for id, prompt in prompts.items():
            if id in self.client.prompts:
                raise ValueError(
                    f"Prompt {id} already exists. Could not add {id} from {path}."
                )
            self.client.prompts[id] = prompt
        return prompts

    def from_string(self, text: str, override: bool = False):
        prompts = Prompt.reads_all(text)
        # Add all prompts to self.client.prompts
        for id, prompt in prompts.items():
            if id in self.client.prompts and not override:
                raise ValueError(
                    f"Prompt {id} already exists. Could not add {id}."
                )
            self.client.prompts[id] = prompt
        return prompts
...
class BaseChat(SyncResource):
    ...
    ) -> (Prompt, ChatConfig, IChatClient, Tuple[str, None]):
