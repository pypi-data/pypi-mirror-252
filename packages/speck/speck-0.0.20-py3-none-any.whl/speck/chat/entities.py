...
class Prompt(str):
    ...
    def read_all(cls, path: str, client: "BaseClient" = None) -> dict[str, "Prompt"]:
        with open(path, "r") as f:
            return cls.reads_all(f.read(), client=client)

    @classmethod
    def read(cls, path: str, id: Union[str, None] = None, **kwargs) -> "Prompt":
        with open(path, "r") as f:
            return cls.reads(f.read(), id=id, **kwargs)

    @classmethod
    def reads(cls, text: str, id: Union[str, None] = None, **kwargs) -> "Prompt":
        name = id or kwargs.get("name")
        if name is not None:
            prompts = cls.reads_all(text)
            return prompts[name]
        else:
            return cls._read(text, id=name)

    @classmethod
    def reads_all(cls, text: str, client: "BaseClient" = None) -> dict[str, "Prompt"]:
        prompts = {}
        lines = []
