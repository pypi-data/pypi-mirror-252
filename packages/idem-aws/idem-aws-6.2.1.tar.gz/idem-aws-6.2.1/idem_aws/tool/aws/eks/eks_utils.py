...
def update_labels(old_labels: dict, new_labels: dict) -> Dict[str, Any]:
    ...
    labels = {}
    final_labels = {}
    temp_new_labels = {**old_labels}
    if labels_to_add:
        final_labels["addOrUpdateLabels"] = labels_to_add
        temp_new_labels = {**temp_new_labels, **labels_to_add}

    if labels_to_remove:
        final_labels["removeLabels"] = labels_to_remove
        for each_label in labels_to_remove:
            temp_new_labels.pop(each_label)
    if final_labels:
        labels["final_labels"] = final_labels
        labels["labels"] = temp_new_labels
    return labels
...
def update_taints(old_taints: list, new_taints: list) -> Dict[str, Any]:
    ...
    taints = {}
    final_taints = {}
    temp_new_taints = {**old_taints}
    if taints_to_add:
        final_taints["addOrUpdateTaints"] = taints_to_add
        taints.append(taints_to_add)
    if taints_to_remove:
        final_taints["removeTaints"] = taints_to_remove
        for each_taints in taints_to_remove:
            temp_new_taints.pop(each_taints)
    if final_taints:
        taints["final_taints"] = final_taints
        taints["taints"] = temp_new_taints
    return final_taints
...
def get_updated_node_group_config(
    ...
    if current_labels:
        final_node_group_config["labels"] = current_labels.get("labels")
        final_node_group_config["final_labels"] = current_labels.get("final_labels")
    current_taints = update_taints(old_node_group_config.get("taints"), taints)
    if current_taints:
        final_node_group_config["taints"] = current_taints.get("taints")
        final_node_group_config["final_taints"] = current_taints.get("final_taints")
