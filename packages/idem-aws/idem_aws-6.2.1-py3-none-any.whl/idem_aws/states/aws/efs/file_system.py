...
async def present(
    ...
    if before:
        ...
        # This makes sure the created file is saved into esm regardless of subsequent updates fails, or it is thrown
        # into reconciliation loop because the file system is not available yet.
    ...
    if "LifeCycleState" in after and "available" != after.get("LifeCycleState", ""):
        # The create_file_system could return while the file system’s lifecycle state is still 'creating'.
        # There seems to be no waiter on this service resource
        # Throwing this back into reconciler loop for a retry until it becomes available.
        # If a status is not changed to 'available' for the configured retries, it could still fail.
        result["comment"] += [
            f"aws.efs.file_system '{name}' is in '{after.get('LifeCycleState')}' state."
        ]
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    if not ret["result"]:
        hub.log.warning(f"Could not describe file_system {ret['comment']}")
        return {}
