...
def remove_fundus_black(img, label=None):
    # preprocess images
    #   img : origin image
    #   tar_height: height of tar image
    # return:
    ...
    return r_img, (mask * 255).astype(np.uint8), label
...
if __name__ == '__main__':
    ...
    for name in images:
        ...
        res = process_without_gb(img, img, radius_list, centre_list_w, centre_list_h)
        from mintools.easytools.image_show import plt_imshow, opencv_imshow
        # opencv_imshow('aa', res[0])
        plt_imshow([res[0][:,:,::-1]])
        aa = 0
