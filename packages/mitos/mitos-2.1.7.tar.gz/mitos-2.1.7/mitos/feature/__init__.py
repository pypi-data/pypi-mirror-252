...
def cap(start1, stop1, start2, stop2, circular, size):
    ...
    if start1 <= stop1 and start2 <= stop2:
        ...
        if start1 > stop1 and start2 <= stop2:
            return cap(start1, size - 1, start2, stop2, circular, size) + cap(0, stop1, start2, stop2, circular, size)
        elif start1 <= stop1 and start2 > stop2:
            return cap(start1, stop1, start2, size - 1, circular, size) + cap(start1, stop1, 0, stop2, circular, size)
        else:
            return cap(start1, size - 1, start2, size - 1, circular, size) + cap(0, stop1, 0, stop2, circular, size)
...
def cup(start1, stop1, start2, stop2, circular, size):
    ...
    if start1 <= stop1 and start2 <= stop2:
        ...
        if start1 > stop1 and start2 <= stop2:
            return cup(start1, size - 1, start2, stop2, circular, size) + cup(0, stop1, start2, stop2, circular, size)
        elif start1 <= stop1 and start2 > stop2:
            return cup(start1, stop1, start2, size - 1, circular, size) + cup(start1, stop1, 0, stop2, circular, size)
        else:
            return cup(start1, size - 1, start2, size - 1, circular, size) + cup(0, stop1, 0, stop2, circular, size)
    ...
    if step > 0:
        if circular:
            if start <= stop:
                ...
                return list(itertools.chain(list(range(start, length, step)), list(range(restart, stop, step))))
        else:
            if start <= stop:
                return list(range(start, stop, step))
            else:
        ...
        if circular:
            if start >= stop:
                ...
                return list(itertools.chain(list(range(start, -1, step)), list(range(restart, stop, step))))
        else:
            if start >= stop:
                return list(range(start, stop, step))
            else:
...
def genorderwriter(featurelist, acc, outfile, mode="w", start=None, signed=True):
    ...
    for feat in featurelist:
        if signed is True and feat.strand < 0:
...
#        print "merge", str( self ), str( other )
        ...
        if self.name is None or other.name.startswith(self.name) or\
                f.name.startswith("orf") and not other.name.startswith("orf"):
            f.name = other.name
            f.type = other.type
        elif other.name is None or self.name.startswith(other.name) or \
        ...
        for m in f.__dict__:
            if m not in other.__dict__:
                continue

            if f.__dict__[m] is None and other.__dict__[m] is not None:
                f.__dict__[m] = other.__dict__[m]
            elif f.__dict__[m] is not None and other.__dict__[m] is None:
                pass
            else:
                if f.__dict__[m] == other.__dict__[m]:
                    pass
...
#
#         # update of name is special
#         if other.name.startswith( self.name ) or self.name is None:
#             prop["name"] = other.name
#         elif self.name.startswith( other.name ) or other.name is None:
#             prop["name"] = self.name
#         else:
#             raise FeatureUnAddable( self, other )
#
...
#             if self.__dict__[m] is None and other.__dict__[m] is not None:
#                 prop[m] = other.__dict__[m]
#             elif self.__dict__[m] is not None and other.__dict__[m] is None:
#                 prop[m] = self.__dict__[m]
...
#
#         # print translation
#         return feature( name = prop["name"], tpe = prop["type"], start = prop["start"], \
...
# copy = prop["copy"], part = prop["part"], mito = prop["mito"] )
    ...
    def gffstr(self, acc, features, size):
        """
...
origin_of_replication
        ...
        trnstype = None
        if self.part == 0 or self.part is None:
            parts = [x for x in features if x.name == self.name and x.copy == self.copy]
            ...
            gff.append("{seqid}\t{source}\t{type}\t{start}\t{end}\t{score}\t{strand}\t{phase}\tID=gene_{name};Name={name};gene_id={name}".format(
            ...
            if trnstype is not None:
                gff.append("{seqid}\t{source}\t{type}\t{start}\t{end}\t{score}\t{strand}\t{phase}\tID=transcript_{name};Name={name};Parent=gene_{name};gene_id={name}".format(
                    seqid=acc, source=self.method, type=trnstype,
                    start=genestart + 1, end=genestop + 1, score=".", strand=self.plusminus(),
                    phase=".", name=self.outputname(anticodon=True, part=False)))
...
#            return self.name.lower()
        ...
        if copy and self.copy is not None:
            name += "_" + str(self.copy)
        if part and self.part is not None:
