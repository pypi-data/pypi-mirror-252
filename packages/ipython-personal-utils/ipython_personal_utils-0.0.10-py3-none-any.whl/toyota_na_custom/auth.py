...
class ToyotaOneAuth:
    ...
    def __init__(
        self, callback=None, initial_tokens=None, otp_url=None, otp_timeout=20, refresh_secs=300
    ):
                ...
                if "callbacks" in data:
                    for cb in data["callbacks"]:
                        if (
                            ...
                            cb["input"][0]["value"] = await self.request_otp()
                        elif (
                            cb["type"] == "PasswordCallback"
    ...
    async def request_otp(self):
        time.sleep(self._otp_timeout)
        async with aiohttp.ClientSession() as session:
            async with session.get(self._otp_url) as resp:
                if resp.status != 200:
                    raise LoginError()
                return self._extract_otp(await resp.text())
    ...
    def _extract_otp(self, otp_resp):
        # Last line contains the OTP
        last_line = otp_resp.splitlines()[-1]
        # Assert that the last line is a 6 digit number
        assert len(last_line) == 6 and last_line.isdigit()
        return last_line
