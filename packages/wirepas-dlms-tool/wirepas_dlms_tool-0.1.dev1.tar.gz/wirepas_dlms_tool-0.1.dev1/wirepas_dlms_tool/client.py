...
class Client:
    ...
    def set_nic_system_title(self, nic_system_title: bytes = None):
        ...
        self.gx_client.translator.systemTitle = nic_system_title
    ...
    def get_addresses_from_payload(self, payload: bytes):
        """ Get client and server addresses from a payload. """
        if len(payload) < 11:
            raise ValueError("DLMS payload is too short.")
    ...
    def message_to_xml(self, msg: str) -> str:
        ...
        Args:
            msg (str|bytes): message to decode in hexadecimal or in bytes.
        """
        try:
