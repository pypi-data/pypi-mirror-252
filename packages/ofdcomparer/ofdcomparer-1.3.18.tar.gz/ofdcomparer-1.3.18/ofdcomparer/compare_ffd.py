...
from ofdcomparer.utils import Utils
...
class ComparerOfd:
    ...
    def compare_etalon_fn_ofd(self, etalon: dict, changes: dict = None,
        ...
        if bool(changes):
            self.utils.change_values_in_dict(dict_needs_to_change=etalon, changes=changes)
    ...
    def compare_last_fd_in_fn_and_ofd(
        ...
        if bool(changes):
            self.utils.change_values_in_dict(dict_needs_to_change=fd, changes=changes)
        self.DTO10.wait_for_sent_all_fd()
    ...
    def __append_to_result(self, status, message, key, comparable, etalon):
        ...
        try:
            ...
            logging.error(f"{inspect.currentframe().f_code.co_name} [ERROR] {status=} {message=} {key=} {comparable} {etalon}")
