...
class v0:
    ...
    def get_swap_quote(self, params: EulithSwapRequest) -> (float, List[TxParams]):
        ...
        if not self.ew3.is_atomic():
            warn('Performing swaps outside an atomic context is not recommended; its dangerous. '
                 'Use DeFi Armor.', DeprecationWarning)

            if self.ew3.wallet_address and not params.get("from_address", None):
                params.update({"from_address": self.ew3.to_checksum_address(self.ew3.wallet_address)})
    ...
    def create_draft_client_whitelist(self, auth_address: str, addresses: List[str]) -> int:
        raise EulithDeprecationException('This method has been removed. Use create_draft_client_whitelist_v2 instead.')

    def create_draft_client_whitelist_v2(self, display_name: str, addresses: List[whitelists_v2.AddressOnChain]) -> int:
        """
        ...
        """
        request = whitelists_v2.rpc.CreateRequest(display_name=display_name, addresses=addresses)
    ...
    def append_to_draft_client_whitelist(self, auth_address: str, addresses: List[str], chain_id: Optional[int] = None) -> int:
        ...
        """
        raise EulithDeprecationException('This method has been removed. Use append_to_draft_client_whitelist_v2 instead.')

    def append_to_draft_client_whitelist_v2(self, list_id: int, addresses: List[whitelists_v2.AddressOnChain]) -> whitelists_v2.Whitelist:
        """
    ...
    def submit_draft_client_whitelist_signature(self, list_id: int, signer: Signer) -> bool:
        raise EulithDeprecationException('This method has been removed. Use submit_activate_whitelist_signature instead.')

    def get_current_client_whitelist(self, auth_address: str, chain_id: Optional[int] = None) -> CurrentWhitelists:
        raise EulithDeprecationException('This method has been removed. Use get_whitelist_by_id instead.')
    ...
    def start_activate_whitelist(self, list_id: int, auth_address: str, safe_address: str, chain_id: int) -> whitelists_v2.rpc.StartActivateResponse:
        request = whitelists_v2.rpc.StartActivateRequest(list_id=list_id, auth_address=auth_address, safe_address=safe_address, chain_id=chain_id)
        return self.ew3.eulith_service.start_activate_whitelist(request)

    def submit_activate_whitelist_signature(self, signer: Signer, activate_request: whitelists_v2.rpc.StartActivateResponse) -> whitelists_v2.rpc.SubmitActivateSignatureResponse:
        typed_data_payload = ActivateHashInputEip712(activate_request["hash_input"]).typed_data()
        signature = signer.sign_typed_data(typed_data_payload, HexBytes(activate_request["hash"]))
        serialized_signature = normalize_signature(signature)

        request = whitelists_v2.rpc.SubmitActivateSignatureRequest(activation_id=activate_request["activation_id"], signer_address=signer.address, signature=serialized_signature, hash=activate_request["hash"])
