...
class EulithService:
    ...
    def publish_client_whitelist(self, request: whitelists_v2.rpc.PublishRequest) -> whitelists_v2.rpc.PublishResponse:
        json_rpc = self.eulith_provider.make_request(RPCEndpoint("eulith_publish_whitelist"), [request])
        raise_if_error(json_rpc)
        return cast(whitelists_v2.rpc.PublishResponse, json_rpc["result"])

    def start_activate_whitelist(self, request: whitelists_v2.rpc.StartActivateRequest) -> whitelists_v2.rpc.StartActivateResponse:
        json_rpc = self.eulith_provider.make_request(RPCEndpoint("eulith_start_activate_whitelist"), [request])
        raise_if_error(json_rpc)
        return cast(whitelists_v2.rpc.StartActivateResponse, json_rpc["result"])

    def submit_activate_whitelist_signature(self, request: whitelists_v2.rpc.SubmitActivateSignatureRequest) -> whitelists_v2.rpc.SubmitActivateSignatureResponse:
        json_rpc = self.eulith_provider.make_request(RPCEndpoint("eulith_submit_activate_whitelist_signature"), [request])
        raise_if_error(json_rpc)
        return cast(whitelists_v2.rpc.SubmitActivateSignatureResponse, json_rpc["result"])

    def get_whitelist_by_id(self, request: whitelists_v2.rpc.GetByIdRequest) -> whitelists_v2.rpc.GetByIdResponse:
        json_rpc = self.eulith_provider.make_request(RPCEndpoint("eulith_get_whitelist_by_id"), [request])
        raise_if_error(json_rpc)
    ...
    def dydx_v3_get_ace_managed_accounts(self, ace_address: str) -> (List[DydxV3AceManagedAccount], dict):
        try:
            params = {
                "ace_address": ace_address,
            }

            response = self.eulith_provider.make_request(RPCEndpoint("eulith_dydx_v3_get_ace_managed_accounts"), [params])
            raise_if_error(response)
            return response["result"], {}
        except EulithRpcException as e:
            return None, RPCResponse(error=str(e))

    def dydx_v3_create_order(self, order: DyDxV3CreateOrderParams, signature: Union[Signature, str], ace_address: str):
        try:
            params = {
                "ace_address": ace_address,
                "signature": str(signature),
            ...
            response = self.eulith_provider.make_request(RPCEndpoint("eulith_dydx_v3_create_order"), [params])
    ...
    def dydx_v3_get_for_account(self, ace_address: str, dydx_account_name: str, data: str):
        try:
            params = {
                "ace_address": ace_address,
            ...
            response = self.eulith_provider.make_request(RPCEndpoint("eulith_dydx_v3_get_for_account"), [params])
    ...
    def get_gmx_v2_tickers(self):
        try:
            response = self.eulith_provider.make_request(RPCEndpoint("eulith_gmx_v2_ticker_lookup"), [])
            raise_if_error(response)
            return response['result'], {}
        except EulithRpcException as e:
            return None, RPCResponse(error=str(e))
    ...
    def get_gmx_v2_funding_rates(self):
        try:
            response = self.eulith_provider.make_request(RPCEndpoint("eulith_gmx_v2_funding_rate_lookup"), [])
            raise_if_error(response)
            return response['result'], {}
        except EulithRpcException as e:
            return None, RPCResponse(error=str(e))
