...
async def _do_start_box(
    ...
    process = await _start_kyzylborda_box_process(token, box_env, [], environ, [])
...
async def start_oneshot(
    ...
    process = await _start_kyzylborda_box_process(token, box_env, argv, environ, volumes)
    return OneShot(token, scope, process, box_env)
