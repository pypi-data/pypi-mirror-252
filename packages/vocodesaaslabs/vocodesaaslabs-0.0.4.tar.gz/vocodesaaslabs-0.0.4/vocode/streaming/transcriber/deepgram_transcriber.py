...
from new_event_manager import event_manager_new
...
class DeepgramTranscriber(BaseAsyncTranscriber[DeepgramTranscriberConfig]):
    def __init__(
        ...
        self.event_manager_new = event_manager_new #ajeya
    ...
    def get_deepgram_url(self):
        ...
        # print(f"wss://api.deepgram.com/v1/listen?{urlencode(url_params)}") #Ajeya
        # return f"wss://api.deepgram.com/v1/listen?{urlencode(url_params)}"
        return "wss://api.deepgram.com/v1/listen?encoding=mulaw&sample_rate=8000&language=en-IN&model=nova-2-phonecall&smart_format=true&filler_words=true&interim_results=true"
            ...
            async def sender(ws: WebSocketClientProtocol):  # sends audio to websocket
                while not self._ended:
                    try:
                        ...
                        break
                    await ws.send(data)
                self.event_manager_new.add_event({
                    "event_type":"complete",
                    "data":{
                        "transcript":self.complete_transcription
                    }
                }) #Ajeya
            ...
            async def receiver(ws: WebSocketClientProtocol):
                ...
                while not self._ended:
                    ...
                    if speech_final:
                        ...
                        self.complete_transcription += buffer + " " #Ajeya
                        self.event_manager_new.add_event({
                            "event_type":"interim",
                            "data":{
                                "transcript":buffer
