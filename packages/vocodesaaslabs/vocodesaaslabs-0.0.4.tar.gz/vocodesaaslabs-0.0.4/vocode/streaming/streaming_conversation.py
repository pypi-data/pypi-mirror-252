...
class StreamingConversation(Generic[OutputDeviceType]):
    ...
    class SynthesisResultsWorker(InterruptibleWorker):
        ...
        def __init__(
            ...
            self.input_queue = input_queue
            self.conversation = conversation
            self.event_manager_new = event_manager_new
        ...
        async def process(
            ...
            try:
                ...
                self.conversation.logger.debug("Message sent: {}".format(message_sent)) #Ajeya
                self.event_manager_new.add_event({
                    "event_type":"bot_data",
                    "data":{
                        "transcript":message_sent
                    }
                }) 
                # event_manager_new.add_event({"Bot Data":message_sent})
                if cut_off:
