...
def enrichmentscoreBETA(dfpfcclus, df_dev, fc=3, pvalcutoff=0.1, shortcut=True):
    ...
    This function calculates an enrichment score for a given set of genes and clusters. It takes in the following inputs:

    dfpfcclus: a Pandas DataFrame object that represents gene expression data
    df_dev: a Pandas DataFrame object that represents developmental data
    fc: a float value that is used as a fold change cutoff
    pvalcutoff: a float value that is used as a p-value cutoff
    shortcut: a boolean value that is used to determine whether to use a shortcut or not
    The function first joins the two input data frames based on cluster labels, then calculates the mean and median expression levels for each cluster. It then calculates the enrichment score for each gene and cluster using a formula that involves the fold change, mean expression level, and number of non-zero counts. The function then uses these scores to select marker genes for each cluster. The selected marker genes are those that meet certain criteria, such as having a higher fold change and enrichment score compared to other clusters.

    If shortcut is set to True, the function uses a simplified version of the algorithm that does not calculate the enrichment scores. Instead, it directly compares the mean expression levels and fold changes of each gene across different clusters to select marker genes.

    The function returns a list of selected marker genes.
    """
    # enrichmentscoreBETA that calculates an enrichment score for a given set of genes and clusters.
    #dfpfcclus: a Pandas DataFrame object that represents gene expression data
    #df_dev: a Pandas DataFrame object that represents developmental data
    #fc: a float value that is used as a fold change cutoff
    #shortcut: a boolean value that is used to determine whether to use a shortcut or not
...
def LabelGene_Scaling(datax, commongene, mprotogruop, tftable, thrs=None, score=None,
