...
def refresh(ip_address=None, record_name=None, token=None):
    """
    刷新给定 IP 地址和记录名称的 DNS 记录。

    参数:
        ip_address (str, 可选): 要刷新的 IP 地址。默认为 None。
        record_name (str, 可选): 要刷新的 DNS 记录名称。默认为 None。
        token (str, 可选): 用于访问 DNS 服务的认证令牌。默认为 None。

    返回:
        bool: 如果 DNS 记录成功刷新，返回 True，否则返回 False。
    """
    # 接收参数
    args = sys.argv[1:]
    # print(f'args: {args}')
    ...
    try:
        ...
        if not host:
            raise ValueError('No valid host')
        ...
        dns_records = cf.zones.dns_records.get(
            zone_id, params={'name': dns_record_name, 'type': dns_record_type}
        )
        ...
        if dns_records:
            ...
            print(
                f'DNS record for {dns_record_name} ({
                    dns_record_type}) IP({ip_address}) updated successfully'
            )
        else:
            # 如果记录不存在，则添加一个新记录
            cf.zones.dns_records.post(zone_id, data=dns_record_data)
            print(
                f'New DNS record for {dns_record_name} ({
                    dns_record_type}) IP({ip_address}) added successfully'
            )
        return True
