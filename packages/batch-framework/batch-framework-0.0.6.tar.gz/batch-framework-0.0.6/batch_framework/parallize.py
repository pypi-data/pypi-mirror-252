...
class MapReduce(ETLGroup):
    ...
    def __init__(self, map: ObjProcessor, parallel_count: int,
        ...
        class MapClass(ObjProcessor):
            ...
            def __repr__(self):
                return super().__repr__().replace('ObjProcessor', 'MapClass') + '-' + map_name
...
class AddPartitionKey(SQLExecutor):
    ...
    def __repr__(self):
        return super().__repr__().replace(
            'SQLExecutor', 'AddPartitionKey') + '-' + self._map_name
...
class EfficientDivide(ObjProcessor):
    ...
    def __repr__(self):
        return super().__repr__().replace('ObjProcessor',
                                          'EfficientDivide') + '-' + self._map_name
    ...
    def transform(self, inputs: List[vx.DataFrame],
        ...
        for _, df in table.groupby('partition'):
            assert len(
                df) > 0, 'output dataframes of EfficientDivide should not be 0 size.'
