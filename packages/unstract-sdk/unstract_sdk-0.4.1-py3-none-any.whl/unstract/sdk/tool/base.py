...
class AbstractTool(ABC, StreamableBaseTool):
    ...
    def __init__(self, log_level: LogLevel = LogLevel.INFO) -> None:
        ...
        Args:
            ...
            workflow_id (str): Workflow ID that the tool is run from
        """
        self.start_time = datetime.datetime.now()
        self.workflow_id = ""
        self.execution_id = ""
        super().__init__(log_level=log_level)
        # TODO: Avoid TOOL_DATA_DIR requirement for static commands
        self._exec_metadata = self._get_exec_metadata()
        self.workflow_id = self._exec_metadata.get(MetadataKey.WORKFLOW_ID)
        self.execution_id = self._exec_metadata.get(MetadataKey.EXECUTION_ID)
        self.org_id = self._exec_metadata.get(MetadataKey.ORG_ID)
    ...
    def from_tool_args(cls, args: list[str]) -> "AbstractTool":
        ...
        return cls(log_level=parsed_args.log_level)
