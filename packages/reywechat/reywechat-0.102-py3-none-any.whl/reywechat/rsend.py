...
from typing import Any, List, Dict, Literal, Callable, Union, Optional, NoReturn, overload
...
class RSend(object):
    ...
    def auto_at(
        self,
        params: Dict[Literal["send_type", "receive_id", "text"], Any]
    ) -> Dict[Literal["send_type", "receive_id", "text", "user_id"], Any]:
        """
        Based on the user name in the text, automatic add `@`.
        """

        # Get parameter.
        member_dict = self.rwechat.rclient.get_room_member_dict(params["receive_id"])
        ...
        # Break.
        if member_dict_exist == {}:
            return params

        # Add.
        pattern = "|".join(member_dict_exist.values())
        replace = lambda match: "@%s " % match[0]
        text_at = sub(pattern, text, replace)
        user_id = ",".join(member_dict_exist)
