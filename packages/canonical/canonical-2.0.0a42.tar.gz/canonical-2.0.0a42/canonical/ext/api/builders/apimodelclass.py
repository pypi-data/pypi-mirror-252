...
class APIModelClassBuilder(IBuilder[T], Generic[T]):
    ...
    def build_input_model(self):
        if self.building == 'domain' and self.inspector.is_concrete(self.model):
            self.model.__create_model__ = self.subclass(self.model, 'create', {})
    ...
    def get_subclass_attrs(self, model: type[pydantic.BaseModel], mode: str) -> dict[str, Any]:
        return {
    ...
    def subclass(self, model: type[pydantic.BaseModel], mode: str, attrs: dict[str, Any]) -> type[T]:
        """Subclass the configured model for the given presentation
        `mode`.
        """
        from ..apimodel import APIModel
        new_class = type(model.__name__, self.get_presentation_bases(), {
            **attrs,
