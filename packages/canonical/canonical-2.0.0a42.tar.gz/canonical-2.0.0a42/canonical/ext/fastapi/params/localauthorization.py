...
class LocalAuthorizationContext(BaseAuthorizationContext):
    ...
    async def get_cluster_bindings(self):
        cache_key = f'clusterrolebindings:users:{self.email}'
    ...
    async def get_namespace_bindings(self):
        ...
        bindings = await self.cache.get(cache_key, list[RoleBinding], keyalg='sha256')
        if bindings is None:
            self.logger.debug(
    ...
    async def setup_cluster(self):
        if self._cluster_roles != NotImplemented:
            return
        roles: list[ClusterRole] | None = await self.cache.get('clusterroles', list[ClusterRole], keyalg='sha256')
        if roles is None:
            roles = []
            self.logger.debug("Retrieving cluster roles and permissions")
            async for role in self.repo.all(ClusterRole):
                self.logger.debug("Registering cluster role (name: %s)", role.metadata.name)
                roles.append(role)
            await self.cache.set('clusterroles', roles, list[ClusterRole], keyalg='sha256', ttl=self.cache_ttl)
        self._cluster_roles = {x.metadata.name: x for x in roles}