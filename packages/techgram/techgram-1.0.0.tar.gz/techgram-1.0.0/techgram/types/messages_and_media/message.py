...
import techgram
from techgram import raw, enums
from techgram import types
from techgram import utils
from techgram.errors import MessageIdsEmpty, PeerIdInvalid
from techgram.parser import utils as parser_utils, Parser
...
class Message(Object, Update):
    ...
    async def _parse(
        ...
        if isinstance(message, raw.types.MessageService):
            ...
            if isinstance(action, raw.types.MessageActionChatAddUser):
                new_chat_members = [types.User._parse(
                    client, users[i]) for i in action.users]
            ...
            sender_chat = types.Chat._parse(
                client, message, users, chats, is_chat=False) if not from_user else None

            parsed_message = Message(
                id=message.id,
                date=utils.timestamp_to_datetime(message.date),
                chat=types.Chat._parse(
                ...
                migrate_to_chat_id=utils.get_channel_id(
                    migrate_to_chat_id) if migrate_to_chat_id else None,
            ...
            client.message_cache[(parsed_message.chat.id,
                                  parsed_message.id)] = parsed_message

            return parsed_message

        if isinstance(message, raw.types.Message):
            entities = [types.MessageEntity._parse(
                client, entity, users) for entity in message.entities]
            ...
            if reply_markup:
                if isinstance(reply_markup, raw.types.ReplyKeyboardForceReply):
                    ...
                    reply_markup = types.InlineKeyboardMarkup.read(
                        reply_markup)
            ...
            parsed_message = Message(
                ...
                via_bot=types.User._parse(
                    client, users.get(message.via_bot_id, None)),
    ...
    async def ask(
        ...
        """Bound method *ask* of :obj:`~techgram.types.Message`.
        ...
        Parameters:
            ...
            quote (``bool``, *optional*):
                ...
                If *reply_to_message_id* is passed, this parameter will be ignored.
                Defaults to ``True`` in group chats and ``False`` in private chats.
            parse_mode (:obj:`~techgram.enums.ParseMode`, *optional*):
                By default, texts are parsed using both Markdown and HTML styles.
                You can combine both syntaxes together.
            entities (List of :obj:`~techgram.types.MessageEntity`):
                List of special entities that appear in message text, which can be specified instead of *parse_mode*.
            disable_web_page_preview (``bool``, *optional*):
                Disables link previews for links in this message.
            disable_notification (``bool``, *optional*):
            ...
            reply_to_message_id (``int``, *optional*):
                If the message is a reply, ID of the original message.
            reply_markup (:obj:`~techgram.types.InlineKeyboardMarkup` | :obj:`~techgram.types.ReplyKeyboardMarkup` | :obj:`~techgram.types.ReplyKeyboardRemove` | :obj:`~techgram.types.ForceReply`, *optional*):
                Additional interface options. An object for an inline keyboard, custom reply keyboard,
                instructions to remove reply keyboard or to force a reply from the user.
            filters (:obj:`Filters`):
