...
from re import findall as re_findall, sub as re_sub, S as RS
...
def search_market(keyword: str) -> List[Dict[Literal["code", "name", "type", "url"], str]]:
    ...
    labels_result: Tuple[str, str] = re_findall(pattern, response.text, RS)
    table = []
    for index, (label_class, div_text) in enumerate(labels_result):
        ...
        stocks_result = re_findall(pattern, div_text)
        for stock_url, stock_text in stocks_result:
            pattern = "<.+?>"
            stock_info = re_sub(pattern, "", stock_text)
