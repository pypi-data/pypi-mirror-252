...
def init_parser():
    ...
    # parser.set_defaults(function='f5c', chrom="NR_103073.1", pos=875, len=10, strand='-', cpu=8,norm=True,base_shift=2,pore='r9',
    ...
    # parser.set_defaults(function='f5c', chrom="NR_103073.1", pos=2030, len=10, strand='+', cpu=8,norm=True,base_shift=2,pore='r9',
...
if __name__ == '__main__':
    ...
    # draw PCA
    feature_matrix, label = extract_kmer_feature(df_copy, 3, args.pos + 1)
    plot_PCA(feature_matrix, label, results_path)

    # draw p_value
    new_df = calculate_MANOVA_result( args.pos+1,df_copy,500,args.len)
    MANOVA_plot(new_df, results_path)
    print('Finished')
