...
def db_download(environment):
    ...
    try:
        with tempfile.NamedTemporaryFile() as tmp_password_file:
            ...
            transfer.put(tmp_password_file, db_password_filename)

        click.echo(f"Dumping database for {username}")
        connection.run(
            f"PGPASSWORD=$(cat {db_password_filename}) pg_dump --host container-db01 --port 5432 {username} --username {username} --clean --no-owner --no-privileges  > {db_dump_filename}"  # noqa: E501
        )
    finally:
        connection.run(f"rm {db_password_filename}")
