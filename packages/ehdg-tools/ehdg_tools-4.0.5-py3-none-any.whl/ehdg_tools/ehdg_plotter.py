    ...
    if len(rows2) > 0:
        ...
        if gaze_csv_dir:
            ...
            if trial_name is None:
                trial_name = os.path.basename(output_dir)
            trial_array = str(trial_name).split("_", 1)
            trial_id = trial_array
...
def get_plot_info(data_dir, plot_info_input, summary_csv_dir_input=None):
    ...
    folder_array = get_folder_name_from_dir(data_dir, "trial_id", "disk_condition", summary_csv_dir_input)
...
def get_folder_name_from_dir(dir_input, trial_id_header_input, disk_condition_header_input,
    ...
    csv_dir = summary_csv_name_input
    if not os.path.isfile(csv_dir):
        print("Invalid summary csv file location.")
    output_array = []
    ...
    if len(rows) > 0:
        for row in rows:
            ...
            trial_folder_dir = os.path.join(dir_input, folder_name_input)
            if not os.path.isdir(trial_folder_dir):
                print(f"Trial folder directory: {trial_folder_dir} is missing")
                return
