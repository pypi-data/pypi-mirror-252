...
LEADERBOARD_ENDPOINT = "/leaderboard?start_date={start_date}&end_date={end_date}"
...
def get_submissions(developer_key=None, date_range:Optional[Tuple[str, str]]=None):
    headers = {"developer_key": developer_key}
    start_date = date_range[0] if date_range[0] else ''
    end_date = date_range[1] if date_range[1] else ''
    url = get_url(LEADERBOARD_ENDPOINT, start_date=start_date, end_date=end_date)
    print(url)
...
def _get_cache_file_path(func, args, kwargs):
    ...
    fname = 'cache-' + str(hash(signature))
