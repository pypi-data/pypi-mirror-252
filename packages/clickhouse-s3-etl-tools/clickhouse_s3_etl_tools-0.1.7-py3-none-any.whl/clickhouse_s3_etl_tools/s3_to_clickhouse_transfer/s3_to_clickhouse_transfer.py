...
def check_on_cluster(metadata: MetadataConfig, config: Configuration):
    if (
        ...
        raise OnClusterClickhouseError(
            ...
            cluster=config.ON_CLUSTER_DIRECTIVE,
            message="Your table is replicated. Specify 'on-cluster-directive'",
        )
...
def create_table_if_not_exists(
    ...
    if not conn.check_if_table_exists(
        ...
        create_table_query = update_create_table_query(
            metadata.create_table_query, config
        )
...
def insert_to_ch_whole_table(config: Configuration, conn: ClickHouseConnector) -> None:
    """
    ...
    try:
        ...
        return insert_to_ch_whole_table(config, conn)
...
def transfer_s3_to_clickhouse(config: Configuration):
    ...
    with (
        ...
        if metadata.engine.lower() in ["join", "view", "merge", "null", "materializedview"]:
            logger.info(f"Engine {metadata.engine} doesn't require to be transfered to clickhouse")
        else:
        ...
        check_rows_mismatch(s3_row_count=metadata.total_rows,
