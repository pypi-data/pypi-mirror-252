...
class ClickHouseConnector:
    ...
    def execute(self, sql):
        ...
        try:
            ...
            if 'columns declaration list' in e.message and e.code == 62:  # bug of clickhouse for s3 cluster https://github.com/ClickHouse/ClickHouse/issues/54615
                raise ClickHouseColumnsDeclarationErrorS3(url=self.db_url,
                                                          message="Old bug in clickhouse have to change S3Cluster to S3")
            else:
                raise clickhouse_driver.errors.ServerException from e
    ...
    def get_column_list(self, database: str, table_name: str) -> str:
        ...
        columns_list = ",\n".join(
            [
                f'toString("{k}") as {k}'
                if v == "UUID" or "AggregateFunction" in v
                else f'"{k}"'
