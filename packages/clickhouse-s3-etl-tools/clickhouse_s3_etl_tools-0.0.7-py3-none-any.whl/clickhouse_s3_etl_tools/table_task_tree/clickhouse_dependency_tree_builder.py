...
def process_dependencies(
    ...
    if (
        ...
        handle_materialized_view_dependency(table_config, table_node, trigger_with_parent)

    if table_config["EXTRACTED_TABLE_NAME_FROM"] != "" and table_config["ENGINE"] in ("View", "MaterializedView"):
        handle_view_dependency(table_config, table_node, parents_by_id)
...
def validate_tables_and_dependencies(parents_by_id: Dict[str, str], tables: List[Dict[str, Any]]) -> None:
    """
    ...
    """
    table_names_databases = [get_key(table_name=table["TABLE_NAME"], database=table["DATABASE"]) for table in tables]
...
def update_dependency_dict(dict_for_update: Dict[str, str], main_dict: Dict[str, str]) -> Dict[str, str]:
    """
    ...
    for node, parent in main_dict.items():
        if node in main_dict_copy:
            ...
            if parent in dict_for_update_reverse and dict_for_update_reverse[parent] != node:
...
def get_dict_dependencies(url: str, databases: List[str]) -> Tuple[Dict[str, str], List[Dict[str, Any]]]:
    """
    ...
    for row in result:
        ...
        table_node = get_key(table_name=table_config["TABLE_NAME"], database=table_config["DATABASE"])
        process_dependencies(table_config, table_node, parents_by_id, trigger_with_parent)
