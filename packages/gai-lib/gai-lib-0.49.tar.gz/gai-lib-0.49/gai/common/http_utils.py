    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if response.status == 200:
                ...
                raise Exception(f"httppost_async: Error={response.text()} data={data}")
...
def httppost(url, data=None, files=None):
    ...
    try:
        if files:
            ...
            if "stream" in data:
                response = requests.post(url, json=data, headers=headers, stream=data["stream"])
            else:
                response = requests.post(url, json=data, headers=headers)
        if response.status_code == 200:
            return response
        else:
            raise Exception(f"httppost: Error={response.text} url={url}")
