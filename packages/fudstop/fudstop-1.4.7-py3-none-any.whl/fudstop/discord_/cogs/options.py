...
class OptionsCOG(commands.Cog):
    def __init__(self, bot):
        self.bot=bot
        self.poly = PolygonOptions(user='chuck', database='markets', host='localhost', port=5432, password='fud')
        self.wt = WebullTrading()
        self.wb = WebullOptions(user='chuck', database='markets', host='localhost', password='fud', port=5432)
    ...
    @options.sub_command()
    async def all_trades(self, inter:disnake.AppCmdInter, ticker:str=commands.Param(autocomplete=ticker_autocomp)):
        """View all option trades for an underlying ticker - ordered by volume"""
        await self.poly.connect()
        await inter.response.defer()

        query = f"""SELECT underlying_symbol, strike, call_put, expiry,size, price FROM optiontrade WHERE underlying_symbol = '{ticker}' and expiry > '2023-10-04' ORDER BY size DESC LIMIT 5000"""

        results = await self.poly.fetch(query)



        df = pd.DataFrame(results, columns=['sym', 'strike', 'call/put', 'exp', 'size', 'price'])
        ...
        for chunk in chunks:
            ...
            embed.add_field(name=f"Viewing all option trades for:", value=f"> **{ticker}**")
            embeds.append(embed)
