...
import asyncio
from fudstop.apis.polygonio.mapping import STOCK_EXCHANGES, stock_condition_dict
...
class StockMarket:
    def __init__(self):
        ...
        self.database = PolygonDatabase(user='chuck', database='markets', host='localhost', password='fud', port=5432)
        self.poly = Polygon(user='chuck', host='localhost', port=5432, database='markets', password='fud')
    ...
    async def handle_msg(self, msgs: WebSocketMessage, data_queue=asyncio.Queue):
        for m in msgs:
            ...
            if isinstance(m, EquityAgg):
                ...
                data['volume_indicator'] = "High" if data['trade_size'] > 1000 else "Medium" if data['trade_size'] > 100 else "Low"
    ...
    while True:
        ...
        if data.get('type') == 'EquityTrade':
            ...
            if webhook_url is not None and dollar_cost >= 150000 and 'FINRA' not in exchange:
                ...
                embed = DiscordEmbed(title=f"Sector Feed - {sector} | {ticker}", description=f"# > Size: **{size}** | Price: **${price}**\n# > Dollar Cost: **${round(float(dollar_cost),2)}**", color=market.colors.get('blue'))
            ...
            if webhook_url is not None and dollar_cost >= 1000000 and 'FINRA' in exchange:
                logo = await market.get_logo(ticker)
                hook = AsyncDiscordWebhook(webhook_url, content=f"<@375862240601047070>")
                embed = DiscordEmbed(title=f"SECTOR DARK POOLS - {sector} | {ticker}", description=f"# > Size: **{size}** | Price: **${price}**\n# > Dollar Cost: **${round(float(dollar_cost),2)}**", color=market.colors.get('gold'))
                embed.add_embed_field(name=f"Conditions:", value=f"> **{conditions}**")
                embed.add_embed_field(name=f"Exchange:", value=f"> **{exchange}**")
                embed.add_embed_field(name=f"Volume Indicator:", value=f"> **{volume_indicator}**")
                embed.set_thumbnail(logo)
                ...
                await market.database.batch_insert_dataframe(df, 'live_dps', unique_columns='insertion_timestamp')       
