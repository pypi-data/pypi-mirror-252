...
class LayerNextClient:
    ...
    def auto_tag_collection(
        ...
        res_obj = self.create_or_update_job(
            session_id, job_name, JobType.GENERATE_AUTO_TAGGING.value, 0, JobStatus.QUEUED.value)
    ...
    def auto_annotate_project(
        ...
        res_obj = self.create_or_update_job(
            session_id, job_name, JobType.GENERATE_AUTO_ANNOTATION.value, 0, JobStatus.QUEUED.value)
    ...
    def generate_embeddings_for_collection(
        ...
        res_obj = self.create_or_update_job(
            session_id, job_name, JobType.GENERATE_EMBEDDING.value, 0, JobStatus.QUEUED.value)
        ...
        if ("success" in res_obj and res_obj["success"] == True) and res_obj["jobId"] is not None:
            ...
            raise Exception("Error occurred while create job")
        print(
            f"Embedding generation started for collection : {collection_id}, refer job: {job_id}")
        ...
        for embedding in embedding_list:
            ...
            if count == len(embedding_list)-1:
                ...
                if ('uniqueName' in embedding and isinstance(embedding['uniqueName'], str)):
                    ...
                    self.create_or_update_job(session_uuid, job_name, JobType.UPLOAD_EMBEDDING.value, 0, JobStatus.FAILED.value, {
                ...
                if response["success"] == False:
                    self.create_or_update_job(session_uuid, job_name, JobType.UPLOAD_EMBEDDING.value, 0, JobStatus.FAILED.value, {
                                              "embeddingUploadFailedObjectKeys": object_key_list, "errorList": "Error occur while uploading embeddings"})
                    object_key_list = []
                    return response
                ...
                self.create_or_update_job(session_uuid, job_name, JobType.UPLOAD_EMBEDDING.value, progress, status, {
                                          "embeddingUploadTryObjectKeys": object_key_list, "embeddingUploadSuccessCount": inserted_count})
                object_key_list = []
                return {
                    "success": True,
