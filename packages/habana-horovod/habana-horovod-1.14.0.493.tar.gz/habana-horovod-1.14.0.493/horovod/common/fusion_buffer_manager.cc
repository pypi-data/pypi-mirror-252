...
void FusionBufferManager::FreeDeviceBuffers(int device) {
  for (auto& tensor_fusion_buffer : tensor_fusion_buffers_) {
    std::tuple<int, Framework, int> key = tensor_fusion_buffer.first;
    std::pair<std::shared_ptr<PersistentBuffer>, int64_t>& value =
        tensor_fusion_buffer.second;

    auto& current_device = std::get<0>(key);
    auto& buffer_ptr = value.first;
