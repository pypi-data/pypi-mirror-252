...
def parse_args():
    ...
    parser.add_argument(
        "-i", "--inputMD", type=str, help="Entrer le répertoire du fichier Markdown"
    )
    parser.add_argument(
        "-style", "--liencss", type=str, help="Entrer le répertoire du fichier css"
    )
    parser.add_argument(
        "-a",
        "--anonimize",
...
def main():
    ...
    if args.toAll:
        ...
        cv.to_html_file(get_output_name(args.inputMD, anonimized=False), args.liencss)
        cv.to_pdf_file(get_output_name(args.inputMD, anonimized=False), args.liencss)
    if args.anonimize:
        if args.toHtml:
            cv_anonimized.to_html_file(
                get_output_name(args.inputMD, anonimized=True), args.liencss
        ...
        if args.toPdf:
            ...
            # print("Fichier PDF anonyme enregistré avec succès dans", get_output_name(args.inputMD,anonimized=True))
    else:
        if args.toHtml:
            cv.to_html_file(
