...
class CV:
    ...
    def to_html_content(self, css_lien, for_pdf: bool):
        ...
        with open(self.html_template, "r", encoding="utf-8") as html_blanche:
            ...
            if for_pdf == True:
                html_content = html_content.replace("LOGO_TETE", self.logo_tete_pdf)
                html_content = html_content.replace("LOGO_PIED", self.logo_pied_pdf)
            else:
                html_content = html_content.replace("LOGO_TETE", self.logo_tete_html)
                html_content = html_content.replace("LOGO_PIED", self.logo_pied_html)
        return html_content
    ...
    def to_pdf_file(self, output_pdf_file, css_lien):
        ...
        html = HTML(string=self.to_html_content(css_lien, for_pdf=True), base_url="")
