...
def identify_document_type(text):
    ...
    if re.search(iraq_id_pattern, text, re.IGNORECASE):
        return "INC"
...
def check_logo_existence_iraq(logo_content, id_card_content, threshold=3):
...
def detect_logo(client, input_image_content, country, compare_type=None, side=None):
    ...
    if country == 'IRQ' and compare_type != 'template':
        reference_logo_path = pkg_resources.resource_filename('idvpackage', 'iraq_id_logo.png')
        with open(reference_logo_path, 'rb') as logo_file:
            reference_image_content = logo_file.read()
        result = check_logo_existence_iraq(reference_image_content, input_image_content, 3)
    ...
    if country == 'IRQ' and compare_type == 'template':
        if side == 'front':
            threshold = 40
            reference_logo_path = pkg_resources.resource_filename('idvpackage', 'iraq_standard_template.png')
        if side == 'back':
            threshold = 140
            reference_logo_path = pkg_resources.resource_filename('idvpackage', 'iraq_back_standard_template.jpg')
...
def data_comparison_check(data, country):
    ...
    if user_data:
        if not compare_dates(standardize_date(data.get('dob', '')), standardize_date(user_data.get('dob', ''))):
        ...
        if not fuzzy_match_fields(first_name, user_data.get('first_name', '').lower()):
            data_comparison['breakdown']['first_name']['result'] = 'consider'

        if not fuzzy_match_fields(last_name, user_data.get('last_name', '').lower()):
...
def data_consistency_check(data, front_id_text, back_id_text, country):
    ...
    if country == 'IRQ':
        ...
        doc_type1 = identify_document_type(translated_front_id_text)
        doc_type2 = identify_document_type(translated_back_id_text)
...
def data_validation_check(data, country):
    ...
    if country == 'UAE' or country == 'IRQ':
        ...
        doe = data.get('expiry_date')
        expiry_result = is_valid_and_not_expired(get_dates_to_generic_format(doe))
...
def image_integrity_check(data, front_id_text, back_id_text, coloured, blurred, glare, missing_fields, country):
    ...
    image_integrity['breakdown']['conclusive_document_quality']['properties']['corner_removed'] = is_valid_and_not_expired(get_dates_to_generic_format(data.get('expiry_date')))
...
def main_details(data, country):
    ...
    try:
        ...
        keys = ['name', 'first_name', 'gender', 'issuing_place', 'last_name', 'nationality', 'issuing_country']
