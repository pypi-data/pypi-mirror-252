...
def outputdepth(pdbfile):
    parser = PDBParser(QUIET=True)
    structure = parser.get_structure(pdbfile[:-4], pdbfile)
    depths, residues = computedepth(structure)
    with open(structure.id+'-residue.depth', 'w') as f:
        for key in depths.keys():
            f.write(f'{key}\t{residues[key]}\t{depths[key]}\n')    

def computedepth(biopdbstruct, scale=0.5, proberadius=2.5, margin=2.5):
    import time
...
def listpdbatoms(biopdbstruct):
    ...
    for chain in biopdbstruct[0]:   # get only first model in structure
        for res in chain:
            for atom in res:
                if not atomisH(atom):
                    if (res.id[0][0:2] != 'H_') and (res.id[0][0:1] != 'W'):     # if not heteroatom
...
def edt_scipy(image, scale):
    ...
    depths = distance_transform_edt(image)
    depths[bg == -1] = -1    
    depths /= scale
    return depths
