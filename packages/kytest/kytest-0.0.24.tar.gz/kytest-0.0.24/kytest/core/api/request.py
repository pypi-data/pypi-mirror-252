...
class HttpReq(object):
    ...
    @staticmethod
    def assertEq(path: typing.Union[str, list], value):
        """
        等于
        """
        ...
        for _path in path:
            logger.info(f'查找"{_path}"')
            search_value = jmespath.search(_path, ResponseResult.response)
    ...
    @staticmethod
    def assertNotEq(path: typing.Union[str, list], value):
        """
        不等于
        """
        if isinstance(path, str):
    ...
    @staticmethod
    def assertLenEq(path: typing.Union[str, list], value):
        """
        长度等于
        """
        if isinstance(path, str):
        ...
        for _path in path:
            ...
            if isinstance(search_value, list):
                for i, _search_value in enumerate(search_value):
                    ...
                    assert len(_search_value) == value, f'匹配到的第{i+1}个结果: {len(_search_value)} 不等于 {value}'
            else:
                logger.info(f"断言: {len(search_value)} 等于 {value}")
                assert len(search_value) == value, f'{len(search_value)} 不等于 {value}'

    @staticmethod
    def assertLenGt(path: typing.Union[str, list], value):
        """
        长度大于
        """
        if isinstance(path, str):
        ...
        for _path in path:
            ...
            if isinstance(search_value, list):
                for i, _search_value in enumerate(search_value):
                    ...
                    assert len(_search_value) > value, f'匹配到的第{i + 1}个结果: {len(_search_value)} 小于等于 {value}'
            else:
                logger.info(f"断言: {len(search_value)} 大于 {value}")
                assert len(search_value) > value, f'{len(search_value)} 小于等于 {value}'

    @staticmethod
    def assertLenLt(path: typing.Union[str, list], value):
        """
        长度小于
        """
        if isinstance(path, str):
        ...
        for _path in path:
            ...
            if isinstance(search_value, list):
                for i, _search_value in enumerate(search_value):
                    ...
                    assert len(_search_value) < value, f'匹配到的第{i + 1}个结果: {len(_search_value)} 大于等于 {value}'
            else:
                logger.info(f"断言: {len(search_value)} 小于 {value}")
                assert len(search_value) < value, f'{len(search_value)} 大于等于 {value}'

    @staticmethod
    def assertGt(path: typing.Union[str, list], value):
        """
        大于
        """
    ...
    @staticmethod
    def assertLt(path: typing.Union[str, list], value):
        """
        小于
        """
        if isinstance(path, str):
    ...
    @staticmethod
    def assertRg(path: typing.Union[str, list], start, end):
        """
        范围
        """
        if isinstance(path, str):
    ...
    @staticmethod
    def assertIn(path: typing.Union[str, list], value):
        """
        被包含
        """
        if isinstance(path, str):
    ...
    @staticmethod
    def assertNotIn(path: typing.Union[str, list], value):
        """
        不被包含
        """
        if isinstance(path, str):
    ...
    @staticmethod
    def assertCt(path: typing.Union[str, list], value):
        """
        包含
        """
        if isinstance(path, str):
    ...
    @staticmethod
    def assertAnyCt(path: list, value):
        """
        有一个包含
        """
        search_list = []
        ...
        for _search_value in search_list:
            if value in _search_value:
                assert True
                break
        else:
            assert False, f'{search_list} 都不包含 {value}'

    @staticmethod
    def assertNotCt(path: typing.Union[str, list], value):
        """
        包含
        """
        if isinstance(path, str):
