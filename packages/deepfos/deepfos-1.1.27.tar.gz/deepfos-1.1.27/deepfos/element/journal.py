...
class AsyncJournalModel(ElementBase[JournalModelAPI]):
    ...
    def _gen_batch_payload(
        ...
        if is_editing:
            ...
            head_data = head_data.rename(
                columns={"_type": "journalTypeCode", relation_field: "headMainId"}
            )
        ...
        if is_editing:
            ...
            line_data = line_data.rename(columns={relation_field: "headMainId"})
            line_data = line_data.merge(
                head_data[["headMainId", "journalTypeCode"]],
                on="headMainId"
            )
