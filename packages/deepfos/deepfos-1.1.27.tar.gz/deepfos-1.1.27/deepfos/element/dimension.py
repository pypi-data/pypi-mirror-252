...
class DimMemberV12(DimensionMemberOperationSw):
    """仅用alias特性便于做字段兼容"""
    is_active = Field(True, alias='isActive')
    is_modula = False
    aggweight = 1.0
    sharedmember = Field(False, alias='sharedMember')
    datatype = "NUMBER"
    parent_name = Field(ROOT, alias='parentName')
    origin_name: str = Field(None, alias='originName')
    origin_parent_name: str = Field(None, alias='originParentName')
...
class AsyncDimension(ElementBase[DimensionAPI]):
    ...
    async def save(
        ...
        is_v1_0 = self.api.version == self.api.default_version
        dim_members = []
        for v in chain(
            ...
            mbr = v.dict(exclude_none=True)
            if is_v1_0:
                mbr.setdefault(INDEX_FIELD, 0)
                dim_members.append(mbr)
            else:
                dim_members.append(DimensionMemberOperationDto.construct_from(
                    **DimMemberV12.construct_from(**mbr).dict(by_alias=True)
                ))
        ...
        if len(dim_members) > 0:
            if is_v1_0:
                ...
                r = await self._save_tree(dim_members)
    ...
    async def load_dataframe(
        ...
        **langugage_keys: str
        ...
        if (
            INDEX_FIELD not in df.columns
            and self.api.version == self.api.default_version
        ):
            df[INDEX_FIELD] = 0
    ...
    async def _save_replace(self, reorder, member_list):
        ...
        if not is_v1_0:
            member_list = [
                DimMemberV12.construct_from(**mbr).dict(
