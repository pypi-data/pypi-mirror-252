    ...
    for r in model.reactions:
        if len(r.metabolites) == 1: 
            if r.id.startswith("EX_") == False: 
                cnt += 1
                print(cnt, ':', r.id, ':', r.reaction, ':', r.bounds)
...
def check_artificial_atoms(model, preview=None):
...
def check_mass_unbalances(model, threshold=1e-5):
    ...
    for r in model.reactions:
        ...
        if len(suggestions.keys()) > 0:
            cnt += 1
            print(cnt, ':', r.id, ':', r.reaction, ':', suggestions)
            found_rids.append(r.id)
    
    
    if found_rids == []:
        print("No mass-unbalanced reactions found.")
    return found_rids



def check_charge_unbalances(model, threshold=1e-5):
    ...
    if found_rids == []:
        print("No charge-unbalanced reactions found.")
    return found_rids
...
def search_biomass(model, show_reaction=False, verbose=True):
    ...
    if found_rids == []:
        print("No biomass reactions found.")
    return found_rids
