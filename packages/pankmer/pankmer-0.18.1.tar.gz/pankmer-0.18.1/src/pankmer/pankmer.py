...
class PKResults:
    ...
    def get_sequences_scores(self, sequences: dict, cpu: int = None) -> dict:
        ...
        core_chunks = [
            [
                self.results_dir,
                {kmer: fillna_val for kmer in combined_kmers[i : i + chunk_size]},
            ]
            for i in range(0, len(combined_kmers), chunk_size)
        ]
        ...
        return {
            name: self.get_sequence_score(seq, scored_kmers)
            for name, seq in sequences.items()
        }

    def get_regional_scores(
        self, reference: str, regions: dict, cpu: int = None
    ) -> dict:
        """
