...
def datatype_check_integer(excel_file, sheet_name, column_name):
    ...
    #get a list as o/p with the datatype check result(True or false); invert it; filter the column with that mask and get the failed row indices alone
    ...
    if non_integer_rows.empty:
        ...
        return {"No of rows failed": len(non_integer_row_numbers),
...
if __name__ == "__main__":
    result = datatype_check_integer(
        ...
        "Unique_Farmers", "COH delivered volume in KG")
    print(result)
