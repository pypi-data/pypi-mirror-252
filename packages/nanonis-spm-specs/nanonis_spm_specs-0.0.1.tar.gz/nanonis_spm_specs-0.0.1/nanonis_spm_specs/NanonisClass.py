...
@author: Samuel O'Neill and Markus A. Huber
...
class Nanonis:
    ...
    def __init__(self, port, IP_Adress):
        # enter IP and Port of Nanonis server
        self.TCP_IP = IP_Adress
        self.TCP_PORT = port
    ...
    def send(self, Command, Body, BodyType):
        ...
        while (Recv_BodySize != len(Recv_Body) or counter < 1000):  # Making sure all the data is received
            Recv_Body = Recv_Body + self.socket.recv(Recv_BodySize - len(Recv_Body))
            counter += 1
        if (self.displayInfo == 1):
