...
def _validate_args(function):
    ...
    def type_checker(*args, **kwargs):
        try:
            ...
            function_specific_validation(bound_args_dict, function.__name__)
...
def function_specific_validation(args_dict, function_name):
...
def frequency_validator(frequency: str, function_name):
    if function_name in ["resample", "get_universe"]:
        business_pattern = "b?"
    else:
        business_pattern = ""
...
def sample_period_validator(period: int, _):
    # period_pattern = fr"([1-9][0-9]{{0,3}})?(D|W|M|Q|SA|A)"
...
validation_functions: dict[str, Callable[[Any, str], Any]] = {
    ...
    "sample_period": sample_period_validator,
...
def get_sm_attributevalue(attribute: str):
    ...
    if smattributevalue is None:
        raise PrismValueError(
            ...
            valid_list=list(set(const.SMValues.keys())-set(['Security Type', 'Country']))
        )
