...
class QCloud(QCloudService):
    ...
    def _batch_submit_and_query(self, 
        ...
        if extar_header_data:
            recv_string = self._send_request(str_url=batch_url, 
                                             post_data=task_msg,
                                             use_json_dormat=False,
                                             extra_headers=extar_header_data)
        else:
            recv_string = self._send_request(batch_url, task_msg)
    ...
    def async_batch_real_chip_measure(self, 
        ...
        if super().use_compress_data:
            ...
            recv_str = self._send_request(str_url=super().big_data_batch_compute_url, 
