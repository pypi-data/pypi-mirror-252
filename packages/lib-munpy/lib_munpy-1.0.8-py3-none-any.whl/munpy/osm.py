...
def get_osm_map(top, left, bottom, right, save_path, address=config.OSM_API, port=5000):
    url = f'http://{address}:{port}/download_osm_file/{top}/{left}/{bottom}/{right}'
...
if __name__ == "__main__":
    osm = OSMHandler('madrid')
    osm.create_munich_files(save=True)
