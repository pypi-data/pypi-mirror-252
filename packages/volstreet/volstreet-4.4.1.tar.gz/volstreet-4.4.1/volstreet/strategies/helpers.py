...
from volstreet.angel_interface.fetching import LiveFeeds, fetch_quotes
...
def sleep_until_next_action(interval_minutes: int, exit_time: tuple[int, int]) -> None:
    next_timestamp = current_time() + timedelta(minutes=interval_minutes)
    next_timestamp = min(
        next_timestamp,
    ...
    sleep_time = max((next_timestamp - current_time()).total_seconds(), 0)
    sleep(sleep_time)
...
def filter_out_illiquid_options(
    ...
    if LiveFeeds.price_feed_connected() and all(
        [option.token in LiveFeeds.price_feed.data_bank for option in options]
    ):
        return [
            option
            for option in options
        ...
        return [
            ...
            if current_time()
            - datetime.strptime(
                quotes[option.token]["exchTradeTime"], "%d-%b-%Y %H:%M:%S"
            )
            < timedelta(minutes=timedelta_minutes)
        ]
