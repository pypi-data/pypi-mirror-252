...
from volstreet.utils.core import current_time, get_background_tasks
...
from volstreet.angel_interface.fetching import LiveFeeds, fetch_book
    ...
    def run(self):
        ...
        if LiveFeeds.price_feed_connected():
            for index in self.indices_to_trade:
                ...
                LiveFeeds.price_feed.subscribe_options(
                    index, range_of_strikes=strikes_to_track
                )
...
class Client:
    ...
    def __init__(
        ...
        self._post_init(websockets, run_tasks)

    @classmethod
    def from_name(cls, client: str, *args, **kwargs) -> "Client":
        try:
        ...
        instance = cls(user, pin, apikey, authkey, *args, **kwargs)
        instance.name = client
        instance.webhook_urls["default"] = os.getenv(f"{client}_WEBHOOK_URL")

        return instance

    def _post_init(self, websockets: bool, run_tasks: bool) -> None:
        # Set the long-running functions to run at 9:15:05
        start_time = current_time().replace(hour=9, minute=15, second=5)
        seconds_to_start = (start_time - current_time()).total_seconds()
        if websockets:
            if seconds_to_start < 0:
                Thread(target=LiveFeeds.start_feeds).start()
            else:
                logger.info(f"Starting LiveFeeds at {start_time}")
                Timer(seconds_to_start, LiveFeeds.start_feeds).start()

        if run_tasks:
            background_tasks = get_background_tasks(self, "continuously")
            for task in background_tasks:
                if seconds_to_start < 0:
                    logger.info(
                        f"Starting continuous thread in client {self.name}: {task.__name__}"
                    )
                    threading.Thread(target=task).start()
                else:
                    logger.info(
                        f"Starting continuous thread in client {self.user} at {start_time}: {task.__name__}"
                    )
                    timer = Timer(seconds_to_start, task)
                    timer.start()
...
def run_client(client: Client, notifier_level="INFO") -> None:
