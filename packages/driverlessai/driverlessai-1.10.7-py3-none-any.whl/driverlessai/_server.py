...
class Version:
    ...
    def __init__(self, version_str: str) -> None:
        if not version_str:
            raise ValueError("A version cannot be None or empty")
        match = Version._PATTERN.match(version_str)
        if not match:
            raise ValueError(
                f"Version '{version_str}' doesn't match "
                f"regex '{Version._PATTERN.pattern}'."
            )
        version_components = [0, 0, 0, 0]
        for i in range(match.lastindex):
            version_components[i] = int(match.group(i + 1))
        self._version_components = tuple(version_components)
        self._version_str = version_str

    def __eq__(self, other: Any) -> bool:
        """Return self==other."""
        if isinstance(other, (str, Version)):
            return self._version_components == self._parse(other)._version_components
    ...
    def __repr__(self) -> str:
        """Return repr(self)."""
        return repr(self._version_components)
