...
def format_subscriptions(ctx, subscriptions, get_usage_metrics=False, **kwargs):
    orgs_column = [column("id"), column("organisation")]
...
def reconcile_billing_subscriptions(
    ...
    for account in accounts:
        ...
        for sub in account.status.subscriptions:
            ...
            for org in account.status.orgs:
                ...
                val = counts.setdefault(admin_state, 0)
                counts[admin_state] = val + 1
                if (
                    admin_state == "active"
                    ...
                    results.append({"org": org, "sub": sub})
                    subscriptionsNotActive += 1
                    subscriptionGood = False
            if counts.get("active", 0) == 0 and (
                sub["status"] == "active" or sub["status"] == "trialing"
            ):
                print(
    ...
    columns = [
        column("organisation", getter=org_getter),
        column("id", getter=org_id_getter),
        column("admin_state", getter=state_getter),
