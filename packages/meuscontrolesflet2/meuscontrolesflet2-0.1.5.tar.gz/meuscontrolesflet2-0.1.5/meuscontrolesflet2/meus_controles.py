...
class ShowDF(UserControl):
    def __init__(self,df):
        super().__init__()
        self.df = df
        self.d1 = DataTable(border = border.all(1,'white,0.3'),heading_row_color = 'white,0.1')
        self.Colunas_tabela()
        self.Linhas_tabela()

    def Colunas_tabela(self):
        self.d1.columns = [DataColumn(Text(i,text_align='center',selectable = True,style=TextThemeStyle.TITLE_MEDIUM)) for i in list(self.df.columns)]
    ...
    def Linhas_tabela(self):
        ...
        for l,i in enumerate(df_lista):
            cell = [ DataCell(Row([Text(j,text_align='center',selectable = True)],alignment='left')) for j in i]
