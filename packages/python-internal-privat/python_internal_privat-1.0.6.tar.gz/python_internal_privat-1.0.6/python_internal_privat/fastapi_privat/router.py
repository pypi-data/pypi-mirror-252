...
async def client_info(
    ...
    try:
        ...
        if payload is not None:
            mng.token = payload.get("token")
            mng.iban = payload.get("iban")
...
async def statement(
    schema: PrivatSchemaStatement, session: AsyncSession = Depends(async_session)
) -> Dict:
    try:
        mng = AsyncPrivatManager()
        ...
        if payload is not None:
            mng.token = payload.get("token")
            mng.iban = payload.get("iban")
            response = await mng.get_statement(schema.period, schema.limit)
            return response
        return mng.does_not_exsists_exception()
    except Exception as exc:
        exception = {"detail": str(exc)}
