...
async def read_privat(user_id: str, session: AsyncSession) -> Dict:
    try:
        ...
        if query.first() is not None:
            token = query.first()[0].privat_token
            query = await session.execute(select(mdl).where(mdl.user_id == user_id))
            iban = query.first()[0].privat_iban
            return {"token": token, "iban": iban}
        return None
    except Exception as exc:
        exception = {"detail": str(exc)}
        return exception
...
async def update_privat(user: str, schema: PrivatSchemaUpdate, session: AsyncSession) -> Dict:
    try:
        mng = AsyncPrivatManager()
...
async def delete_privat(user: str, session: AsyncSession) -> Dict:
    try:
        ...
        if query.first() is not None:
            query = await session.execute(
                delete(mdl).where(mdl.user_id == user)
            )
            await session.commit()
