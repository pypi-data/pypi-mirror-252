...
from typing import Union, Tuple, Any, List, ByteString
import getpass
import sys
import re
import os
...
class AnsiColorConverter:
    def __init__(self):
        ...
        self._regex = re.compile(r"\033\[38;2;(\d+);(\d+);(\d+);(\d+)m")

    def escape_to_argb(self, escape: bytes) -> Union[Tuple[int, int, int, int], None]:
        escape_str = escape.decode(self._encoding)
        match = self._regex.match(escape_str)
    ...
    def argb_to_fore_escape(self, argb: Union[Tuple[int, int, int, int], None, str]) -> bytes:
        if isinstance(argb, str):
            return argb
        else:
            return f"\033[38;2;{argb[1]};{argb[2]};{argb[3]}m".encode(self._encoding)
    
    def argb_to_back_escape(self, argb: Union[Tuple[int, int, int, int], None, str]) -> bytes:
        if isinstance(argb, str):
            return argb
        else:
            return f"\033[48;2;{argb[1]};{argb[2]};{argb[3]}m".encode(self._encoding)
    
    
    def hex_to_escape(self, code: Union[str, bytes], hashtag: bool = True) -> bytes:
        lcode: bytes = b''

        if isinstance(code, str):
            lcode = code.encode(self._encoding)

        elif isinstance(code, bytes):
            lcode = code
        
        else:
            raise TypeError("The method can only accept bytes and str")
        
        if hashtag:
            lcode = lcode.lstrip(b'#')

        rgb = tuple(int(lcode[i:i+2], 16) for i in (0, 2, 4))
...
class AnsiConsole(AnsiEscapeCodes):
    ...
    def getesc(
        self,
        style: Union[List[str], str, bytes]
    ) -> str:
    ...
    def reset(
        ...
        return sys.stdout.write(f"{self.Styles.RESET}{end}")
    
    def printf(
        self,
        text: Union[str, bytes],
        style: Union[List[str], str, bytes] = None,
        end: str = '\n'
    ) -> Tuple[int, int]:
    ...
    def _parse_ansi_escape(
        self,
        escape_code: Union[str, bytes]
    ) -> Tuple[int, int, int]:
    ...
    def passf(
        ...
        usr_input = getpass.getpass(self.getesc(style)+prompt)
        self.reset()
        return usr_input
    ...
    def titlef(
        ...
        write_result: int = sys.stdout.write(f"\033]0;{ltitle}\007")
        sys.stdout.flush()
        return write_result
