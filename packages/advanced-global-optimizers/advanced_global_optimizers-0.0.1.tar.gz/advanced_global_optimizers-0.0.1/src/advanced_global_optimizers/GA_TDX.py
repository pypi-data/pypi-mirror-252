...
from .GATDX_base import GATDXtool
import time

class GA_TDX(GATDXtool):
    ...
    def evaluate(self, population, problem):
        x = (population - self.config.norm_lb) / (self.config.norm_ub - self.config.norm_lb)
        x = x * (problem.ub - problem.lb) + problem.lb
        cost = problem.eval(x)
    ...
    def gm(self, population, population_cost, problem):
        ...
        o_worse = self.nonuniform_mutation(x_worse, self.config.norm_lb, self.config.norm_ub)
    ...
    def optimize(self, problem):
        st_time = time.time()
        self.init(problem)
        population = np.random.rand(self.NP, problem.dim) * (self.config.norm_ub - self.config.norm_lb) + self.config.norm_lb
        population_cost = self.evaluate(population, problem)
        FEs_rec = {}
        for t in self.config.tolerance:
            FEs_rec[t] = {}
        ...
        while self.FEs < self.maxFEs:
            ...
            for t in self.config.tolerance:
                for p in self.config.precision:
                    if np.max(gs) <= 0 and np.max(hs) <= t and self.gbest[0] <= p and FEs_rec[t][p] < 0:
                        FEs_rec[t][p] = self.FEs
                        break

        ed_time = time.time()
        return {'x': self.gbest_solution, 'fun': self.gbest[0], 'maxcv': self.gbest[1], 'nfev_rec': FEs_rec, 'runtime': ed_time - st_time, 'nfev': self.FEs}
