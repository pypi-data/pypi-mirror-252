import numpy as np
from pypop7.optimizers.es.sepcmaes import SEPCMAES as sepcmaes
import time
...
class SEPCMAES:
    ...
    def evaluate(self, problem, x):
        ...
        x = np.clip(x, self.config.norm_lb, self.config.norm_ub)
    ...
    def optimize(self, problem):
        ...
        mean = (self.config.norm_ub + self.config.norm_lb) / 2
        options = {
           ...
           'c_c': self.c_c,
           'mean': mean * np.ones((problem.dim,)),
           'sigma': self.sigma,
           }
        optimizer = sepcmaes(p, options)  # initialize the optimizer
