...
def prepare_code_for_moxing(data_download_method):
    ...
    code_cache_file_path = os.path.join(local_code_path, "code_cache_file.txt")
    if not os.path.exists(code_cache_file_path):            
        if code_url != "":
            code_to_env(code_url, local_code_path, code_need_unzip, data_download_method)
    else:
        print(f'Detected code_cache_file.txt already exist, code has been prepared!')            
...
def prepare_dataset_for_moxing(data_download_method):
    ...
    dataset_cache_file_path = os.path.join(local_dataset_path, "dataset_cache_file.txt")
    if not os.path.exists(dataset_cache_file_path):  
        ...
        print(f'Detected dataset_cache_file.txt already exist, dataset has been prepared!')   
...
def prepare_pretrain_model_for_moxing(data_download_method):
    ...
    pretrainmodel_cache_file_path = os.path.join(local_pretrain_model_path, "pretrainmodel_cache_file.txt")
    if not os.path.exists(pretrainmodel_cache_file_path):  
        ...
        print(f'Detected pretrainmodel_cache_file.txt already exist, pretrainmodel has been prepared!')   
    ...
    if not os.path.exists(code_cache_file_path):
        ...
        if all_code_copied:
            with open(code_cache_file_path, 'w') as f:
                f.write('This is a code cache file used to indicate that the code preparation is complete. Please do not delete this file.Unless you want to overwrite the code or fetch the code again.')
    else:
        print(f'Detected code_cache_file.txt already exist, code has been prepared!') 
    return local_code_path
    ...
    if not os.path.exists(dataset_cache_file_path):      
        ...
        if all_datasets_copied:
            with open(dataset_cache_file_path, 'w') as f:
                f.write('This is a dataset cache file used to indicate that the dataset preparation is complete. Please do not delete this file. Unless you want to overwrite the dataset or fetch the dataset again.')
    else:
        print(f'Detected dataset_cache_file.txt already exist, dataset has been prepared!')
    return local_dataset_path    
...
def prepare_pretrain_model_for_mount(data_download_method):
    ...
    if is_directory_empty(local_pretrain_model_path) == False:
        ...
        mount_success = False 
    pretrainmodel_cache_file_path = os.path.join(local_pretrain_model_path, "dataset_cache_file.txt")
    if not os.path.exists(pretrainmodel_cache_file_path):     
        ...
        if all_pretrainmodels_copied:
            with open(pretrainmodel_cache_file_path, 'w') as f:
                f.write('This is a pretrainmodel cache file used to indicate that the pretrainmodel preparation is complete. Please do not delete this file. Unless you want to overwrite the pretrainmodel or fetch the pretrainmodel again.')
    return local_pretrain_model_path
