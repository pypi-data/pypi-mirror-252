...
def get_stock_history_by_yfinance(ticker:str, intervals:list):
    for interval in intervals:
        try:
            ...
            buf = yf.download(tickers=ticker, period=_period, interval=_interval)
            print(buf)
            df = pd.DataFrame(buf, columns=['date', 'open', 'high','low','close', 'adj close', 'volume'])
            if buf.empty:
                return buf
            else:
                df = buf
            ...
            df['ticker'] = ticker2
            df.to_csv(data_dir + f'/{ticker2}_hist_{interval}.csv', index=False)
        except Exception as e:
            print('Exception: {}'.format(e))
