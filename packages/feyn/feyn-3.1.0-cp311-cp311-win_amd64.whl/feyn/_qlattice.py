...
class QLattice(QLatticeNotebookMixin if detect_notebook() else object):
    ...
    def sample_models(
        ...
        for name in input_names:
            if ":" in name:
                raise ValueError(f"Input names with a colon ':' are not allowed, please rename '{name}'.")
        ...
        try:
            query = Query(query_string, max_complexity, input_names, function_names, output_name)
