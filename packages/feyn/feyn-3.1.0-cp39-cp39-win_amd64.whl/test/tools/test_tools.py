...
from feyn.tools._data import _stratified_split, _adjust_random_subset
        ...
        with self.subTest("Can split dict of arrays"):
            with self.assertLogs("feyn.tools._data", level="WARNING") as logger:
                ...
                self.assertEqual(
                    logger.output,
                    [
                        "WARNING:feyn.tools._data:Deprecation: using dicts of numpy arrays are deprecated and will be removed in future versions of Feyn."
                    ],
                )
    ...
    def test_warnings_and_exceptions(self):
        ...
        with self.subTest(
            ...
            self.assertEqual(
                logger.output,
                [
                    "WARNING:feyn.tools._data:The sample count in one of the subsets deviates from expected ratio by 0.133. Do you have enough data to split?",
                ],
            )
