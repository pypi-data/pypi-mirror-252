...
class DataExtractor:
    ...
    def convert_docx_to_txt(self, filename, file_storage_path):
        final_result = {}
        try:
            docx_files = glob.glob(filename)
            logger.log(f"docx_files:::   {docx_files}")

            for docx_path in docx_files:
                txt_path = docx_path[:-5] + '.txt'
                with open(docx_path, 'rb') as infile:
                    with open(txt_path, 'w', encoding='utf-8') as outfile:
                        doc = docx2txt.process(infile)
                        logger.log("=========")
                        # Adjust paragraph separation and check for blank lines
                        for line in doc.split('\n'):
                            if line.strip():  # Check if the line is not empty after stripping whitespace
                                outfile.write(line + '\n')

                if os.path.exists(txt_path):
                    logger.log(f"Text file created: {txt_path}")
                    
                    # Check if the text file is empty
                    if os.path.getsize(txt_path) == 0:
                        logger.log(f"Text file is empty: {txt_path}")
                        # Call convert_docx_to_pdf function when the text file is empty
                        # raise ValueError("Simulated error during text file creation")
                        self.convert_docx_to_pdf(docx_path)
                else:
                    logger.log(f"Failed to create text file for: {docx_path}")
            ...
            logger.log(f'\n Print exception after manually  : {returnErrOfDocx}', "0")
            final_result['status'] = 0
            final_result['error'] = str(returnErrOfDocx)
            # Call convert_docx_to_pdf function in case of an exception
            final_result=self.convert_docx_to_pdf(docx_path)
        logger.log(f"final_result:::459 {final_result}")
        return final_result 

    
    def convert_docx_to_pdf(self, file_path):
        final_result = {}
        logger.log("indide convert_docx_to_pdf")
        try:
