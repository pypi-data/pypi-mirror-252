...
class Report:
    ...
    Methods:
        ...
        _load_raw_page(offset: int, limit: int) -> list[dict[str, any]]
            Loads a page of items from the Sklik API.

        load_page(offset: int, limit: int) -> list[dict[str, any]]
            Loads a page of items from the Sklik API and formats them so that one dict
            contains one time unit with one object.
    ...
    def _load_raw_page(self, offset: int, limit: int) -> list[dict[str, any]]:
        """
        :param offset: The offset value for data pagination.
        :param limit: The maximum number of data items to retrieve per page.
        :return: A list of dictionaries representing the raw page data.
        ...
        return self._api.call(self.service, 'readReport', args=args)['report']
    ...
    def load_page(self, offset: int, limit: int) -> list[dict[str, any]]:
        ...
        """
        report = self._load_raw_page(offset, limit)
        output = [dict(**stat,
