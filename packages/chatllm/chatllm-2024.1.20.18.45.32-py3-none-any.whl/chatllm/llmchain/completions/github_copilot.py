...
class Completions(object):
    ...
    def get_access_token(self, api_key):
        token = self._get_access_token(api_key=api_key)
        expires_at = int(token.split('exp=')[1][:10])  # tid=7fdaed051cf26939e7cc11e4aed37893;exp=1705749197
        if expires_at < time.time():  # 过期刷新
            token = self._get_access_token(api_key, dynamic_param=time.time())
        return token

    @staticmethod
    @retrying
    @ttl_cache(ttl=1500)  # todo: 清除缓存
