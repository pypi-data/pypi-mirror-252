...
def tree_path(user_query, columns, description):
    function_tree = '''
    {
    ...
    messages = [
        {
            ...
            + '''Your task is:
                if there are function(s) needed to be applied, to find the path to the group where the next function to be applied next belong;
                else, return []
...
def schema_gpt_tree(user_query, column_description, description):
    tree = {
        "text_processor": [
            {
                "name": "para_sep",
    ...
    path = tree_path(user_query, column_description, description)
    print(path)
...
def table_gen_pd(user_query, table, enum, description, status):
    while True:
        response = schema_gpt_tree(user_query + " I am given a table with the following columns: " + description, enum, description)
        # print(response)
