...
class LDA:
    ...
    def viz(self):
        ...
        print("  you may execute 'pyLDAvis.display(lda.vis_data)'")
...
## LDA Process
def lda_process(doc_file, chi=False, eval=False):
...
## NMF Process
def nmf_process(doc_file, chi=False, eval=False):
...
class BTM:
    ...
    def generate_embeddings(self, doc):
        # Load the Word2Vec model for customized embeddings
        w2v_file = files('wordtm.models').joinpath("CUV_w2v_model")
        w2v_model = Word2Vec.load(w2v_file)

        # Convert the text to tokens using the BERT tokenizer
        input_ids = torch.tensor([self.btokenizer.encode(doc, 
                                        max_length=512, 
                                        truncation=True,
                                        add_special_tokens=True)])
    
        # Generate the BERT embeddings
        outputs = self.bmodel(input_ids)
        bert_embeddings = outputs[0][0, 1:-1, :].detach().numpy()
    
        # Generate the customized embeddings using Word2Vec
        tokenized_text = [word for word in doc.split()]
        w2v_embeddings = [w2v_model.wv[word] for word in tokenized_text
                             if word in w2v_model.wv.key_to_index]
    
        # Combine the BERT and customized embeddings
        combined_embeddings = np.concatenate([bert_embeddings, w2v_embeddings],\
                                             axis=0)
    ...
    def embedding(self):
        self.btokenizer = BertTokenizer.from_pretrained('bert-base-chinese')
        self.bmodel = BertModel.from_pretrained('bert-base-chinese')
        
        j_pro_docs = [" ".join(doc) for doc in self.pro_docs]

        # Generate the document embeddings using the combined embeddings
        document_embeddings = [self.generate_embeddings(doc) 
                               for doc in j_pro_docs]
    ...
    def fit_chi(self):
        ...
        if self.embed:
            document_embeddings = self.embedding()
    ...
    def show_topics(self):
        ...
        for topic, words in self.model.get_topic_freq().head(self.num_topics).iterrows():
            if not self.model.get_topic(topic): continue
...
## BTM Process
def btm_process(doc_file, chi=False, eval=False):
