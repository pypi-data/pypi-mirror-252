...
def get_cluster_spec(
    ...
    cluster_spec = ClusterSpec(
        ...
        spark_env_vars={
            "TECTON_CLUSTER_NAME": tecton.conf.get_or_raise("TECTON_CLUSTER_NAME")
        },
...
def launch_jobs_and_wait(databricks_client, splits, parallel_job_count, cluster_spec, workspace_name, feature_service_name, timestamp_key, spine_path, output_path):
    ...
    # TODO: change print to log and make resumable state
    while remaining_splits or active_jobs:
        # poll jobs
        for run_id, split in list(active_jobs.items()):
            if databricks_client.jobs.get_run(run_id).state.life_cycle_state.name in DATABRICKS_TERMINAL_STATE_NAMES:
                success = databricks_client.jobs.get_run(run_id).state.result_state.name == "SUCCESS"
                logger.info(f"Run {run_id} finished with success={success} for split {split}")
                if not success:
                    remaining_splits.append(split)
                    logger.info(f"Retrying split {split}")
        ...
        while remaining_splits and len(active_jobs) < parallel_job_count:
            ...
            logger.info(f"Launched run {run_id} for split {split}")
