...
from sdk.src.mini_logger import MiniLogger  # TODO: remove src from sdk
...
class UserContext:
    ...
    def _initialize(self, user_identifier: str = None, password: str = None, user_jwt: str = None):
        ...
        self._get_user_data_login_response(validate_user_jwt_response=data)
    ...
    def get_effective_profile_preferred_lang_code(self) -> LangCode:
        ...
        for lang_code_enum in LangCode:
            if lang_code_enum.value == self.lang_code:
                return lang_code_enum
        raise ValueError(f"{self.lang_code} is not a valid language code")
    ...
    def _authenticate_by_user_identification_and_password(self, user_identifier: str,
        ...
        try:
            ...
            # TODO Sending both user_identification and userIdentification
    ...
    def _authenticate_by_user_jwt(self, user_jwt: str) -> requests.Response:
        ...
        # TODO Please use get_http_headers() from python-sdk-python-package
        headers = {"Content-Type": "application/json"}
        # TODO authentication_login_validate_user_jwt_response
        validate_user_jwt_response_json = requests.post(
            url=authentication_login_validate_user_jwt_url,
            data=json.dumps(validate_user_jwt_request_json, separators=(",", ":")), headers=headers
        )
        if validate_user_jwt_response_json.status_code != HTTPStatus.OK:
            MiniLogger.error(
                "user-context-remote-python-package _authenticate_by_user_jwt() validate_user_jwt_response_json.status_code != HTTPStatus.OK " + validate_user_jwt_response_json.text)
            # MiniLoger.end()
            raise Exception(validate_user_jwt_response_json.text)
        MiniLogger.end(_AUTHENTICATE_BY_USER_JWT_METHOD_NAME,
                       object={"validate_user_jwt_response": validate_user_jwt_response_json})
        return validate_user_jwt_response_json

    # Used only in one please
    # TODO Can we change it to private?
    def _get_user_data_login_response(self, validate_user_jwt_response: requests.Response) -> None:
    ...
    def get_country_id(self) -> int:  # TODO: temp solution
        return 972
