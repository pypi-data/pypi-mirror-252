...
def verify_version(version):
    """check if valid Semantic Versioning and remove preceding 'v' character"""
...
class GeneratePyproject(object):
    ...
    def set_license_file(self):
        ...
        logging.warning("License file not found. Skipping..")
    ...
    def write_setup_py(self):
        """writes the 'setup.py' to the 'src' dir"""
...
setup()
"""
        self.write_file(content, self.setup_filename)
    ...
    def write_toml_file(self):
        ...
        # The values should already be set by the set_ methods
...
"""
        self.write_file(content, self.toml_filename)

    def write_file(self, content, filename):
        file_ = os.path.join(self.src_dir, filename)
        with open(file_, 'w') as f:
            f.write(content)

    def get_current_version(self):
        file_ = os.path.join(self.src_dir, self.toml_filename)
        del_ = 'version = "'
        with open(file_,) as f:
            content = f.readlines()

        for line in content:
            if line.startswith(del_):
                return line.split(del_)[-1].rstrip('"\n')
        return ""
...
def main():
    if not PACKAGE_NAME:
        logging.error("PACKAGE_NAME not set.")
        exit(2)

    version = verify_version(VERSION)
    if not version:
        logging.error(f"{str(VERSION)} is not a valid version.")
        exit(2)

    gp = GeneratePyproject(PACKAGE_NAME, version)
    gp.init_project()
    current_version = gp.get_current_version()
    print("---")
