...
from stigg.generated import FetchEntitlementsQuery, GetEntitlements, GraphQLClientHttpError, GetPaywallInput, \
  GetPaywall, GraphQlClientInvalidResponseError, FetchEntitlementQuery, GetEntitlement
...
class StiggClient(Client):
    ...
    def get_entitlement(self, query: FetchEntitlementQuery) -> GetEntitlement:
        if self.enable_edge is False:
            return super().get_entitlement(query)

        url, params = build_get_entitlement_data(self.edge_api_url, query)
...
class AsyncStiggClient(AsyncClient):
    ...
    async def get_entitlement(self, query: FetchEntitlementQuery) -> GetEntitlement:
        if self.enable_edge is False:
            return await super().get_entitlement(query)
