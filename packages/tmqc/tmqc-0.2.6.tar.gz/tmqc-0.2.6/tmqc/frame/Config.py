...
class cFurture(ConfigBase):
    def __init__(self):
        ...
        ContractFee = namedtuple(
            'ContractFee', ['open_fee', 'close_fee', 'close_today_fee', 'margin_rate'])
        ...
        # ANSI编码通常对应于GB2312
        with open("conf/contract_pt_mul.ini", "rt", encoding='GB2312') as f:
            l = f.readline()
            while l:
                arrs = re.split(r"\s+", l)
                if arrs[0][0] != "#":
                    self.pt_mul[arrs[0]] = ContractMutil(
                        mintick=float(arrs[1])/10000.0, mutil=float(arrs[2]))
                l = f.readline()

        ContractSlippage = namedtuple('ContractSlippage', ['slippage'])
        # ANSI编码通常对应于GB2312
        with open("conf/contract_slippage.ini", "rt", encoding='GB2312') as f:
            l = f.readline()
            while l:
                arrs = re.split(r"\s+", l)
                if arrs[0][0] != "#":
                    self.slippage[arrs[0]] = ContractSlippage(
                        slippage=float(arrs[1])/100)
                l = f.readline()
