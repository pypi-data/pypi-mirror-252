...
class NeosException(Exception):
    "Base exception class for Neos Client"
...
class NeosClient:
    ...
    def is_alive(self) -> bool:
        ...
        return True if response.startswith("NeosServer is alive\n") else False
    ...
    def get_completion_code(self, job_number: int, job_password: str) -> str:
        ...
        that are "Waiting" or "Running". Returns "Normal", "Out of memory",
        "Timed out", "Disk Space", "Server error", "Unknown Job", or
        "Bad Password"
    ...
    def kill_job(self, job_number: int, job_password: str, message="") -> str:
        """
        ...
        str
        """
        return self.neos.killJob(job_number, job_password, message)
    ...
    def get_final_results(
        ...
        hang until the job is finished. The function returns a base-64 encoded
        object.
    ...
    def get_intermediate_results(
        ...
        """
        Gets intermediate results of a job submitted to NEOS, starting at the
        specified character offset up to the last received data. Intermediate
        results are usually the standard output of the solver daemon. Note
        that because output does not stream for jobs with "long" priority
        (default value), getIntermediateResults() will not return any results
        for long priority jobs. Output does stream for jobs with "short"
        priority (maximum time of 5 minutes).
    ...
    def print_queue(self):
        ...
        if not self.is_alive():
            raise NeosException("NeosServer is not alive. Try again later.")
    ...
    def submit_job(
        ...
        if job_number == 0:
            raise NeosException(f"NEOS Server error! {job_password}")
