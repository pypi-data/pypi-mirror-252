...
class DocumentdbConfigV1Config(BaseModel):
    """
    ...
    sync_pack_mode: Optional[Dict[str, Any]] = Field(default=None, description="Indicates whether synced data will be packed into a single entry(column), or unpacked with one layer of nested fields.")
    connection_type: Optional[Dict[str, Any]] = Field(default=None, description="Possible values:`SshTunnel`, `PrivateLink` . `SshTunnel` is used as a value if this parameter is omitted in the request and the following parameter's values are specified: `tunnel_host`, `tunnel_port`, `tunnel_user`.")
    port: Optional[StrictInt] = Field(default=None, description="Port of the primary node. Ignored if `hosts` value is provided.")
    host: Optional[StrictStr] = Field(default=None, description="Host IP address of the primary node. Ignored if `hosts` value is provided.")
    ...
    always_encrypted: Optional[StrictBool] = Field(default=None, description="Require TLS encryption.")
    ...
    __properties: ClassVar[List[str]] = ["public_key", "tunnel_port", "password", "sync_pack_mode", "connection_type", "port", "host", "tunnel_host", "schema_prefix", "always_encrypted", "user", "tunnel_user"]
    ...
    def sync_pack_mode_validate_enum(cls, value):
        ...
        if value not in ('UNPACKED_MODE', 'PACKED_MODE'):
            raise ValueError("must be one of enum values ('UNPACKED_MODE', 'PACKED_MODE')")
        return value
    ...
    def connection_type_validate_enum(cls, value):
        ...
        if value not in ('PrivateLink', 'SshTunnel'):
            raise ValueError("must be one of enum values ('PrivateLink', 'SshTunnel')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DocumentdbConfigV1Config from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DocumentdbConfigV1Config from a dict"""
        if obj is None:
            return None
