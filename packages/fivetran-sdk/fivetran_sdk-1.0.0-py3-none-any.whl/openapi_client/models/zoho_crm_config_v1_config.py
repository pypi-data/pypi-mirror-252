...
class ZohoCrmConfigV1Config(BaseModel):
    """
    ...
    data_center: Optional[Dict[str, Any]] = Field(default=None, description="Data Center")
    __properties: ClassVar[List[str]] = ["schema", "data_center"]

    @field_validator('data_center')
    def data_center_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('EU', 'AU', 'IN', 'CN', 'US'):
            raise ValueError("must be one of enum values ('EU', 'AU', 'IN', 'CN', 'US')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ZohoCrmConfigV1Config from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ZohoCrmConfigV1Config from a dict"""
        if obj is None:
            return None
