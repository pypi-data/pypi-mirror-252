# coding: utf-8

"""
    OpenAPI Definition

    The OpenAPI Specification is a standard format to define the structure and syntax of REST APIs. OpenAPI documents are both machine and human-readable, which enables anyone to easily determine how each API works. [More details](https://www.openapis.org/faq)

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


from __future__ import annotations
import pprint
import re  # noqa: F401
import json

from pydantic import BaseModel, Field, SecretStr, StrictBool, StrictInt, StrictStr, field_validator
from typing import Any, ClassVar, Dict, List, Optional
from typing import Optional, Set
from typing_extensions import Self

class HanaSapHvaEccConfigV1Config(BaseModel):
    """
    
    """ # noqa: E501
    public_key: Optional[StrictStr] = Field(default=None, description="Public Key")
    agent_user: Optional[StrictStr] = Field(default=None, description="The agent's user.")
    instance_number: Optional[StrictStr] = Field(default=None, description="Two-digit number (00-97) of the SAP instance within its host.")
    backint_executable_path: Optional[StrictStr] = None
    agent_config_method: Optional[Dict[str, Any]] = None
    backint_configuration_path: Optional[StrictStr] = None
    agent_password: Optional[SecretStr] = Field(default=None, description="The agent user's password. It must have a minimum length of 10 characters.")
    odbc_driver_manager_library_path: Optional[StrictStr] = Field(default=None, description="ODBC manager library path")
    agent_public_cert: Optional[StrictStr] = Field(default=None, description="The agent public certificate.")
    client_id: Optional[StrictStr] = None
    snc_partner_name: Optional[StrictStr] = None
    tunnel_port: Optional[StrictInt] = Field(default=None, description="SSH port, specify only to connect via an SSH tunnel.")
    database: Optional[StrictStr] = Field(default=None, description="The Hana database name.")
    password: Optional[SecretStr] = Field(default=None, description="The user's password.")
    hana_mode: Optional[Dict[str, Any]] = Field(default=None, description="The mode for connecting to HANA server. Available options: Single container (default), Multiple containers - Tenant database, Multiple containers - System database, Manual port selection - This option is used only if the database port needs to be specified manually.")
    host: Optional[StrictStr] = Field(default=None, description="DB instance host or IP address.")
    schema_prefix: Optional[StrictStr] = Field(default=None, description="Destination schema prefix. Prefix for each replicated schema. For example with prefix 'x', source schemas 'foo' and 'bar' get replicated as 'x_foo' and 'x_bar'. The prefix is permanent and cannot be changed after connection creation")
    agent_host: Optional[StrictStr] = Field(default=None, description="The host of the agent. This is the same as the database host, since the agent must be installed on the same machine as the source database.")
    hana_backup_password: Optional[SecretStr] = None
    log_on_group: Optional[StrictStr] = None
    connection_type: Optional[Dict[str, Any]] = Field(default=None, description="Possible values: `Directly`, `PrivateLink`, `SshTunnel`. `SshTunnel` is used as a value if this parameter is omitted in the request and any of the following parameter's values is specified: `tunnel_host`, `tunnel_port`, `tunnel_user`. Otherwise, `Directly` is used as a value if the parameter is omitted.")
    rfc_library_path: Optional[StrictStr] = None
    service_name: Optional[StrictStr] = None
    system_id: Optional[StrictStr] = None
    odbc_sys_ini_path: Optional[StrictStr] = Field(default=None, description="odbc.ini and odbcsinst.ini location")
    snc_library_path: Optional[StrictStr] = None
    always_encrypted: Optional[StrictBool] = None
    tunnel_user: Optional[StrictStr] = Field(default=None, description="SSH user, specify only to connect via an SSH tunnel.")
    snc_name: Optional[StrictStr] = None
    sap_source_schema: Optional[StrictStr] = Field(default=None, description="The Hana schema name where the SAP tables reside.")
    port: Optional[StrictInt] = Field(default=None, description="The port number.")
    archive_log_format: Optional[StrictStr] = None
    tunnel_host: Optional[StrictStr] = Field(default=None, description="SSH host, specify only to connect via an SSH tunnel (do not use a load balancer).")
    archive_log_path: Optional[StrictStr] = None
    agent_port: Optional[StrictInt] = Field(default=None, description="The port number of the agent.")
    user: Optional[StrictStr] = Field(default=None, description="The username.")
    __properties: ClassVar[List[str]] = ["public_key", "agent_user", "instance_number", "backint_executable_path", "agent_config_method", "backint_configuration_path", "agent_password", "odbc_driver_manager_library_path", "agent_public_cert", "client_id", "snc_partner_name", "tunnel_port", "database", "password", "hana_mode", "host", "schema_prefix", "agent_host", "hana_backup_password", "log_on_group", "connection_type", "rfc_library_path", "service_name", "system_id", "odbc_sys_ini_path", "snc_library_path", "always_encrypted", "tunnel_user", "snc_name", "sap_source_schema", "port", "archive_log_format", "tunnel_host", "archive_log_path", "agent_port", "user"]

    @field_validator('agent_config_method')
    def agent_config_method_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('UserId', 'Token', 'None'):
            raise ValueError("must be one of enum values ('UserId', 'Token', 'None')")
        return value

    @field_validator('hana_mode')
    def hana_mode_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('SingleContainer', 'MultiContainersTenant', 'MultiContainersSys', 'ManualPort'):
            raise ValueError("must be one of enum values ('SingleContainer', 'MultiContainersTenant', 'MultiContainersSys', 'ManualPort')")
        return value

    @field_validator('connection_type')
    def connection_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('Directly', 'PrivateLink', 'SshTunnel'):
            raise ValueError("must be one of enum values ('Directly', 'PrivateLink', 'SshTunnel')")
        return value

    model_config = {
        "populate_by_name": True,
        "validate_assignment": True,
        "protected_namespaces": (),
    }


    def to_str(self) -> str:
        """Returns the string representation of the model using alias"""
        return pprint.pformat(self.model_dump(by_alias=True))

    def to_json(self) -> str:
        """Returns the JSON representation of the model using alias"""
        # TODO: pydantic v2: use .model_dump_json(by_alias=True, exclude_unset=True) instead
        return json.dumps(self.to_dict())

    @classmethod
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of HanaSapHvaEccConfigV1Config from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    def to_dict(self) -> Dict[str, Any]:
        """Return the dictionary representation of the model using alias.

        This has the following differences from calling pydantic's
        `self.model_dump(by_alias=True)`:

        * `None` is only added to the output dict for nullable fields that
          were set at model initialization. Other fields with value `None`
          are ignored.
        """
        excluded_fields: Set[str] = set([
        ])

        _dict = self.model_dump(
            by_alias=True,
            exclude=excluded_fields,
            exclude_none=True,
        )
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of HanaSapHvaEccConfigV1Config from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return cls.model_validate(obj)

        _obj = cls.model_validate({
            "public_key": obj.get("public_key"),
            "agent_user": obj.get("agent_user"),
            "instance_number": obj.get("instance_number"),
            "backint_executable_path": obj.get("backint_executable_path"),
            "agent_config_method": obj.get("agent_config_method"),
            "backint_configuration_path": obj.get("backint_configuration_path"),
            "agent_password": obj.get("agent_password"),
            "odbc_driver_manager_library_path": obj.get("odbc_driver_manager_library_path"),
            "agent_public_cert": obj.get("agent_public_cert"),
            "client_id": obj.get("client_id"),
            "snc_partner_name": obj.get("snc_partner_name"),
            "tunnel_port": obj.get("tunnel_port"),
            "database": obj.get("database"),
            "password": obj.get("password"),
            "hana_mode": obj.get("hana_mode"),
            "host": obj.get("host"),
            "schema_prefix": obj.get("schema_prefix"),
            "agent_host": obj.get("agent_host"),
            "hana_backup_password": obj.get("hana_backup_password"),
            "log_on_group": obj.get("log_on_group"),
            "connection_type": obj.get("connection_type"),
            "rfc_library_path": obj.get("rfc_library_path"),
            "service_name": obj.get("service_name"),
            "system_id": obj.get("system_id"),
            "odbc_sys_ini_path": obj.get("odbc_sys_ini_path"),
            "snc_library_path": obj.get("snc_library_path"),
            "always_encrypted": obj.get("always_encrypted"),
            "tunnel_user": obj.get("tunnel_user"),
            "snc_name": obj.get("snc_name"),
            "sap_source_schema": obj.get("sap_source_schema"),
            "port": obj.get("port"),
            "archive_log_format": obj.get("archive_log_format"),
            "tunnel_host": obj.get("tunnel_host"),
            "archive_log_path": obj.get("archive_log_path"),
            "agent_port": obj.get("agent_port"),
            "user": obj.get("user")
        })
        return _obj


