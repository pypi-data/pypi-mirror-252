...
class AuroraConfigV1Config(BaseModel):
    """
    ...
    replica_id: Optional[StrictInt] = Field(default=None, description="Unique ID within the MySQL replica set. Must be an integer different from all other master and replica servers within the same group.")
    update_method: Optional[Dict[str, Any]] = Field(default=None, description="The method to detect new or changed rows. <br>Supported values:<br>`BINLOG` - Fivetran uses your binary logs (also called binlogs) to request only the data that has changed since our last sync. This is the default value if no value is specified. <br>`TELEPORT` - Fivetran's proprietary replication method that uses compressed snapshots to detect and apply changes.")
    ...
    __properties: ClassVar[List[str]] = ["public_key", "connection_type", "replica_id", "update_method", "always_encrypted", "tunnel_user", "tunnel_port", "database", "password", "port", "host", "tunnel_host", "schema_prefix", "user"]
    ...
    def update_method_validate_enum(cls, value):
        ...
        if value not in ('BINLOG', 'TELEPORT'):
            raise ValueError("must be one of enum values ('BINLOG', 'TELEPORT')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AuroraConfigV1Config from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AuroraConfigV1Config from a dict"""
        if obj is None:
            return None
