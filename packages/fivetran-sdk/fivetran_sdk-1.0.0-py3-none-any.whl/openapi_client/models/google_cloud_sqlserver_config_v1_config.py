...
class GoogleCloudSqlserverConfigV1Config(BaseModel):
    """
    ...
    tunnel_user: Optional[StrictStr] = Field(default=None, description="SSH user, only specify when connecting via an SSH tunnel.")
    tunnel_port: Optional[StrictInt] = Field(default=None, description="SSH port, only specify when connecting via an SSH tunnel.")
    ...
    tunnel_host: Optional[StrictStr] = Field(default=None, description="SSH host, only specify when connecting via an SSH tunnel (do not use a load balancer).")
    ...
    __properties: ClassVar[List[str]] = ["public_key", "connection_type", "update_method", "always_encrypted", "tunnel_user", "tunnel_port", "database", "password", "port", "host", "tunnel_host", "schema_prefix", "user"]
    ...
    def update_method_validate_enum(cls, value):
        ...
        if value not in ('NATIVE_UPDATE', 'TELEPORT'):
            raise ValueError("must be one of enum values ('NATIVE_UPDATE', 'TELEPORT')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GoogleCloudSqlserverConfigV1Config from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GoogleCloudSqlserverConfigV1Config from a dict"""
        if obj is None:
            return None
