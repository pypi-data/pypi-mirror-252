...
class MarkdownFormatter:
    ...
    def convert_links(self, content: str) -> str:
        ...
        def replace_obsidian(match):
            ...
            text = (
                self._strip_blank(parts[1])
                if len(parts) > 1
                else link.replace("%20", " ")
            )
...
class MarkdownHandler(MarkdownFormatter):
    def __init__(self, root_path: Path, output_file: str = "README.md"):
