...
class MarkdownFormatter:
    ...
    def replace_spaces_in_filenames(self, replace_with: str = "-"):
        """
        Replace spaces in filenames with the specified character.
        """
        for path in self.root_path.rglob("*"):
            if path.is_file():
                new_name = path.name.replace(" ", replace_with).replace(
                    "%20", replace_with
                )

                if new_name != path.name:
                    path.rename(path.parent / new_name)
                    logging.info(f"Renamed '{path.name}' to '{new_name}'")

    def _valid_link(self, path: str) -> str:
        # convert relative path to absolute path and remove ../
        ...
        if self._is_url(path):
            ...
            for suffix in self.suffixes:
                try:
                    if abs_path.with_suffix(suffix).is_file():
                        return (path + suffix).strip()
                except FileNotFoundError:
                    continue
        logging.warning(f"Invalid path: {path}")
        return "Invalid path or deleted file"
