...
class Webview:
    def __init__(self,debug:bool =False ,wnd:HWND =None,init_js :str =None) -> None:
        self.pwebview=0
    ...
    def resolve(self,seq:str,status:int,result:str):
        _webview_return(self.pwebview,seq.encode('utf8'),status,result.encode('utf8'))
    def bind(self,name:str,fn:
             Callable[[str,str],Union[None,str,Tuple[str,int]]]
             )->None:
         
        def wrapped_fn(seq:c_char_p,req:c_char_p,args:c_void_p):
            _result=fn(seq.decode('utf8'),req.decode('utf8'))
            
            if _result is None:return 
