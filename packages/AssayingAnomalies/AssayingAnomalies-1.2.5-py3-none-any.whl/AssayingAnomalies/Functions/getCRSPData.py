...
def getCRSPData(params):
    ...
    # Check if the user only wants domestic common equity or not.
    if params.domComEqFlag:
        crsp_msf = db.raw_sql(f"select * from CRSP.MSF where date>='01/01/{start}' and date<='12/31/{end}' and shrcd=10 and shrcd=11")
    else:
