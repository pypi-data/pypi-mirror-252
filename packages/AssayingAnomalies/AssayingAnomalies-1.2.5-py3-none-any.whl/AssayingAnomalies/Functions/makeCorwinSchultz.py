...
def makeCorwinSchultz(params):
    ...
    daskhi.iloc[dprc < 0] = np.nan
    dbidlo.iloc[dprc < 0] = np.nan

    # Take the absolute value of daily prices
    dprc = dprc.abs()

    # Compute lclose
    lclose = dprc.shift(1).fillna(np.nan) * dcfacpr / dcfacpr.shift(1).fillna(np.nan)
    ...
    # Compute lhi and llo
    lhi = daskhi.shift(1).fillna(np.nan) * dcfacpr / dcfacpr.shift(1).fillna(np.nan)
    llo = dbidlo.shift(1).fillna(np.nan) * dcfacpr / dcfacpr.shift(1).fillna(np.nan)
