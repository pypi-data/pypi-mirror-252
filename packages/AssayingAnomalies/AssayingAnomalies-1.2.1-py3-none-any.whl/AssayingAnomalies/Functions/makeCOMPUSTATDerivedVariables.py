...
def makeCOMPUSTATDerivedVariables(params):
    ...
    # Find additional dates in ffdata that are not in BE
    additional_dates = ffdata.index.difference(BE.index)

    # Extend BE to include these additional dates
    BE_extended = BE.reindex(BE.index.union(additional_dates))

    # Update BE with data from ffdata for the common firms and additional dates
    for permno in common_permno:
        BE_extended.loc[additional_dates, permno] = ffdata.loc[additional_dates, permno]
    ...
    # Slice the DataFrame BE_extended to match the date range of me
    start_date = me.index[0]
    end_date = me.index[-1]
    BE_extended = BE_extended[(BE_extended.index >= start_date) & (BE_extended.index <= end_date)]

    # Save the extended book equity dataframe, which we rename back to BE
    BE_extended.to_csv(comp_path + 'BE.csv')
    ...
    BE_extended.columns = me.columns
    BE_extended.index = me.index

    # Divide 'BE' by Shifted 'me' for Common Firms
    bm = BE_extended.divide(me.shift(6), axis=0)
    ...
    # :TODO Next, the matlab file creates another HML variable to be compared with the hml factor downloaded from Ken French's website. To do this, I also need to call the makeBivSortInd and runBivSort functions, which isn't made yet.
