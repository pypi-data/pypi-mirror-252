...
class ImageCrop:
    ...
    def crop(self, slot_config, img):
        ...
        if interest_region_crop.get('need_crop'):  # 是否需要兴趣区域裁剪
            ...
            # 取人体和衣服最靠近边缘的点
            left_edge, right_edge = self._get_x_human_and_cloth(img)
            top_edge, bottom_edge = self._get_y_human_and_cloth(img)
    ...
    @staticmethod
    def _get_y_human_and_cloth(img):
        """上下取人体和衣服最靠外的点"""
        # 衣服识别范围不存在,人的识别范围也不存在
        human_y_flag = False
        cloth_y_flag = False
        ...
        if human_y_flag and cloth_y_flag:
