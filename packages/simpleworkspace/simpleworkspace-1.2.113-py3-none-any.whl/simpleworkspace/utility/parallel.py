...
class Parallel:
    ...
    def Invoke(self, *actions: _Callable[[None], _T]) -> list[_T]:
        with self._GetExecutor() as executor:
            ...
            #await in all, but wait with unpacking, incase one is going to throw exception, allow rest to finish up atleast
            finishedPromises = [x for x in _as_completed(promises)] 
            #can raise TimeoutError or CancelledError or any other exception from the supplied action
            results = [x.result() for x in finishedPromises]
            return results
