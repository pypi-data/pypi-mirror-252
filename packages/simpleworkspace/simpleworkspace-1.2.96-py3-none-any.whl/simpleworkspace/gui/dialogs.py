...
class RichMessageBox(MessageBox):
    ...
    @classmethod
    def Show(cls, text:str, description: str = None, windowTitle: str = None, windowTopMost=False):
        '''
...
class ButtonBox(MessageBox):
    ...
    def _FrameFooter_Mapper(self, frame: _tk.Frame):
        ...
        for btnName in self.settings_Buttons:
            ...
            def OnClick_Btn(event, btnName=btnName): #capture current instance of btnOptions
                self.dialogResult = btnName
                self.root.destroy()
            btn.bind('<Button-1>', OnClick_Btn)
    ...
    @classmethod
    def Show(cls, buttons: list[str], message: str = "", windowTitle: str = None, windowTopMost=False):
        '''
