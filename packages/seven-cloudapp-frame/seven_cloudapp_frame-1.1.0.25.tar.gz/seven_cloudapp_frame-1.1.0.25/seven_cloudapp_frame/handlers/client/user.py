...
class LoginHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        user_info = invoke_result_data.data["user_info"] if invoke_result_data.data.__contains__("user_info") else None # 如果业务自查，已经存在用户信息就不进行创建
        if not user_info:
            ...
            invoke_result_data.data = user_info
