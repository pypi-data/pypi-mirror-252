...
class BaseAPIWrapper():
    ...
    def _prepare_params(self, base_params, additional_params=None):
        ...
        """
        logging.info(f"Entering _prepare_params() with base parameters: {base_params}")
        ...
        logging.info("Exiting _prepare_params()")
    ...
    def _send_request(self, method, path, params=None, data=None, json=None, headers=None, is_auth=False):
        
        """Send a request to the API."""

        if is_auth:
            url = self._url(path, is_auth=True)
        else:
            url = self._url(path)
    ...
    def post(self, path, data=None, headers=None, is_auth=False):
        """POST request."""
        return self._send_request('POST', path, data=data, headers=headers, is_auth=is_auth)
    ...
    def convert_yyyy_mm_dd_to_timestamp(self, date_str, is_end_date=False):
        ...
        """
        logging.info(f'Entering convert_yyyy_mm_dd_to_timestamp() for date {date_str}')
        ...
        logging.info("Exiting convert_yyyy_mm_dd_to_timestamp()")
        return timestamp
    ...
    def validate_date_yyyy_mm_dd(self, date_str):
        ...
        """
        logging.info("Entering validate_date_yyyy_mm_dd()")
        try:
            datetime.strptime(date_str, '%Y-%m-%d')
            logging.info("Date string is valid. Exiting validate_date_yyyy_mm_dd()")
            return True
        except ValueError:
            logging.error("Incorrect data format, should be YYYY-MM-DD. Exiting validate_date_yyyy_mm_dd()")
            return False