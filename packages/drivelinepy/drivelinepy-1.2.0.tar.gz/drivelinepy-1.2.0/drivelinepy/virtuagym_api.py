...
class VirtuagymAPI(BaseAPIWrapper):
    ...
    def _fetch_data(self, endpoint, initial_params):
        logging.info(f'Entering _fetch_paginated_data() for endpoint {endpoint}')
        all_data_list = []
        ...
        while True:
            ...
            if response is None:
                logging.error(f"Error fetching data from {endpoint}. The response is None.")
                break
            ...
            if response.get("status", {}).get("results_remaining", 0) > 0:
                ...
                if '=' in next_page_str:
                    ...
                    logging.info(f"Fetching next page: {params['sync_from']} remaining {response['status']['results_remaining']}")
                else:
                    break
            else:
                break
        logging.info("Exiting _fetch_paginated_data()")
    ...
    def get_club_events(self, club_id, timestamp_start=None, timestamp_end=None, member_id=None, schedule_id=None):
        ...
        logging.info(f'Entering get_club_events() for club id {club_id}')
        ...
        logging.info("Exiting get_club_events()")
    ...
    def get_event_participants(self, club_id, event_id=None, timestamp_start=None, timestamp_end=None, fill_guestname=False, schedule_id=None):
        ...
        logging.info("Entering get_event_participants()")
        ...
        logging.info("Exiting get_event_participants()")
    ...
    def get_club_members(self, club_id, club_member_id=None, sync_from=None, from_id=None, with_options=None, any_sub_club=None, rfid_tag=None, external_id=None, email=None, max_results=None):
        ...
        """
        logging.info("Entering get_club_members()")
        ...
        logging.info("Exiting get_club_members()")
