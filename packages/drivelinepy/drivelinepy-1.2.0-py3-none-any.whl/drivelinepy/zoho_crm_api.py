...
class ZohoCrmAPI(BaseAPIWrapper):
            ...
            if data_key in response:
                if isinstance(response[data_key], list):
                    ...
                    if "info" in response:
                        ...
                        logging.error(f"No 'info' key found in the response for page {page}")
                        break
    ...
    def get_customer_records(self, module, customer_id=None, first_name=None, last_name=None, email=None, page=1):
        """
        ...
        """
        logging.info("Entering get_customer_records()")

        if self.check_if_module_is_supported(module):
            endpoint = module
        else:
            logging.error("Invalid module provided.")
            return None
        ...
        logging.info(f"Exiting get_customer_records() with total {len(all_records)} records fetched")
        return all_records
