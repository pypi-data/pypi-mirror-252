...
class ZohoBillingAPI(BaseAPIWrapper):
    ...
    def _refresh_access_token(self):
        ...
        logging.info("Entering _refresh_access_token()")
        ...
        if response is None:
            logging.error("Failed to refresh access token - response is None")
            return False
        else:
            ...
            if "access_token" in response:
                ...
                logging.error("Failed to refresh access token - access token does not exist in response")
                return False
    ...
    def check_if_access_token_needs_refreshed(self):
        ...
        """
        logging.info("Entering check_last_refresh()")

        time_since_last_refresh = datetime.now() - self.time_of_last_refresh

        if time_since_last_refresh.seconds > 3600:
            logging.info("Exiting check_last_refresh() - Token needs to be refreshed")
            return True
        else:
            logging.info("Exiting check_last_refresh() - Token does not need to be refreshed")
            return False
    ...
    def _get_headers(self):
        logging.info("Entering _get_headers()")
        ...
        logging.info(f'Headers have been set!')
        logging.info("Exiting _get_headers()")
        return headers
    ...
    def _fetch_data(self, endpoint, initial_params, data_key, page=1):
        logging.info(f'Entering _fetch_data() for endpoint {endpoint}')
        all_data_list = []
        iteration = 0
        ...
        while True:
            ...
            if iteration > self.api_iteration_limit:
                logging.warning(f"Pagination limit of {self.api_iteration_limit} reached for {endpoint}")
                break
            # if page is none, then we are fetching a single object, so don't include page in the params
            if page is None:
                logging.info(f"Fetching data from {endpoint} for a single object")
            else:
                logging.info(f"Fetching data from {endpoint} for page {page}")
                params["page"] = page
            response = self.get(endpoint, params=params, headers=headers)

            if response is None:
                logging.error(f"Error fetching data from {endpoint} for page {page}. The response is None.")
                break
            ...
            if data_key in response:
                if isinstance(response[data_key], list):
                    ...
                    logging.info(f"Retrieved {data_count} {data_key} from page {page}")
                    if "page_context" in response:
                        if response["page_context"]["has_more_page"]:
                            page += 1
                        else:
                            logging.info(f"No more pages to fetch from {endpoint}")
                            break
                    else:
                        logging.error(f"No 'page_context' key found in the response for page {page}")
                        break
                elif isinstance(response[data_key], dict):
                    all_data_list.append(response[data_key])
                    logging.info(f"Retrieved 1 {data_key} from page {page}")
                    break
                else:
                    logging.error(f"Unexpected data type for {data_key} in response")
                    break               
            else:
                logging.error(f"No '{data_key}' key found in the response for page {page}")
                break

        logging.info(f"Exiting _fetch_data() with total {len(all_data_list)} {data_key} fetched")
    ...
    def get_subscriptions(self, subscription_id=None, customer_id=None, filter_by=None, page=1):
        ...
        """
        logging.info("Entering get_subscriptions()")
        params = {}
        ...
        if subscription_id:
            ...
            if filter_by:
                if not self._validate_subscription_filter(filter_by):
                    logging.error("Invalid subscription status filter provided.")
                    return None
        ...
        logging.info(f"Exiting get_subscriptions() with total {len(all_subscriptions)} subscriptions fetched")
    ...
    def get_customers(self, customer_id=None, customer_name=None, email=None, phone=None, filter_by=None, page=1):
        ...
        """
        logging.info(f"Entering get_customers()")
        ...
        if customer_id:
            ...
            if filter_by:
                if self._validate_customer_status_filter(filter):
                    ...
                    logging.error("Invalid customer status filter provided.")
                    return None  
            params = {
        ...
        logging.info(f"Exiting get_customers() with total {len(all_customers)} customers fetched")
    ...
    def get_invoices(self, invoice_id=None, customer_id=None, subscription_id=None, filter_by=None, page=1):
        ...
        """
        logging.info("Entering get_invoices()")
        params = {}
        endpoint = "invoices"
        data_key = endpoint
        ...
        if invoice_id:
            ...
            if filter_by:
                if self._validate_invoice_filter(filter_by):
                    ...
                    logging.error("Invalid invoice status filter provided.")
                    return None
        ...
        logging.info(f"Exiting get_invoices() with total {len(all_invoices)} invoices fetched")
        return all_invoices
    ...
    def get_credit_notes(self, credit_note_id=None, customer_id=None, customer_name=None, filter_by=None, page=1):
        ...
        """
        logging.info(f"Entering get_credit_notes()")
        ...
        if credit_note_id:
            ...
            if filter_by:
                if self._validate_credit_note_status_filter(filter_by):
                    ...
                    logging.error("Invalid credit note status filter provided.")
                    return None  
            params = {
                "customer_id": customer_id,
        ...
        logging.info(f"Exiting get_credit_notes() with total {len(all_credit_notes)} credit notes fetched")
    ...
    def _validate_filter(self, filter_value, valid_filters):
        logging.info("Entering filter validation")

        if filter_value in valid_filters:
            logging.info("Filter value is valid.")
            return True
        else:
