...
class TrafficMirrorOperation:
    ...
    def create(
        self, ec2_client, reservation, actions, cancellation_context, logger, cloudshell
    ):
        """# noqa
        ...
        # session numbers: session number is the AWS priority of a traffic mirror
        # session; the lower a session number,
        # the earlier it gets the opportunity to capture traffic from a source nic.
        # anyways, session numbers MUST be unique; to provide this behavior, we are
        # using cloudshell pool checkout
        # to read more about session numbers, go here: https://docs.aws.amazon.com/vpc/latest/mirroring/traffic-mirroring-session.html  # noqa
        # to read more about cloudshell pools, go here: https://help.quali.com/Online%20Help/0.0/TestShell-API/TestShell%20Python%20API.html#CheckoutFromPool  # noqa
        ...
        logger.info(
            "Determined session numbers: "
            + ", ".join(str(f.session_number) for f in fulfillments)
        )

        success = False

        try:
            with CheckCancellationThread(
                cancellation_context, self._cancellation_service
            ):
                logger.info("Getting or creating traffic mirror targets...")
                self._get_or_create_targets(ec2_client, reservation, fulfillments)

                logger.info("Creating traffic mirror filters and sessions...")
                self._create_traffic_filters_and_sessions(ec2_client, fulfillments)
    ...
    def _get_or_create_targets(self, ec2_client, reservation, fulfillments):
        ...
        targets_found_nics_to_target_id = (
            self._traffic_mirror_service.find_traffic_targets_by_nics(
                ec2_client, target_nics
            )
        )
    ...
    def _create_traffic_filters_and_sessions(self, ec2_client, fulfillments):
        ...
        for fulfillment in fulfillments:
            ...
            fulfillment.mirror_session_id = self._create_session(
                ec2_client, fulfillment
            )
    ...
    def _get_or_create_sessions(self, ec2_client, fulfillments):
        ...
        session_name_to_found_session = (
            self._traffic_mirror_service.find_sessions_by_session_names(
                ec2_client, session_names
            )
        )
        ...
        for s in session_names:
            ...
            if s not in found_session_names:
                ...
                fulfillment.mirror_session_id = session_name_to_found_session[s][
                    "TrafficMirrorSessionId"
                ]

    def _create_targets_or_assign_existing_targets(
        self,
        ec2_client,
        ...
        for target_nic in target_nics_to_fulfillments.keys():
            if target_nic not in nics_to_found_target_ids:
                ...
                self._assign_target_to_fulfillments(
                    target_nics_to_fulfillments[target_nic], target
                )
            else:
                self._assign_target_to_fulfillments(
                    target_nics_to_fulfillments[target_nic],
    ...
    def _there_are_source_and_target_nics(self, actions):
        for a in actions:
            if not a.actionParams.sourceNicId:
                raise Exception(f"Missing a source nic on actionId {a.actionId}")
            if not a.actionParams.targetNicId:
                raise Exception(f"Missing a target nic on actionId {a.actionId}")
    ...
    def _checkout_session_numbers(self, actions, cloudshell, logger, reservation):
        ...
        [
            source_nic_to_traffic_action[a.actionParams.sourceNicId].append(a)
            for a in actions
        ]
        for source in source_nic_to_traffic_action.keys():
            self.get_unique_session_number_and_assign_to_mirror_session_request(
                cloudshell, logger, reservation, source, source_nic_to_traffic_action
            )

    def get_unique_session_number_and_assign_to_mirror_session_request(
        self, cloudshell, logger, reservation, source, source_nic_to_traffic_action
    ):
        for action in source_nic_to_traffic_action[source]:
            session_number = action.actionParams.sessionNumber
            action.actionParams.sessionNumber = self._session_number_service.checkout(
                cloudshell, logger, reservation, source, session_number
            )
    ...
    def _session_numbers_are_valid(actions, logger):
        ...
        """
        error_msg = (
            "Session number must be either empty or an integer in the range 1-32766"
        )
        ...
        for a in actions:
            try:
                ...
                if (
                    session_number
                    and not isinstance(session_number, int)
                ...
                if a.actionParams.sessionNumber.strip() == "":
                    ...
                    raise ValueError(
                        f"Session number must be an integer, or an empty string! "
                        f"Passed an invalid session number "
                        f"{a.actionParams.sessionNumber} in action {a.actionId}"
                    )
    ...
    def remove(self, ec2_client, reservation, actions, logger, cloudshell):
        ...
        remove_results = [
            RemoveTrafficMirroringResult(actionId=a.actionId, success=False)
            for a in actions
        ]
        ...
        try:
            ...
            TrafficMirrorCleaner.delete_mirror_sessions(ec2_client, session_ids)

            self._releasing_session_numbers(
                cloudshell, reservation, ec2_client, logger, sessions
            )
            ...
            if len(traffic_mirror_filter_ids) > 0:
                ...
                TrafficMirrorCleaner.delete_mirror_filters(
                    ec2_client, traffic_mirror_filter_ids
                )
    ...
    def _releasing_session_numbers(
        self, cloudshell, reservation, ec2_client, logger, sessions
    ):
        session_numbers = [str(s["SessionNumber"]) for s in sessions]
        traffic_mirror_session_network_interface_id = next(
            s["NetworkInterfaceId"] for s in sessions
        )
        TrafficMirrorCleaner.release_session_numbers_from_pool_by_session_ids_and_network_interface_id(  # noqa
            cloudshell,
            self._session_number_service,
            logger,
            reservation,
    ...
    def _find_sessions_to_remove(self, ec2_client, actions):
        ...
        sessions.extend(
            self._traffic_mirror_service.find_sessions_by_session_ids(
                ec2_client, session_ids_from_request
            )
        )
        traffic_mirror_target_nic_ids = [
            a.targetNicId for a in actions if a.targetNicId
        ]
        traffic_mirror_target_ids = self._traffic_mirror_service.find_traffic_mirror_target_ids_by_target_nic_ids(  # noqa
            ec2_client, traffic_mirror_target_nic_ids
        )

        sessions.extend(
            self._traffic_mirror_service.find_sessions_by_traffic_mirror_target_ids(
                ec2_client, traffic_mirror_target_ids
            )
        )
    ...
    def validate_remove_request(request, logger):
        ...
        for a in actions:
            ...
            if not a["sessionId"] and not a["targetNicId"]:
                raise Exception(
                    f"Must have either sessionId or target_nic_id for actionId "
                    f"{a.actionId} but received empty values"
                )

        logger.info("Completed validation for Remove Traffic Mirroring request...")

    def find_traffic_mirror_target_nic_id_by_target_id(
        self, ec2_client, traffic_mirror_target_id
    ):
        return (
            self._traffic_mirror_service.find_traffic_mirror_target_nic_id_by_target_id(
                ec2_client, traffic_mirror_target_id
            )
        )
    ...
    "definitions": {
        ...
        "filterRule": {
            ...
            "properties": {
                ...
                "destinationPortRange": {"type": ["object", "null"]},
                "sourceCidr": {"type": "string"},
                "sourcePortRange": {"type": ["object", "null"]},
