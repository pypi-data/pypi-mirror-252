...
class DeployedAppPortsOperation:
    def __init__(
        self, vm_custom_params_extractor, security_group_service, instance_service
    ):
        """# noqa
    ...
    def get_formated_deployed_app_ports(self, custom_params):
        ...
        :return:
        """
        inbound_ports_value = self.vm_custom_params_extractor.get_custom_param_value(
            custom_params, "inbound_ports"
        )
        outbound_ports_value = self.vm_custom_params_extractor.get_custom_param_value(
            custom_params, "outbound_ports"
        )

        if not inbound_ports_value and not outbound_ports_value:
            return (
                "No ports are open for inbound and outbound traffic outside of the"
                " Sandbox"
            )
    ...
    def get_app_ports_from_cloud_provider(
        self, ec2_session, instance_id, resource, allow_all_storage_traffic
    ):
        """# noqa
        ...
        for network_interface in network_interfaces:
            network_interfaces_dict[network_interface.subnet_id].append(
                network_interface
            )
        ...
        for key, value in network_interfaces_dict.items():
            for network_interface in value:
                ...
                custom_security_group = (
                    self.security_group_service.get_custom_security_group(
                        ec2_session=ec2_session, network_interface=network_interface
                    )
                )

                inbound_ports_security_group = (
                    self.security_group_service.get_inbound_ports_security_group(
                        ec2_session=ec2_session, network_interface=network_interface
                    )
                )
                ...
                for security_group in security_groups:
                    ip_permissions_string = self._ip_permissions_to_string(
    ...
    def _ip_permissions_to_string(self, ip_permissions):
        ...
        for ip_permission in ip_permissions:
            if ip_permission["FromPort"] == ip_permission["ToPort"]:
                ...
                port_str = "{}-{}".format(
                    ip_permission["FromPort"], ip_permission["ToPort"]
                )
                port_postfix = "s"

            result.append(
                "Port{}: {}, Protocol: {}, Source: {}".format(
                    port_postfix,
                    port_str,
    ...
    def _port_rule_to_string(self, port_rule):
        ...
        if port_rule.from_port == port_rule.to_port:
            ...
            port_str = f"{port_rule.from_port}-{port_rule.to_port}"
            port_postfix = "s"

        return f"Port{port_postfix} {port_str} {port_rule.protocol}"
