...
class TestSecurityGroups(TestCase):
    ...
    # def test_instance_sg(self):  # noqa
    #     instance = Mock()  # noqa
    #     instance.security_groups = [Mock(), Mock()]  # noqa
    #     self.sg_service.delete_all_security_groups_of_instance(instance)  # noqa
    #     self.assertTrue(instance.security_groups[0].delete.callled and instance.security_groups[1].delete.callled)  # noqa
    ...
    def test_create_sg(self):
        ...
        self.assertTrue(
            ec2_session.create_security_group.called_with(
                "name",
    ...
    def test_get_default_sg_name(self):
        ...
        security_group_name = SecurityGroupService.CLOUDSHELL_SANDBOX_DEFAULT_SG.format(
            reservation_id
        )
    ...
    def test_get_isolated_sg_name(self):
        reservation_id = "res"
        sg_name = self.sg_service.sandbox_isolated_sg_name(
    ...
    def test_get_security_group_by_name(self):
        ...
        self.assertRaises(
            ValueError, self.sg_service.get_security_group_by_name, vpc, "name"
        )
    ...
    def test_set_shared_reservation_security_group_rules(self):
        ...
        self.sg_service.set_shared_reservation_security_group_rules(
            sg, "man", isolated_sg, True
        )

        self.assertTrue(
            sg.authorize_ingress.called_with(
                IpPermissions=[
                    {
                        "IpProtocol": "-1",
                        "FromPort": -1,
                        "ToPort": -1,
                        "UserIdGroupPairs": [{"GroupId": "man"}],
                    },
                    {
                        "IpProtocol": "-1",
                        "FromPort": -1,
                        "ToPort": -1,
                        "UserIdGroupPairs": [{"GroupId": "id"}, {"GroupId": "dummy"}],
                    },
                ]
            )
        )
    ...
    def test_set_sg_security_group_rules(self):
        ...
        self.assertTrue(
            sg.security_group.authorize_egress.called_with(
                [self.sg_service.get_ip_permission_object(port_data_out)]
            )
        )
        self.assertTrue(
            sg.security_group.authorize_ingress.called_with(
                [self.sg_service.get_ip_permission_object(port_data_in)]
            )
        )
    ...
    def test_remove_allow_all_outbound_rule(self):
        ...
        sg.revoke_egress.assert_called_with(
            IpPermissions=[
                {
                    "IpProtocol": "-1",
                    "FromPort": 0,
