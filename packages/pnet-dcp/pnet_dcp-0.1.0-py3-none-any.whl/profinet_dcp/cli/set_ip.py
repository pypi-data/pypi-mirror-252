...
from profinet_dcp.pnio_dcp import DCP
...
def set_ip_func(com, args):
    ...
    for dev in identified_devices:
        Logging.logger.info(f"Device: {dev}")
    ...
    Logging.logger.info(f"Set IP address for: {device}")
    ip_conf = [args.ip_address, args.subnet_mask, args.gateway_address]

    
    response = dcp.set_ip_address(device.MAC, ip_conf, store_permanent=args.permanent):

    if response:
        Logging.logger.info(f"Successfully applied {ip_conf}")
    else:
        Logging.logger.error(f"Set IP address failed: {response.get_message()}")
