...
class OnlineQueryResponseImpl(OnlineQueryResult):
    ...
    def _df_repr(self, stringify: bool = False):
        info = []
        for x in self.data:
        ...
        if len(self.data) > 0:
            ...
            try:
                try:
                    ...
                    # We were crashing when trying to construct a DF with
                    # a column that has a struct dtype. The struct column
                    # values were serialized as a list of mixed types, which
                    # causes a crash because polars can't infer the dtype.
                    # To circumvent that and since we are concerned only
                    # about displaying a value, we convert all values to a
                    # string here.
                    content = str(pl.DataFrame(self._df_repr(stringify=True)))
            except:
