...
class H2OGPTEAsync:
    ...
    async def delete_collections(self, collection_ids: Iterable[str]) -> Job:
        """
        ...
        return await self._wait_for_completion(_to_id(ret))

    async def delete_documents(self, document_ids: Iterable[str]) -> Job:
        """
    ...
    async def ingest_from_file_system(
        self, collection_id: str, root_dir: str, glob: str
    ) -> Job:
        """
    ...
    async def ingest_uploads(
        self, collection_id: str, upload_ids: Iterable[str]
    ) -> Job:
        """
    ...
    async def ingest_website(self, collection_id: str, url: str) -> Job:
        """
    ...
    async def _get(self, endpoint: str) -> Any:
        res = await self._client.get(self._address + endpoint, timeout=None)
        self._raise_error_if_any(res)
        return json.loads(res.text)
    ...
    async def _post(self, endpoint: str, data: Any) -> Any:
        ...
        res = await self._client.post(
            self._address + endpoint, content=content, timeout=None
        )
        self._raise_error_if_any(res)
        return json.loads(res.text)

    async def _delete(self, endpoint: str) -> Any:
        res = await self._client.delete(self._address + endpoint, timeout=None)
        self._raise_error_if_any(res)
        return json.loads(res.text)
    ...
    async def _job(self, method: str, **kwargs: Any) -> Any:
        return await self._post("/rpc/job", [method, kwargs])

    async def _wait_for_completion(self, job_id: str, timeout=None) -> Job:
        if timeout is None:
            timeout = 86400
        ...
        n = 0
        while True:
            await asyncio.sleep(min(1, dt * pow(2, n)))
            job = await self.get_job(job_id)
            if n < 10:  # Avoid hard math
                n += 1
            if job.completed or job.canceled:
                return job
            if time.time() > deadline:
                raise TimeoutError(f"Job {job.kind} ({job_id}) timed out")

    def _raise_error_if_any(self, res: httpx.Response) -> None:
        if res.status_code == 200:
            return
        message = res.content.decode(errors="replace")
        if res.status_code == 400:
            raise InvalidArgumentError(f"Invalid argument: {message}")
        elif res.status_code == 401:
            raise UnauthorizedError(f"Unauthorized: {message}")
        elif res.status_code == 404:
            raise ObjectNotFoundError(f"Object not found: {message}")
        elif res.status_code == 500:
            raise RuntimeError(f"Server error: {message}")
        else:
            raise httpx.HTTPError(f"HTTP Error {res.status_code}: {message}")
