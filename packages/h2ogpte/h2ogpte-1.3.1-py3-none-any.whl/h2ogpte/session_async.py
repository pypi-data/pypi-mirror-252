...
class SessionAsync:
    ...
    def __init__(self, chat_session_id: str, client: "H2OGPTEAsync"):
        ...
        self.pre_prompt_query: Optional[str] = None
        self.prompt_query: Optional[str] = None
        self.rag_type: Optional[str] = None
        self.hyde_no_rag_llm_prompt_extension: Optional[str] = None
    ...
    async def query(
        ...
        *,
            ...
            rag_config:
                ...
                rag_type: str one of
                    ...
                    :code:`"rag"` RAG (Retrieval Augmented Generation) - RAG with
                        ...
                        contexts from a collection for generating a response.
        ...
        async def send_recv_query(correlation_id: str) -> ChatMessage:
        ...
        return await asyncio.wait_for(send_recv_query(correlation_id), timeout=timeout)
    ...
    async def _poll(self) -> None:
        ...
        for line in data.splitlines():
            raw = json.loads(line)
            if raw["session_id"] != self.chat_session_id:
                continue
            t = raw["t"]
            if t == "cx":
    ...
    def _process_acknowledgment(self, res: ChatAcknowledgement) -> None:
        ...
        for msg in self._messages:
            if msg.correlation_id == res.correlation_id:
                info = msg
                break
        if info is None:  # This shouldn't happen?
            return
        info.query_id = res.message_id
    ...
    async def __aenter__(self) -> "SessionAsync":
        ...
        api_key = self._client._api_key  # type: ignore[reportPrivateUsage]
        ...
        collection = await self._client.get_collection_for_chat_session(
            self.chat_session_id
        )
        self.system_prompt = collection.system_prompt
        self.pre_prompt_query = collection.pre_prompt_query
        self.prompt_query = collection.prompt_query
        self.rag_type = collection.rag_type
        self.hyde_no_rag_llm_prompt_extension = (
