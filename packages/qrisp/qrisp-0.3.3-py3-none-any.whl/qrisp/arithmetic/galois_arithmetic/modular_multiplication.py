...
from qrisp.arithmetic.adders import qcla
from qrisp.qtypes import QuantumFloat
from qrisp import cx, bin_rep, control, swap, invert, mcx
from qrisp.arithmetic.galois_arithmetic.mod_tools import modinv, montgomery_encoder
inpl_adder = qcla
...
def montgomery_red(t, a, b, N, m, permeable_if_zero = False):
    ...
    for k in range(len(a)):
        inpl_adder(-((2**k*b))*modinv(N, 2**(m+1)), u, ctrl = a[k])
...
def montgomery_mod_semi_mul(a, b, output_qg = None, permeable_if_zero = False):
    ...
    if output_qg is None:
        ...
        if output_qg.modulus != N:
            raise Exception("Output QuantumGalois has incompatible modulus")
    ...
    # Typecast to QuantumGalois and increase the Montgomery shift    
    from qrisp import QuantumGalois
    t.__class__ = QuantumGalois
    t.modulus = a.modulus
    t.m = a.m + m_shift
...
def semi_cl_inpl_mult(a, X, ctrl = None, treat_invalid = False):
    ...
    from qrisp import multi_measurement, less_than, redirect_qfunction, QuantumGalois
