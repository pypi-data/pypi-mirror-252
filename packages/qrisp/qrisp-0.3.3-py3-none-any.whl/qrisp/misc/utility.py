...
def multi_measurement(qv_list, shots=10000, backend=None):
    ...
    compiled_qc = qompiler(
        qv_list[0].qs, intended_measurements=sum([qv.reg for qv in temp], []), compile_mcm = False
    )
...
def t_depth_indicator(op, epsilon):
    ...
    elif op.name in ["cx", "cx", "cz", "x", "y", "z", "s", "h", "s_dg", "measure", "reset", "qb_alloc", "qb_dealloc", "barrier"]:
        return 0
