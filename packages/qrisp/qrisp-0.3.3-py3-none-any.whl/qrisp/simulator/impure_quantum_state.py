...
class ImpureQuantumState:
    ...
    def conditionally_apply_operation(self, operation, qubits, clbits, ctrl_state):
        # Perform operation application in parallel. This is possible because many
        # functions in here release the GIL (such as numpy's np.dot)
        if thread_limit > 1:
            threads = []
            ...
            for i in range(len(self.states)):
                for j in range(len(clbits)):
                    if self.outcome_list[i][clbits[j]] != ctrl_state[j]:
                        break
                else:
                    mp_wrapper = lambda: self.states[i].apply_operation(operation.base_op, qubits)
            ...
            for i in range(len(self.states)):
                for j in range(len(clbits)):
                    if self.outcome_list[i][clbits[j]] != int(ctrl_state[j]):
                        break
                else:
                    self.states[i].apply_operation(operation.base_op, qubits)

    def disentangle(self, qubit_index):
        for qs in self.states:
            qs.disentangle(qubit_index)
