...
def cq_gidney_adder(a, b, c_in = None, c_out = None, ctrl = None):
    ...
    for i in range(len(b)-1):
        ...
        if i != 0:
            ...
            # For the reasoning behind this distinction check the comment above the line
            # "if i != len(b) -2:"
            if ctrl is None:
                cx(c_in, b[i])
            else:
                mcx([ctrl, c_in], b[i], method = "jones")
            
            mcx([c_in, b[i]], gidney_anc[i], method = "gidney", ctrl_state = c_inv(a[i]) + "1")
        ...
        if i != len(b) -2:
            if ctrl is None:
                ...
                mcx([ctrl, gidney_anc[i]], b[i+1], method = "jones")
    ...
    if ctrl is None:
        ...
        mcx([ctrl, gidney_anc[-1]], b[-1], method = "jones")
