...
def mcx(controls, target, method="auto", ctrl_state=-1, num_ancilla=1):
    ...
    * ``gidney`` is a very unique way for synthesizing a logical AND. The Gidney Logical AND
      performs a circuit with T-depth 1 to compute the truth value and performs another circuit
      involving a measurement and a classically controlled CZ gate for uncomputation. The
      uncomputation circuit has T-depth 0, such that the combined T-depth is 2. More details
      `here <https://arxiv.org/abs/1709.06648>`_.
      
    * ``amy`` is a Toffoli-circuit (ie. only two control qubits are possible), which
        (temporarily) requires one ancilla qubit. However, instead of the no-ancilla T-depth
        4, this circuit achieves a T-depth of 2. Find the implementation in 
        `this paper <https://arxiv.org/pdf/1206.0758.pdf>`_.
