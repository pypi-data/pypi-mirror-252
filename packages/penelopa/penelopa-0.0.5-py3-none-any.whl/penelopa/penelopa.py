...
_S='Received invalid JSON response from GPT'
_R='updated_code'
_Q='original_code'
_P='line_number'
_O='file_path'
...
def confirm_changes(file_path,changes):
	...
	return _D
class Penelopa:
	def __init__(self,config):self.config=config;self.api_client=AIClient(api_key=config[_J],model=config[_L])
	...
	def run(self):
		...
		logging.info('Running Penelopa');config=self.config.copy();config[_J]='*******';logging.info('Config: %s',config)
		if not self.config[_J]:self.obtain_gpt_key()
		if not self.config[_A]:self.obtain_task_description()
		updated_listing=_E
		if not self.config[_H]or not os.path.exists(self.config[_H])or updated_listing:updated_listing=self.obtain_listing()
	...
	def parse_edit_blocks(self,response_text):
		...
		for match in matches:file_path,line_number,original_code,updated_code=match;edit_blocks.append({_O:file_path.strip(),_P:int(line_number)if line_number else _I,_Q:original_code.strip(),_R:updated_code.strip()})
		logging.info('Edit blocks: %s',edit_blocks);return edit_blocks
	def process_edit_blocks(self,edit_blocks):
		for block in edit_blocks:
			file_path=block[_O];line_number=block.get(_P);original_code=block[_Q];updated_code=block[_R];change_confirmed=self.confirm_change(file_path,original_code,updated_code)
	...
	def apply_change_to_file(self,file_path,original_code,updated_code,line_number=_I):
		MATCH_LENGTH=10
		with open(file_path,'r')as file:file_contents=file.readlines()
		if not file_contents or line_number is not _I:self.handle_new_code(file_contents,updated_code,line_number);updated=_D
		else:
	...
	def handle_new_code(self,file_contents,updated_code,line_number):
		if line_number is not _I:file_contents.insert(line_number-1,updated_code)
	...
	def refine_task_based_on_questions(self,original_task,questions,user_responses):
		...
		user_message+='\n Write only refined task in the response. The refined task should be about '+self.config[_B]+' itself only that you know (in file listing).';thread,run=self.api_client.create_thread_and_run(user_message,self.config[_C]);completed_run=self.api_client.wait_for_completion(thread.id,run.id);thread_messages=self.api_client.get_thread_messages(thread.id);refined_task=thread_messages[0].content[0].text.value;logging.info('Refined task: %s',refined_task);return refined_task
	def obtain_clarifying_questions_for_task(self,task_description):
		user_message=f'''
                ...
                Given the task description: "\'{self.config[_A]}\'" (and this task will be about the project \'{self.config[_B]}\' itself, that you know in the file listing), generate a list of clarifying questions along with options for possible answers. The response should be in JSON format, where each question is an object with "question" and "answers" fields.
                Example format:
                [
                    {{
                ...
                ''';thread,run=self.api_client.create_thread_and_run(user_message,self.config[_C]);completed_run=self.api_client.wait_for_completion(thread.id,run.id);thread_messages=self.api_client.get_thread_messages(thread.id);clarifying_questions_json=thread_messages[0].content[0].text.value
		try:clarifying_questions=extract_json_from_text(clarifying_questions_json)
		except json.JSONDecodeError:raise ValueError(_S)
	...
	def request_code_update(self,file,file_content,refined_task):
		user_message=f"""
                        Given the task description: '{refined_task}' for project '{self.config[_B]}', generate a code update for the file '{file}': \"'{file_content}'\" (of the project '{self.config[_B]}' itself, that you know in the file listing). Provide a direct response in git-style edit block format. The response should detail the changes to be made to the file using git diff style blocks, including additions at specific line numbers if necessary.
                        Example formats:
                        ```diff
                        {file}
                        <<<<<<< HEAD
                        ...
                        """;thread,run=self.api_client.create_thread_and_run(user_message,self.config[_C]);valid_response_received=_E;attempt=0;max_attempts=10
	...
	def obtain_relevant_files(self,refined_task):
		user_message=f'''
                Given the task description: "\'{refined_task}\'" for project "\'{self.config[_B]}\'" (that you know via file listing), generate a list of files that are relevant to the task that need to be updated. The response should be in JSON format, where each file is an object with the full file path.
                Example format:
                {{
                    "files": ["path/to/file1", "path/to/file2"]
                }}
                ''';thread,run=self.api_client.create_thread_and_run(user_message,self.config[_C]);completed_run=self.api_client.wait_for_completion(thread.id,run.id);thread_messages=self.api_client.get_thread_messages(thread.id);files_json=thread_messages[0].content[0].text.value
	...
	def check_and_create_assistant(self,updated_listing=_E):
		A='assistants'
		if not self.config[_C]:self.obtain_assistant_id(self.api_client.client);file=self.api_client.client.files.create(file=open(self.config[_H],'rb'),purpose=A);updated_listing=_E;self.api_client.client.beta.assistants.files.create(assistant_id=self.config[_C],file_id=file.id)
		if updated_listing:file=self.api_client.client.files.create(file=open(self.config[_H],'rb'),purpose=A);self.api_client.client.beta.assistants.files.create(assistant_id=self.config[_C],file_id=file.id)
	def obtain_listing(self):lister=CodebaseLister(output_filename=self.config[_B]+'.txt');result=lister.generate_listing_file();logging.info(f"Listing file generated: {result}");output_filename=result['output_filename'];self.config[_H]=output_filename;self.save_config();return _D
	def obtain_assistant_id(self,client):
		logging.info('Obtaining assistant ID')
		if self.config[_C]:logging.info('Assistant ID already exists');return
		my_assistant=client.beta.assistants.create(instructions='You are a personal code exptert assistant about the project '+self.config[_B],name=self.config[_B]+' assistant-'+datetime.now().strftime('%Y%m%d%H%M%S'),tools=[{'type':'retrieval'}],model=self.config[_L]);self.config_manager.update_config_values(assistant_id=my_assistant.id);return my_assistant.id
	def obtain_gpt_key(self):
		logging.info('Obtaining OpenAI GPT key')
		if self.config[_J]:logging.info('OpenAI GPT key already exists');return
		dialog=PenelopaDialog('Please enter your OpenAI GPT key: ');user_response=dialog.run();logging.info('User response: **********');self.config_manager.update_config_values(gpt_key=user_response)
...
def main():
	...
	for(key,value)in vars(args).items():
		if value is not _I:config[key]=value
