_W='Chat completion: %s'
_V='max_tokens'
_U='temperature'
_T='The user task is: '
_S=' in terms to solve the user task: '
...
class Penelopa:
	...
	def check_and_create_assistant(self,updated_listing=_E):
		A='assistants'
		if not self.config[_C]:self.obtain_assistant_id(self.api_client.client);full_path_of_listing=os.path.abspath(self.config[_I]);file=self.api_client.client.files.create(file=open(full_path_of_listing,'rb'),purpose=A);updated_listing=_E;self.api_client.client.beta.assistants.files.create(assistant_id=self.config[_C],file_id=file.id)
		if updated_listing:file=self.api_client.client.files.create(file=open(self.config[_I],'rb'),purpose=A);self.api_client.client.beta.assistants.files.create(assistant_id=self.config[_C],file_id=file.id)
	def obtain_listing(self):lister=CodebaseLister(output_filename=self.config[_B]+'.txt');result=lister.generate_listing_file();logging.info(f"Listing file generated: {result}");output_filename=result['output_filename'];self.config[_I]=output_filename;self.save_config();return _D
	def obtain_assistant_id(self,client):
		...
		my_assistant=client.beta.assistants.create(instructions='You are a personal code exptert assistant about the project '+self.config[_B],name=self.config[_B]+' assistant-'+datetime.now().strftime('%Y%m%d%H%M%S'),tools=[{'type':'retrieval'}],model=self.config[_L]);self.config[_C]=my_assistant.id;self.save_config();return my_assistant
	...
	def obtain_files_to_analyze(self,client,project_structure):logging.info('Obtaining files to analyze');system_message='You are API that return files those need to be analyzed for the project '+self.config[_B]+_S+self.config[_A];user_messages=_T+self.config[_A]+'The structure of the project is: '+'\n '+project_structure+"\n Which file we need to analyze before solving a user task? Write the answer in the next json format: \n {'files': [\n   {'file_path': 'path/to/file'},\n ]}";chat_messages=[{_F:_K,_G:system_message}]+[{_F:'user',_G:user_messages}];chat_completion=client.chat.completions.create(messages=chat_messages,model=self.config['fine_tuned'],temperature=self.config[_U],max_tokens=self.config[_V],top_p=self.config['top_p']);response=chat_completion.choices[0].message.content.strip();logging.info(_W,response);chat_messages.append({_F:_K,_G:response});return chat_messages,response
	def obtain_commands_for_file(self,client,file,chat_messages):
		logging.info('Obtaining commands for file')
		if os.path.isfile(file):
			with open(file,'r')as f:file_content=f.read();system_message='You are API that return commands for file '+file+_S+self.config[_A];user_messages=_T+self.config[_A]+"The content of the file is: '"+file_content+"'\n Which commands we need to execute before solving a user task? Write the answer in the next json format: \n {'commands': [\n   {'line': number_of_line, 'new_line': 'new_line'},\n ]}";chat_messages.append({_F:_K,_G:system_message});chat_messages.append({_F:'user',_G:user_messages});chat_completion=client.chat.completions.create(messages=chat_messages,model=self.config[_L],temperature=self.config[_U],max_tokens=self.config[_V],top_p=self.config['top_p']);response=chat_completion.choices[0].message.content.strip();logging.info(_W,response);chat_messages.append({_F:_K,_G:response});return chat_messages
		else:return[]
def main():
