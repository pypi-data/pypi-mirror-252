...
class Dispatcher:
    ...
    def __init__(self, client: "pyrogramv2.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return (
                await pyrogramv2.types.Message._parse(self.client, update.message, users, chats,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await pyrogramv2.types.CallbackQuery._parse(self.client, update, users),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                pyrogramv2.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                pyrogramv2.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                pyrogramv2.types.Poll._parse_update(self.client, update),
                PollHandler
            )

        async def chosen_inline_result_parser(update, users, chats):
            return (
                pyrogramv2.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                pyrogramv2.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                pyrogramv2.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ...
                async with lock:
                    for group in self.groups.values():
                        for handler in group:
                            ...
                            except pyrogramv2.StopPropagation:
                                raise
                            except pyrogramv2.ContinuePropagation:
                                continue
                            except Exception as e:
                                log.exception(e)
