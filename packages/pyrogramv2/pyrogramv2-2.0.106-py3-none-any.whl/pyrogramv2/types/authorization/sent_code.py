...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~pyrogramv2.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~pyrogramv2.Client.sign_in` or :meth:`~pyrogramv2.Client.sign_up`).

        next_type (:obj:`~pyrogramv2.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~pyrogramv2.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~pyrogramv2.Client.resend_code`.
    """
