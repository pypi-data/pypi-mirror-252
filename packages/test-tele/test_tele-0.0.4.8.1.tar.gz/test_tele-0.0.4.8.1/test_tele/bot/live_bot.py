...
async def get_images_from_inline(event, is_premium:bool = False):
    ...
    n = 10
    category = query.split()[0] # ambil .px | .md

    await event.client(SetTypingRequest(event.input_chat, SendMessageUploadPhotoAction(0)))

    if query.startswith('.px'):
        from test_tele.features.extractors.pixiv import get_pixiv_list, clean_up_tags, PIXIV_MODE
        px_mode, keywords = await clean_up_tags('no_inline ' + query, PIXIV_MODE, '.px')
