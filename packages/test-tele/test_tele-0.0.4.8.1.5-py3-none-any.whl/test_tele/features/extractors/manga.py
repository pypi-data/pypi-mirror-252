...
async def get_chapter_images(link: str, pid: int = 0, offset=OFFSET):
    ...
    if response.status_code == 200:
        ...
        prev_ch = soup.find('a', {"q:key": "0B_8"})['href']
        next_ch = soup.find('a', {"q:key": "0B_9"})['href']
        ...
        for i, obj in enumerate(data_objs):
            if i > data_objs.index('repeat'):
                image_started = True
            if image_started and isinstance(obj, str):
                if obj.startswith("https://xfs"):
                    if n >= pid:
                        chapter_dict['images'] = obj
                    n += 1
                    if n >= offset:
                        break
