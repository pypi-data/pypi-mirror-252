...
from .git_credential import AddonsCredentialGit, GitCredentialManager
...
def _cmd_setup_git(env_vars: Dict[str, str], filter: List[str], dry_run: bool = True) -> List[List[str]]:
    ...
    for extractor in manager.create_extractors():
        ...
        if filter and ad_def.name not in filter:
    ...
    if not filter or "GITLAB_CI" in filter:
        addon = AddonsCredentialGit.extract_ci_credential(env_vars)
        if not addon:
            return result

        cmd = manager.get_cmd(addon)
        result.append(cmd)
        if dry_run:
            _logger.info("> %s", " ".join(cmd))
        else:
