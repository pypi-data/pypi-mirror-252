...
class GitOdooAddons(AddonsSuffix):
    ...
    def __init__(self, base_key):
        super(GitOdooAddons, self).__init__(base_key, "ADDONS_GIT")
        ...
        self.PROTOCOLE = self.create_key("PROTOCOLE", default=PROTOCOLE_PUBLIC)
    ...
    def _get_protocole(self, env_vars: Dict[str, str], res: Dict[KeySuffix, str]):
        if (
            not self.PROTOCOLE.get_value(env_vars, use_default_key=False, use_default_value=False)
            and res[self.HTTPS_LOGIN]
            and res[self.HTTPS_PASSWORD]
        ):
            return PROTOCOLE_HTTPS
    ...
    def _apply_check(self, env_vars: Dict[str, str], res: Dict[KeySuffix, str]):
        ...
        if res[self.PROTOCOLE] not in FORMAT_GIT_CLONE.keys():
            raise ValueError(
                "The selected protocole %s is not supported, possible values are %s"
                % (res[self.PROTOCOLE], sorted(list(FORMAT_GIT_CLONE.keys())))
            )
        if PROTOCOLE_SSH == res[self.PROTOCOLE]:
            raise ValueError("Protocole [%s] not supported for the moment" % PROTOCOLE_SSH)
...
class GitOdooAddonsResult(OdooAddonsDef):
    def __init__(
        self,
        name,
        gitman_config: GitManAddonsConfig,
        git_path,
        branch,
        clone_path,
        pull_options,
        ...
        super(GitOdooAddonsResult, self).__init__(name, gitman_config)
        ...
        self.server = server
        self.format = FORMAT_GIT_CLONE[protocole]
...
class GitSubDirOdooAddons(ABCSubDirAddons):
    ...
    def __init__(self, base_key: str):
        super().__init__(base_key, "ADDONS_SUBDIR_GIT")
...
class LegacyDepotGit(AddonsSuffix):
    def __init__(self, base_key: str):
        super().__init__(base_key, "DEPOT_GIT")
        self.CLONE_PATH = self.create_key("CLONE_PATH")
    ...
    def extract(self, env_vars: Dict[str, str]) -> LegacyDepotGitResult:
        ...
        return LegacyDepotGitResult(
            self.NAME.full_key, GitManAddonsConfig(clone_path), clone_path=clone_path, value_clone=value_clone
        )
...
class LegacyDepotGitResult(OdooAddonsDef):
    def __init__(
        ...
        super(LegacyDepotGitResult, self).__init__(name, gitman_config)
    ...
    def arg_cmd(self) -> List[List[str]]:
