...
class AddonsCredentialGit(EnvKeyExtractor[GitCredential]):
    ...
    @staticmethod
    def extract_ci_credential(env_vars: Dict[str, str]) -> Optional[GitCredential]:
        if env_vars.get("CI") and env_vars.get("CI_JOB_TOKEN") and env_vars.get("CI_SERVER_HOST"):
            return GitCredential(
                name="GITLAB_CI",
                host=os.getenv("CI_SERVER_HOST"),
                username=os.getenv("CI_JOB_TOKEN_LOGIN", "gitlab-ci-token"),
